<?php
/**
 * Tickets/gov_dept_crp.php — حوكمة مركز البلاغات
 * ─────────────────────────────────────────────────────────────────────────
 * ظهورٌ نطاقيٌّ لمكوّن «حوكمة الإدارة» الواحد على مركز البلاغات:
 ...
 * يمنح، ونطاقُه `gov_dept_crp` يُطابَق على سجلِّ الشاشاتِ في المعالج.
 *
 * ◆ عدّادُ الأحداثِ مقصورٌ على `source_module = 'tickets'` — فما ظهر صفرًا
...
$__pp = check_page_permissions($conn, 'Tickets/gov_dept_crp.php');
...
$GOV_DEPT = array(
    'title'           => 'حوكمة مركز البلاغات',
    'icon'            => 'fa fa-scale-balanced',
    'module_like'     => array('Tickets/'),
    'team_roles'      => array(24),
    'sensitive_like'  => 'ticket%',
    'events_module'   => 'tickets',
    'attest_endpoint' => '../Governance/gov_m14_actions.php',
    'attest_code'     => 'gov.crp.attest',
    'attest_scope'    => 'gov_dept_crp',
    /* لا قياسَ فصلِ واجباتٍ معرَّفًا بعقودِ هذه الإدارة — يُعلَن ولا يُلفَّق */
    'sod_queries'     => array(),
);

/* ── UXW-01 · القشرة (بوابة ٤): مكوّنُ «حوكمة الإدارة» التالي هو الذي يُنفّذ
   include '../inheader.php' (القشرةُ الموحَّدة) ثم insidebar.php قبل التصيير
   (dept_gov_space §القالب) — فالشاشةُ داخلَ القشرةِ لا خارجَها. */
...
if (function_exists('ems_states_bundle')) {
    echo ems_states_bundle('لا قياساتِ حوكمةٍ ضمن نطاق مركز البلاغات',
                           'العدّادُ مقصورٌ على أحداثِ tickets — وما ظهر صفرًا يعني «لا حدثَ منشورٌ باسمِ هذه الإدارة»');
}
