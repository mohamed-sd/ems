...
foreach ($rows as $x) {
    ...
    /* ② مساحةٌ شخصيةٌ أو منظرُ الإدارةِ عن نفسِها؟
       ◆ **ولا يُوثَق بعمودِ `owner_kind` وحدَه**: `main/role_board.php` يظهر في
         **الثمانيَ عشرةَ مساحةً كلِّها** ومالكُه «مساحةُ العملِ الشخصية»، ومع ذلك
         عمودُه `BUSINESS_DEPARTMENT`. فسقطَ من العقدةِ الثانيةِ إلى السادسةِ
         وصار «ممنوعًا» — **فحُذفت لوحةُ الدورِ من تسعِ مساحاتٍ ولوحةُ العملِ من
         أربعَ عشرة**. وكشفَ ذلك **مصفوفةُ الحفظِ** لا القياسُ: 893 ⇒ 612.
       ◆ فيُقرأ الشرطُ من ثلاثةِ شواهدَ لا من عمودٍ واحد: نوعُ المالكِ **أو**
         كونُ المالكِ المساحةَ الشخصيةَ نفسَها **أو** الانتشارُ في ثلثَي المساحات.
         **وعمودٌ واحدٌ خاطئٌ يهدم شجرةً كاملة.** */
    elseif ((($x['owner_kind'] === 'PLATFORM_SHARED')
             || (mb_strpos($x['owner_dept_ar'], 'مساحة العمل') !== false)
             || ($sp >= 12)) && $sp >= 12) {
        $cls = 'PERSONAL_SPACE'; $step = 2;
        $why = "منصةٌ مشترَكةٌ بحكمِ الانتشار: تظهر في {$sp} مساحةً — ما يظهر لكلِّ إدارةٍ ليس تسرّبًا";
    }
    elseif ((($x['owner_kind'] === 'PLATFORM_SHARED')
             || (mb_strpos($x['owner_dept_ar'], 'مساحة العمل') !== false)) && $sp >= 4) {
