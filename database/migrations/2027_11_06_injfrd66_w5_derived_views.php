 ...
 *
 * ◆ INJFIX-SENSITIVE-EXEMPT — **مسٌّ تحقُّقًا لا بثًّا** (GAP-10 · القناة ⑤):
 *   المنظرانِ يمسّانِ أعمدةَ `suppliers` الحسّاسةَ (`bank_account_no` ·
 *   `tax_number` · `identity_number` · `bank_iban` · `commercial_registration`)
 *   **داخلَ مسنداتِ وجودٍ فقط** (`IS NULL` · `<> ''`) فيُخرجان قيمًا منطقيّةً
 *   (`has_bank_account` …) وعدَّ نواقصَ — **ولا يُخرجان القيمةَ الخامَّ في أيِّ
 *   عمودِ إخراج** (مقيسٌ: صفرُ مرجعٍ حسّاسٍ خارجَ مسند). فوجودُ الحسابِ خبرُ
 *   اكتمالِ تأهيلٍ لا سرٌّ — وقيمتُه تبقى خلفَ المقرِّرِ في قنواتِ العرض.
 *
