<?php
/**
 * 2027_11_10_injfrd66_sup18_entitlements_grant.php
 *   SUP-18 — فتحُ تبويبِ «الاستحقاقات» **بلا بندِ تنقّل**
 * ═══════════════════════════════════════════════════════════════════════════
 ...
 *
 * ◆ **والقفلُ الثالثُ يُترك مغلقًا بنصِّ المتطلبِ لا بسهو**: `SUP-18` نصُّه
 *   «تبويبٌ داخل التسويات… **لا شاشةٌ مستقلة**». فصفٌّ في `nav_items` يجعله
 *   شاشةً مستقلةً وهو ما يمنعه النصّ. والبلوغُ من شريطِ ملفِّ التسوية.
 *
 * ◆ **وسلسلةُ البلوغِ هنا بخطوتَين لا خطوة**: شريطُ ملفِّ التسويةِ لا يُصيَّر في
 *   `settlements.php` (له شريطُ عائلةٍ آخر — **وإقحامُ ثانٍ يُصيِّر شريطَين**،
 *   وهو عطبٌ مقيسٌ في ثمانيةِ أسطحٍ من قبل)، بل في `supplier_advances.php`.
 *   فالبلوغُ: سجلٌّ حيٌّ ⇐ شريطُ عائلةٍ ⇐ سلفياتٌ ⇐ شريطُ ملفِّ التسوية.
 *
 * ◆ **وقراءةٌ لا كتابةَ البتّة**: معيارُ `SUP-18` «**صفر استحقاقٍ مُدخَلٍ يدويًّا**»
 *   — فالسطحُ منظرٌ، و`can_add/edit/delete = 0` **لكلِّ دورٍ بلا استثناء**،
 *   والمالكُ نفسُه لا يكتب فيه. **وصلاحيةُ كتابةٍ لا يستعملها أحدٌ اليومَ
 *   يستعملها أحدٌ غدًا.**
 *
 * ◆ **ولا يُخترَع صفُّ موديولٍ ثانٍ لمسارٍ له صفّ** — يُفحَص الوجودُ قبلَ الإدراج.
 *
 * التشغيل:  php database/migrations/2027_11_10_injfrd66_sup18_entitlements_grant.php [--revert]
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
$SEEDED = 'injfrd66:2026-08-23:sup18_entitlements';

/* ◆ `owner => 0` تعني **لا كاتبَ البتّة**: منظرٌ يُقرأ ولا يُكتَب فيه. */
$SURFACES = array(
    array(
        'route'  => 'Suppliers/supplier_entitlements.php',
        'name'   => 'استحقاقات الموردين',
        'parent' => 'Suppliers/settlements.php',
        'owner'  => 0,
        'space'  => 'ادارة الموردين',
        'tab'    => 'التسويات والاستحقاقات',
        'screen' => 'استحقاقات الموردين',
        'dept'   => 'ادارة الموردين',
    ),
);

echo "\n══ INJ-FRD-01 · SUP-18 — فتحُ تبويبِ الاستحقاقات ══\n";
...
foreach ($SURFACES as $S) {
    ...
    if ($mod) { $MID = (int) $mod['id']; $skip[] = 'الموديل قائمٌ سلفًا #' . $MID; }
        ...
        $ownerCol = $S['owner'] > 0 ? $S['owner'] : null;
        $i->bind_param('ssii', $S['name'], $S['route'], $ownerCol, $gid);
    ...
    if ($S['owner'] > 0 && !in_array($S['owner'], $viewers, true)) { $viewers[] = $S['owner']; }
    sort($viewers);
    $added = 0;
    foreach ($viewers as $rid) {
        $w = ($S['owner'] > 0 && $rid === $S['owner']) ? 1 : 0;
    ...
    printf("     ② صلاحية: %d دورًا يقرأ · %s (أُدرج %d)
",
        count($viewers),
        $S['owner'] > 0 ? 'والكتابةُ للدور ' . $S['owner'] . ' وحدَه'
                        : '**وصفرُ كاتبٍ — منظرٌ يُقرأ ولا يُكتَب فيه**',
        $added);
    ...
    $q = $conn->query("SELECT DISTINCT g.profile_id
                         ...
                         JOIN gov_role_profiles p ON p.profile_id = g.profile_id AND p.state = 'active'
                         JOIN users u ON u.id = g.user_id
                        WHERE COALESCE(u.role_id, NULLIF(CAST(u.role AS UNSIGNED),0)) = " . (int) ($S['owner'] > 0 ? $S['owner'] : -999) . "
                          AND g.revoked_at IS NULL AND (g.valid_to IS NULL OR g.valid_to > NOW())");
