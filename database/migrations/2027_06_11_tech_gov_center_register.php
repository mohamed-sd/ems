<?php
/**
 * 2027_06_11_tech_gov_center_register.php
 * ═══════════════════════════════════════════════════════════════════════════
 * تسجيلُ شاشةِ «مركز الحوكمة التقني — الروابط اليتيمة» ثلاثيَّ الأقفال
...
$CODE = 'Governance/tech_gov_center.php';
...
if ($mid === null) {
    $conn->query("INSERT INTO modules (name, code, owner_role_id, is_link, is_quick, icon, display_order)
                  VALUES ('مركز الحوكمة التقني — الروابط اليتيمة', '{$CODE}', 15, '0', 0, 'fa fa-scale-balanced', 610)");
    $mid = $conn->insert_id;
...
if ((int) $exists === 0) {
    $conn->query("INSERT INTO nav_items (role_id, door, group_id, module_id, label_ar, route, icon, sort_order, permission_code, active)
                  VALUES (15, 'GOV', {$grpSql}, {$mid}, 'مركز الحوكمة التقني', '{$CODE}', 'fa fa-scale-balanced', 90, '{$CODE}', 1)");
