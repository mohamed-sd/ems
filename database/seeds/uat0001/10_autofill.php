...
/* ═══════════════════════════════════════════════════════════════════════════
 * ◆ **الاستثناءُ المُلصَقُ بيدٍ رُفع — والمانفستُ صار المصدرَ الواحد.**
 * ═══════════════════════════════════════════════════════════════════════════
 * كان هنا `array('supplier_contracts', 'supplier_contract_lines')` مكتوبًا
 * بيدٍ لأنهما غائبان عن القسم ج. وذلك **سجلٌّ ثانٍ للمحرَّم**: من يقرأ المانفستَ
 * يظنُّهما مسموحَين، ومن يقرأ هذا السطرَ يظنُّهما محرَّمَين — وهو عينُ العيبِ
 * الذي يُنتج الفسادَ. وقد تكرَّر فعلًا: `fin_bank_accounts` و
 * `fin_bank_statement_lines` غائبان كذلك ولم يكن لهما سطرٌ مُلصَقٌ، **فبُذرا
 * خامًا** (18 سطرًا بمتسلسلٍ بخطوةِ 137 · سندان مطابَقان مرتين · 12 مرجعًا يتيمًا).
 *
 * ⇒ فأُدرجت الأربعةُ في القسم ج (المانفست 2026-08-12)، ورُفع الاستثناءُ من هنا،
 *   ومحلَّه **حارسٌ يفضح الانحراف**: قائمةٌ مُعلَنةٌ من الجداولِ المعروفةِ أنها
 *   محكومةٌ بخدمةٍ، وإن غاب أحدُها عن المانفستِ **يُوقَف المِلءُ** ويُسمَّى.
 *   فالمانفستُ مصدرٌ واحدٌ، والحارسُ يمنع أن يعود ناقصًا صامتًا.
 * ═══════════════════════════════════════════════════════════════════════════ */
$MUST_BE_IN_MANIFEST = array(
    'supplier_contracts'        => 'محكومٌ بـSupplierContractService · و`unit` فيه مفرداتٌ مُعرَّفة',
    'supplier_contract_lines'   => 'كُتب فيه نصُّ ملاحظةٍ في عمودِ الوحدةِ 14 مرةً من 20',
    'fin_bank_accounts'         => 'حسابٌ بنكيٌّ لا يُخترع — والقسمُ كان يُدرج bank_* ويُغفل fin_bank_*',
    'fin_bank_statement_lines'  => 'بُذر 18 سطرًا ملفَّقًا · سندان مطابَقان مرتين · 12 مرجعًا يتيمًا',
);
$missingFromManifest = array();
foreach ($MUST_BE_IN_MANIFEST as $tbl => $why) {
    if (!in_array($tbl, $cycleOnly, true)) { $missingFromManifest[$tbl] = $why; }
}
if ($missingFromManifest) {
    fwrite(STDERR, "\n✘ جداولٌ محكومةٌ بخدمةٍ وغائبةٌ عن القسم ج في المانفست — أُوقف المِلءُ:\n");
    foreach ($missingFromManifest as $tbl => $why) {
        fwrite(STDERR, "   · {$tbl} — {$why}\n");
    }
    fwrite(STDERR, "   أضِفها إلى «## ج» في {$manifestPath} ثم أعِد التشغيل.\n"
                 . "   ولا تُلصق استثناءً هنا: سجلّان للمحرَّمِ يتفرَّقان دائمًا.\n");
    exit(1);
}
$SKIP = array_values(array_unique(array_merge($SKIP, $cycleOnly)));
fwrite(STDOUT, '  · مُستثنًى بحكمِ المانفست (قسم ج): ' . count($cycleOnly) . " جدولًا\n");
fwrite(STDOUT, '  · وحارسُ المانفست: ' . count($MUST_BE_IN_MANIFEST) . " جدولًا محكومًا كلُّها مُدرَجةٌ ✔\n");
