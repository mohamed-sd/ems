...
function migrate_run_php_file($path)
{
    ...
    if ($code !== 0) {
        return 'رمز خروج غير صفري: ' . $code;
    }
    /* ══ **الصمتُ ليس نجاحًا.** المُشغِّلُ ينفّذ ملفَّ الـphp **سكربتًا قائمًا
         بذاته** ويحكم برمزِ الخروجِ وحدَه. فملفٌّ كُتب بعقدٍ آخر — مثل
         `return array('up' => …)` الذي يُنتظَر أن يناديه أحدٌ — لا ينفّذ حرفًا،
         ويخرج بصفرٍ، **فيُسجَّل «مُطبَّقًا»** ولا يمسُّ القاعدةَ. وقد حدث فعلًا:
         هجرةٌ سُجِّلت ناجحةً في 511ms وأثرُها في القاعدةِ صفر.
       ⇒ ولأن كلَّ هجرةٍ في هذا المستودعِ **تُعلن ما فعلت** (شاهدٌ مُشغَّلٌ في
         آخرِها)، فمخرَجٌ فارغٌ دليلُ عقدٍ مخالفٍ لا دليلُ عملٍ نظيف. */
    $said = '';
    foreach ($output as $line) { $said .= trim($line); }
    if ($said === '') {
        return 'الملف خرج بصفرٍ ولم يُخرِج حرفًا — عقدٌ مخالف: هجرةُ php سكربتٌ '
             . 'قائمٌ بذاته يُعلن ما فعل، لا مصفوفةً تُعاد (up/down) لا يناديها أحد.';
    }
    return '';
}
