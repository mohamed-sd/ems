<?php
/**
 * [مُستثنى بنيويًا — طبقة الشركة/مصادقة قبل-الجلسة · دفعة هـ-2]
 * ─────────────────────────────────────────────────────────────────────────────
 * مكتبة مصادقة طبقة الشركة (بوابة الاشتراك SaaS) بجلستها المستقلة
 * $_SESSION['company_user'] — طبقةُ هويةٍ ثالثة لا يُنمذِجها عقد البوابة بعد (البوابة
 * تعرف سياقَي fromSession للمستأجر وfromSuperAdminSession للمزوّد فقط). ودوالُّ الدخول
 * هنا (قراءة users بالبريد/الاسم، وسجل التدقيق، وقراءة الاشتراك والصلاحيات) تجري أثناء
 * مصافحة الدخول قبل اكتمال أيّ جلسة — فإجبار البوابة إما يُغلق فيكسر الدخول أو يمنح
 * قراءةً لطلبٍ مجهول. تبقى نداءاتها خامًا كعائلةٍ متماسكة بانتظار قناة سياقٍ لطبقة
 * الشركة (fromCompanySession) إن قُرِّرت لاحقًا — القرار في docs/PROVIDER_BATCH_POLICY_ar.md §هـ-2.
 */
