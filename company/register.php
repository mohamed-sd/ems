...
/* ══ تجميدُ الشركةِ الواحدة (FREEZE · single-company) ══════════════════════
   ◆ **تسجيلُ شركةٍ جديدةٍ لا معنى له**: النظامُ يدير شركةً واحدةً مثبَّتةً،
     فبابُ الاشتراكِ الذاتيِّ بابُ منصّةٍ لا بابُ منتَج.
   ◆ **والحظرُ يحمي جدولَين**: `admin_subscription_plans` و
     `admin_subscription_requests` لا يُكتَب فيهما بعدَ اليوم — فيؤجَّل حذفُهما
     بأمانٍ إلى دفعةٍ لاحقةٍ بلا صفوفٍ جديدةٍ تتراكم.
   ⛔ لا يُحذف الملفُّ — الإغلاقُ قرارُ تشغيلٍ قابلٌ للنقضِ بحذفِ هذه الكتلة. */
header('HTTP/1.1 403 Forbidden');
echo 'Registration is disabled in single-company mode.';
exit();

/**
 * [مُستثنى بنيويًا — تسجيلٌ عامٌّ قبل-الجلسة + تزويدٌ عابر · هـ-2] تسجيلٌ ذاتيٌّ عمومي
 * (لا هويةَ بعد): يقرأ خطط الاشتراك ويُنشئ طلبًا/شركةً+مديرها في admin_companies/users
 * ببنّاءٍ عبر الطبقتين — عائلةُ «المصادقة قبل-الجلسة» + «التزويد العابر» معًا (كمسار
 * قبول الطلب في admin/subscriptions). تبقى نداءاته خامًا. السياسة §هـ-2.
 */
...
ems_public_head('طلب تسجيل شركة | إيكوبيشن'); ?>
    <style>
