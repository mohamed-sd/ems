...
$__pc = ems_post_contract($conn, array(
    ...
    'validate' => function (array $in) {
        ...
        if ($wit === '') { return array('ok' => false, 'msg' => 'شاهد التسليم إلزامي (422)'); }
...
$header_title_html = htmlspecialchars('الوصول والتسليم', ENT_QUOTES, 'UTF-8');
$header_actions = array();
$header_back = false;
include __DIR__ . '/../includes/page_header.php';
// UXW-01 ⑨: حالاتُ الشاشةِ الدنيا (تحميل · فراغ · خطأ) — مخفيةٌ افتراضًا
echo ems_states_bundle('لا أوامر واصلة بانتظار توثيق التسليم', 'أكد الوصول من شاشة «الحركة في الطريق» ليظهر الأمر هنا');
?>
  ...
  <table class="table table-striped" data-no-dt>
    ...
    <tbody>
    <?php if (empty($rows)): ?><tr><td colspan="6" class="text-center text-muted">لا أوامر واصلة بانتظار التسليم</td></tr><?php endif; ?>
    <?php foreach ($rows as $o): ?>
      <tr>
        ...
        <td><?= $o['doc_ref']
              ? '<span class="badge trs-ar-ok">مسلم</span> <small class="text-muted">' . htmlspecialchars($o['doc_ref'], ENT_QUOTES, 'UTF-8') . '</small>'
        ...
        <td>
          ...
          <form method="post" class="ta-arrival-form" data-ems-outbox="1"
                data-ems-outbox-label="تأكيد وصول">
        <?= csrf_field() ?>
            <input type="hidden" name="deliver_id" value="<?= $rid ?>">
            <label class="visually-hidden" for="dlv_wit_<?= $rid ?>">شاهد التسليم</label>
            <input aria-label="شاهد التسليم" id="dlv_wit_<?= $rid ?>" type="text" name="witness_name" required
                   class="form-control form-control-sm trs-ar-w150" placeholder="شاهد التسليم *">
            <label class="visually-hidden" for="dlv_note_<?= $rid ?>">ملاحظة التسليم</label>
            <input aria-label="ملاحظة التسليم" id="dlv_note_<?= $rid ?>" type="text" name="delivery_note"
                   class="form-control form-control-sm trs-ar-w180" placeholder="ملاحظة التسليم">
            <button class="action-btn" type="submit">تأكيد التسليم</button>
          </form>
          <?php else: ?>
