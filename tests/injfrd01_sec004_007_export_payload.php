...
/* ⛔ **الدورُ 3 (الأسطول) لم يعُد له سندٌ في رؤيةِ السائقين**: بعدَ تحوّلِ
     PERM-01 §6 قِيس أنَّ له **صفرَ شاشةٍ** ذاتِ صلةٍ بالسائقين — لا في ورقةِ
     الدليلِ ولا في `nav_items`. وكان يراهم بمنحٍ عَرَضيٍّ من بذرٍ متعدِّدِ
     المصادرِ أُزيل. فاختيارُه هنا كان يقيس **أثرَ العطبِ لا الضابط**.
   ◆ **والبديلُ بسندٍ مقيس**: الدورُ 1 (التشغيل) يملك `Employees/employees.php`
     في هدفِه — فيُصدِّر فعلًا، وليس صاحبَ حقِّ الهاتفِ بنصِّ `SEN-002` (وهي
     الموارد البشريّة). فالمقيسُ يبقى هو هو: **مُصدِّرٌ غيرُ مخوَّلٍ للحقلِ
     الحسّاسِ يستلم قيمةً محجوبة**. */
$UNAUTH = 'محمد';      /* الدور 1 — التشغيل: يصدّر السائقين ولا يملك هاتفَ الموظف */
...
if ($loggedIn) {
    ...
    /* ◆ **ويُفرَّق بين منعَين لا يُخلطان**:
       ⛔ منعُ **صلاحيّةٍ** (لا `can_view` على شاشةِ الكيان) ⇒ **رسوبٌ حقيقيّ** —
          فالشاهدُ يقيس أنَّ التصديرَ يخرج ثمَّ يُقرأ محجوبًا، لا أنَّ الوصولَ سُدَّ.
       ◆ ومنعُ **عزلِ الإدارات** (`SCOPE-403`) ضابطٌ آخرُ قائمٌ بذاتِه لا شأنَ
          له بحجبِ الحقلِ الحسّاس. فالبندُ حينئذٍ **غيرُ مقيسٍ ويُسمّى** — بنمطِ
          بندِ الواجهةِ الحيّةِ أدناه — ولا يُقرأ نجاحًا ولا رسوبًا.
       والتمييزُ بنصِّ الردِّ لا بحجمِه: `SCOPE-403` تُعلن رمزَها. */
    $isScope = ($code === 403 && mb_strpos((string) $body, 'SCOPE-403') !== false);
    if ($served || !$isScope) {
        chk($served, '**التصديرُ خرج فعلًا** — فصفرُ تسريبٍ بسببِ منعِ الوصولِ ليس نجاحًا',
            "HTTP={$code} · بايت={$bytes} · نوع=" . mb_substr($type, 0, 40));
    } else {
        note('تصديرُ الدورِ غيرِ المخوَّلِ ردَّه **عزلُ الإدارات** لا منعُ صلاحيّة — '
           . 'فبندُ «الملفُّ يخرج» **غيرُ مقيسٍ** لهذا الدور، والمقيسُ حجبُ الحقلِ '
           . 'في الحمولةِ أدناه · HTTP=' . $code);
    }
