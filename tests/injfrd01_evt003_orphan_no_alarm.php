...
/* ⚠ **الشاهدُ كان يحمرُّ على الصحّةِ لا على العطب** (‏مقيسٌ 2026-08-27):
     كان يشترط إنذارًا في آخرِ 24 ساعة. والإنذارُ لا يُكتب إلّا حين **يتعثّر**
     شيءٌ فعلًا — فإن سلِمَ النظامُ يومًا كاملًا انقلب الشاهدُ أحمر، ويُقرأ
     «الكاشفُ ميّت» وهو حيٌّ ولا شيءَ يستحقُّ الإنذار.
     المقيسُ يومَها: 293 إنذارًا · أحدثُها 2026-08-26 00:06 · و`statement_build`
     نجح 01:00 فتصفَّر عدّادُ تعثّرِه (‏مهلتُه 108000ث) — فصمتَ الكاشفُ بحقّ.
   ◆ **والقصدُ ليس «وقع عطبٌ حديثًا» بل «الماسحُ يعمل ويكتب حين يجب»** —
     فيُقاس **نبضُ الماسحِ نفسِه** من `ems_job_schedule.last_success_at`،
     وهو أثرٌ يكتبه العاملُ الخلفيُّ في كلِّ دورةٍ ولا يتوقّف على وجودِ عطب.
   ⛔ **ولم يُضعَف الشرط**: `$alerts > 0` باقٍ — فالكاشفُ يجب أن يكون أنذر
     فعلًا مرّةً على الأقل. والحداثةُ تُقبَل من **أحدِ** دليلَين: إنذارٌ حديثٌ
     أو دورةُ ماسحٍ حديثة. فإن مات الماسحُ ولم يُنذر أحدٌ ⇒ **أحمر** كما يجب. */
$scanRun = n($db, "SELECT COUNT(*) FROM `ems_job_schedule`
                    WHERE `is_active` = 1 AND `last_success_at` > NOW() - INTERVAL 24 HOUR");
chk($alerts > 0 && ($recent > 0 || $scanRun > 0),
    'الكاشفُ **موصولٌ بالمجدولِ ويُنذر فعلًا** — أثرٌ مكتوبٌ لا ادّعاء',
    "إنذاراتٌ إجمالًا={$alerts} · في آخرِ 24 ساعة={$recent} · دوراتُ ماسحٍ في آخرِ 24 ساعة={$scanRun}");
