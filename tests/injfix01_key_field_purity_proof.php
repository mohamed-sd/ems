...
/* ◆ **الرسالةُ كانت تطبع 34 حرفًا لا مقيسًا** — `('صفوفٌ معتمدة=' . ($q ? '34' : '?'))`
 *   فلا يمكن أن تخالف نفسَها مهما تغيّر الواقع. والعددُ المجمَّدُ نفسُه خلط
 *   بين أمرَين: «القيمةُ العربيةُ سليمةٌ لم يُتلفها المنظِّف» — وهو المقصود —
 *   و«لا صفَّ يغيّر حالتَه أبدًا» — وهو ليس مقصودًا. فلمّا أُلغيت سياسةٌ وهميةٌ
 *   بحكمٍ (FR-SEC-005) انقلب الشاهدُ أحمرَ على **تغييرٍ مشروع**.
 * ⇒ يُقاس المقصودُ نفسُه: **كلُّ حالةٍ مقروءةٌ من المفرداتِ المشروعة**، ولا
 *   قيمةَ مشوَّهةً، **والعددُ يُطبَع كما قِيس**. */
$total = (int) $conn->query("SELECT COUNT(*) FROM `scr_sensitive_fields`")->fetch_row()[0];
$legal = (int) $conn->query("SELECT COUNT(*) FROM `scr_sensitive_fields`
                              WHERE `status` IN ('مسودة','معتمد','ملغاة')")->fetch_row()[0];
$appr  = (int) $conn->query("SELECT COUNT(*) FROM `scr_sensitive_fields`
                              WHERE `status` = 'معتمد'")->fetch_row()[0];
ok($legal === $total && $appr > 0,
   'قيمةٌ معجميةٌ عربيةٌ مفردةٌ («معتمد») باقيةٌ سليمةً — ولا حالةَ مشوَّهة', $pass, $fail,
   "إجمالي={$total} · بمفرداتٍ مشروعة={$legal} · معتمد={$appr}");
