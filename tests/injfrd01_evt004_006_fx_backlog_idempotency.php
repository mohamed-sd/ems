...
/* ◆ **ولا يُعلَن صفرُ أثرٍ بمقامٍ صفريّ** — إلا أن يكون المتأخرُ نفسُه صفرًا:
   فمستهلكٌ لاحقٌ برأسِ ناقلِه (FINAL_CLOSE ⑨ — `FxRealizationConsumer` وُصل
   ولَحق) **لا متأخرَ له أصلًا فلا تعرُّضَ يُقاس** — وهذا أقوى من تقريرِ أثرٍ
   على متراكم، لا مقامًا صفريًّا مموَّهًا. */
chk($lag === 0 || $nonZero > 0,
    $lag === 0 ? '**المتأخرُ صفرٌ — المستهلكُ لاحقٌ برأسِ ناقلِه فلا تعرُّض**'
               : '**المقامُ غيرُ صفريّ** — في المتأخرِ وقائعُ بمبالغَ حقيقية',
    $lag === 0 ? "متأخرٌ=0 · المؤشرُ عند {$cur}"
