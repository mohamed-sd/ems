...
/* ── القياسُ الحاسمُ على مسارِ التايم شيت ───────────────────────────────────
     ◆ ولماذا لا يُقاس على شاشةِ الوصول: القياسُ كشف أنَّ **لا حسابَ في النظامِ
       كلِّه يستطيع تأكيدَ وصول** — دورُ النقلِ (23) هو الوحيدُ الذي يراها،
       وصلاحيةُ تحريرِه صفر. فالردُّ `GOV-PERM-403-WRITE` لا 409، وكتابةٌ صفريةٌ
       تُقرأ خطأً «لم يُكتب مرتين» وهي لم تكتب أصلًا. سُجِّلت الثغرةُ ولم
       تُداوَ هنا — منحُ صلاحيةِ كتابةٍ قرارُ حوكمةٍ لا أثرُ إصلاحِ واجهة. */
$u6 = $userOfRole(6);
...
if ($oid > 0 && $u23 !== '') {
    ...
    $made = $after - $before;
    /* ◆ الكتابةُ هنا **محجوبةٌ بالحوكمةِ لا بالعُدَّة**: الردُّ `GOV-PERM-403-WRITE`
         لأنَّ دورَ النقلِ الوحيدَ الذي يرى الشاشةَ بلا صلاحيةِ تحرير. فلا يُقاس
         عليها شقُّ «مرةً واحدةً» — قيس على التايم شيت أعلاه — ويُقاس هنا بلوغُ
         الوسمِ وإعلانُ الحجبِ صراحةً بدلَ صمت. */
    $ok($made === 0 && mb_strpos($r1, 'GOV-PERM-403-WRITE') !== false,
        '**وحجبُ الكتابةِ في شاشةِ الوصولِ يُعلَن برمزِه** `GOV-PERM-403-WRITE`',
        "كُتب {$made} · ردٌّ: " . mb_substr(trim(strip_tags($r1)), 0, 100));
    $ok($made <= 1, "ولم تُكتب نسخةٌ ثانيةٌ بأيِّ حال ({$made})");
    $ok(strpos($page, 'data-ems-outbox-label') !== false,
        'ووسمُ الطابورِ على نموذجِ «وصلت» يبلغ المتصفحَ بعنوانِه');

    /* ── كنسُ ما زُرع: المستندُ والحالةُ وأختامُ العطالة ────────────────────── */
    ...
    $say("   كُنس ختامًا: {$swept} مستندًا · وأُعيدت الحالةُ إلى «{$stageBefore}»");
    $leftDoc = 0;
    $rr = $conn->query("SELECT COUNT(*) FROM transfer_delivery_docs WHERE doc_note = 'OUTBOXPROBE'");
    if ($rr && ($x = $rr->fetch_row())) { $leftDoc = (int) $x[0]; }
    $ok($leftDoc === 0, "صفرُ ثغرةٍ من عائلةِ الوسمِ بعد الجولة ({$leftDoc})");
}
@unlink($jar);
