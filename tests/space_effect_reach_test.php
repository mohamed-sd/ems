...
if (!$map) {
    /* ◆ **مساحةٌ لا تُنتج واقعةً واحدةً ليست حالةَ «لم يُقَس» بل كشفًا**: العزلُ
         لا يمكن أن يكسر أثرًا لا وجودَ له — فالشرطُ مُستوفًى بانعدامِ السطح.
       ◆ **لكنَّه دَينٌ يُرفع**: إدارةٌ تعمل ولا تُنتج حدثًا واحدًا يعني أن عملَها
         **لا يصل أحدًا بالحدث** — وأيُّ مستهلِكٍ يحتاج أثرَها يقرؤه من جدولٍ
         مباشرةً أو لا يقرؤه أصلًا. ويُعلَن هنا ولا يُخفى تحتَ «اجتاز». */
    echo "  ◆ **لا واقعةَ واحدةً تُنتجها هذه المساحةُ** — فالعزلُ لم يكسرْ أثرًا\n";
    echo "    (لا أثرَ يُكسَر)، **والدَّينُ المرفوع**: إدارةٌ عاملةٌ بلا حدثٍ منشورٍ\n";
    echo "    لا يصل عملُها مستهلِكيها بالحدثِ — وهذه فجوةٌ في نموذجِ الأحداثِ لا في العزل.\n";
    echo "\n✔ الحكم: PASS بانعدامِ السطح — **مع دَينٍ مُعلَن**\n";
    exit(0);
}
...
if ($target === null) {
    /* ◆ وقائعُ لها مستهلِكون **مُعلَنون** ولا أثرَ مقيسٌ لواحدٍ منهم: الحالُ
         **كما كان قبلَ العزلِ حرفًا** — فلا يُنسب إليه قطعٌ لم يُحدثه.
       ◆ **ولا يُدَّعى إثباتُ وصولٍ لم يقع**: يُعلَن أن الشرطَ مُستوفًى بانعدامِ
         أثرٍ يُكسَر، ويبقى الدَّينُ (مستهلِكون مُعلَنون لا يصلهم شيء) مرفوعًا. */
    echo "  ◆ لهذه المساحةِ " . count($map) . " واقعةً، **ولا واحدةَ لها أثرٌ مقيسٌ واصل**\n";
    echo "    فلا أثرَ يُكسَر — والحالُ كما كان قبلَ العزلِ حرفًا، فلا يُنسب إليه.\n";
    echo "    **والدَّينُ المرفوع**: مستهلِكون مُعلَنون ({$declared} نشِطًا · {$inactive} معطَّلًا) لا يصلهم أثرٌ مقيس.\n";
    echo "\n✔ الحكم: PASS بانعدامِ أثرٍ يُكسَر — **مع دَينٍ مُعلَن**\n";
    exit(0);
}
