...
/* ══ **البرهانُ بالفرقِ لا بغيابٍ محظوظ** ═════════════════════════════════════
   كان الحكمُ: «الإعاشةُ والمعسكر» غائبةٌ عن الصفحة. والمقيسُ أن العقد 2 له
   **البنودُ التسعةُ كلُّها مُجازةٌ** (`approval_state = 'approved'`) —
   و`catering_camp` منها. فالصفحةُ تعرضه **بحقٍّ**، والفحصُ كان يستند إلى غيابٍ
   عارضٍ في بياناتٍ قديمةٍ لا إلى خاصيةٍ في المنتج.
   ⇒ تُبرهَن الخاصيةُ نفسُها — «القائمةُ من مصفوفةِ العقد» — **بالفرق**: يُسحب
     إجازةُ بندٍ واحدٍ **رجعةً**، فيجب أن يغيب عن الصفحة؛ ثم يُعاد فيعود. فلو
     كانت القائمةُ من التسعةِ العامّةِ لظلَّ حاضرًا ورسب الفحص. وهذا حكمٌ يرسب
     عند إفسادِ مفحوصِه — والغيابُ المحظوظُ لا يرسب أبدًا.                      */
/* ══ **الحكمُ على قائمةِ صفِّ الفاحصِ لا على الصفحةِ كلِّها** ══════════════════
   كان الحكمُ: «الإعاشةُ والمعسكر» **غائبةٌ عن الصفحة**. والمقيسُ أن يومَ البرهانِ
   (2026-06-11) فيه **167 واقعةَ إنتاجٍ** كلُّها من العقد 2، **وللعقد 2 البنودُ
   التسعةُ كلُّها مُجازة** — فالشاشةُ تعرض «الإعاشة» في قوائمِ تلك الصفوفِ بحقٍّ،
   ولا سبيلَ لغيابها عن الصفحةِ ما دامت هناك. فالفحصُ كان يقيس **جمهورًا أوسعَ
   من دعواه** ويُدين شاشةً تفعل الصواب.
   ⇒ يُقتطَع **قائمةُ صفِّ الفاحصِ بعينها** (`<select name="oblig[<lid>]">`
     و`$lid` سطرُ زمنِه الذي بذره) ويُحكَم عليها: فيها بندا مصفوفةِ عقدِه، وليس
     فيها ما لم يُجَز له. فلو بُنيت القائمةُ من التسعةِ العامّةِ لسقط الفحصُ —
     وذاك هو المعنى المقصودُ من البداية.                                        */
$oblBlock = '';
if (preg_match('~<select name="oblig\[' . (int) $LF . '\]".*?</select>~s', $b, $om)) {
    $oblBlock = $om[0];
}
check($oblBlock !== '', 'وقائمةُ بنودِ صفِّ الفاحصِ مُصيَّرةٌ (oblig[' . (int) $LF . '])');
if ($oblBlock !== '') {
    check(mb_strpos($oblBlock, 'الوقودُ والزيوت') !== false
          && mb_strpos($oblBlock, 'جاهزيةُ المعدة والصيانة') !== false,
        'وفيها بندا مصفوفةِ عقدِه (الوقود · جاهزية المعدة)');
    check(mb_strpos($oblBlock, 'الإعاشةُ والمعسكر') === false,
        '**وليس فيها ما لم يُجَز لعقدِه** — القائمةُ من مصفوفةِ العقد لا من التسعة كلِّها');
}
