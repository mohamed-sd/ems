...
/* ── ④ شارةُ الشريطِ = موافقاتي + مهامي **لهذا المستخدم** (0407) ──────────
   ◆ **تحديثُ 2026-08-17**: «مساحة عملي» رُفعت من شريطِ الأفعالِ بقرارِ المالك،
     ورُفع معها حسابُ عدّادِها. فالبنودُ الثلاثةُ التي كانت تفحص **كيف** يحسب
     الشريطُ الشارةَ لم تعد لها مادّةٌ تُفحَص — والصوابُ إعادةُ صياغتِها لا
     حذفُها: حكمُ 0407 «لا رقمَ شركةٍ يراه الجميعُ في الشريط» صار مُستوفًى
     **بغيابِ الرقمِ أصلًا**، وهذا ما يُثبَّت هنا كي لا يعود بابُ التسريبِ
     مفتوحًا إن أُعيدت الشارةُ يومًا بلا مفتاحِ مستخدم.
   ◆ ومحرّكُ العدِّ نفسُه (`ems_workspace_badge`) **ما يزال مفحوصًا رقميًّا**
     أدناه، لأنّ بلاطاتِ `main/my_workspace.php` ما تزال تستعمله. */
$say("\n── ④ الشارةُ شخصيةٌ لا شركة");
$tb = (string) @file_get_contents($ROOT . '/includes/topbar.php');
$ok(strpos($tb, 'ems_workspace_badge') === false && strpos($tb, "'ems_ws_badge'") === false,
    'شريطُ الأفعالِ بلا شارةِ مساحةِ عملٍ — فلا رقمَ يُحسب ولا يُخبَّأ ولا يُسرَّب');
$ok(strpos($tb, 'ApprovalsInboxService::inbox($GLOBALS[\'conn\'], intval($_SESSION[\'user\'][\'company_id\']') === false,
    'ولا تُنادى بوسيطِ الكيانِ وحدَه — الحارسُ يبقى إن عادت الشارةُ يومًا');
/* ◆ ورفعُ الأيقونةِ ليس إغلاقًا للباب — والفرقُ يُقاس على القاعدةِ لا على
     الملفّ: كم صفَّ تنقّلٍ نشطًا يفتح الشاشة؟ لو صفر، فقد صار الرفعُ حجبًا. */
$__wsNavRow = @$conn->query(
    "SELECT COUNT(*) c FROM nav_items WHERE route LIKE '%my_workspace.php%' AND active = 1");
$__wsNav = ($__wsNavRow && ($__x = $__wsNavRow->fetch_row())) ? (int) $__x[0] : 0;
$ok($__wsNav > 0,
    'وبابُ الشاشةِ لم يُغلق: ' . $__wsNav . ' صفَّ تنقّلٍ نشطًا يفتح مساحةَ عملي');
