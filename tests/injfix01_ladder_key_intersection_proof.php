...
/* ══════════ ⑥ الوصلُ الفعليّ — «تجد سلّمَها» ليست «تقرأ سلّمَها» ═══════════
 * ◆ **العيبُ الذي كشفه تدقيقُ اللقطة**: البنودُ ①-④ تُثبت أن **الاستدلالَ**
 *   يعمل — أي أن كلَّ رحلةٍ تجد سلّمًا نشطًا في السجل. ولا تُثبت أن **الشاشةَ
 *   القائدةَ للخطوةِ تقرأ ذلك السلّمَ عند التنفيذ**. والفرقُ جوهري: مع الأول
 *   يبقى ترتيبُ الخطواتِ والسقفُ و«لا يدَ تمشي خطوتَين» **غيرَ منفَّذ**.
 * ◆ ⇒ فالقياسُ هنا على العمودِ الذي يحمل الحكمَ: `ladder_wired`.
 */
echo "\n── ⑥ الوصلُ الفعليُّ للرحلاتِ بالسلاليم ──\n";
$r = $conn->query("SELECT COUNT(*) FROM `gov_journey_ladders`");
$jrTotal = $r ? (int) $r->fetch_row()[0] : 0;
$r = $conn->query("SELECT COUNT(*) FROM `gov_journey_ladders` WHERE `ladder_wired` = 1");
$jrWired = $r ? (int) $r->fetch_row()[0] : 0;
printf("  الرحلاتُ الموصولةُ فعلًا: **%d من %d**\n", $jrWired, $jrTotal);
if ($jrWired < $jrTotal) {
    echo "  ◆ الشاشةُ تقودُ الخطوةَ ولا تقرأ سلّمَها — فالسلّمُ **مُستدَلٌّ لا مُنفَّذ**.\n";
    echo "  ◆ **ولا يُدَّعى إغلاقُ GAP-01 بالاستدلالِ وحدَه** — وهذا ما كان يُخفيه\n";
    echo "     مقياسُ الإغلاقِ القديمُ حين اكتفى بذكرِ الرمزِ في فاحصٍ أخضر.\n";
}

require_once $ROOT . '/tools/lib/gap_verdict.php';
gapv('GAP-01', $jrTotal > 0 && $jrWired === $jrTotal,
     "أربعَ عشرةَ رحلةً تجد سلّمَها **وتقرؤه عند التنفيذ** — الموصولُ فعلًا {$jrWired}/{$jrTotal}",
     $fail);
