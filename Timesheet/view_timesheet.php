...
<head>
  ...
  <style>
    ...
    .bg-total {
      background: linear-gradient(45deg, #000022, #000022);
    }
    ...
    <div class="row g-3 mb-4 text-center">
      <div class="col-md-3 col-6">
        <div class="stat-card bg-approved shadow-sm">
          <h5>المعتمدة</h5>
          <h3><?= !empty($stats['approved']) ? $stats['approved'] : 0 ?></h3>
        </div>
      </div>
      <div class="col-md-3 col-6">
        <div class="stat-card bg-pending shadow-sm">
          <h5>قيد المراجعة</h5>
          <h3><?= !empty($stats['pending']) ? $stats['pending'] : 0 ?></h3>
        </div>
      </div>
      <div class="col-md-3 col-6">
        <div class="stat-card bg-rejected shadow-sm">
          <h5>المرفوضة</h5>
          <h3><?= !empty($stats['rejected']) ? $stats['rejected'] : 0 ?></h3>
        </div>
      </div>
      <div class="col-md-3 col-6">
        <div class="stat-card bg-hours shadow-sm">
          <h5>إجمالي الساعات المنفذة</h5>
          <h3><?= !empty($stats['total_hours']) ? $stats['total_hours'] : 0 ?></h3>
        </div>
      </div>
    </div>
        ...
        <table id="projectsTable" class="display nowrap table table-striped table-bordered" style="width:100%">
          ...
          <tbody>
            ...
            while ($row = mysqli_fetch_assoc($result)) {
              switch ($row['status']) {
                case "1":
                  $status = "<span class='badge bg-warning text-dark'>تحت المراجعة</span>";
                  break;
                case "2":
                  $status = "<span class='badge bg-success'>تم الاعتماد</span>";
                  break;
                case "3":
                  $status = "<span class='badge bg-danger'>تم الرفض</span>";
                  break;
                default:
                  $status = "<span class='badge bg-secondary'>غير معروف</span>";
              }
