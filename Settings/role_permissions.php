...
/* ═══ للقراءةِ التاريخيّة — والتحريرُ يقع في قالبِ الدور (PERM-01-DEC ق-٥)
   ⛔ **الجدولُ لم يعد يحكم أحدًا**: حُذف الفرعُ القديمُ من دالّةِ القرار في م-5،
     والخمسةُ والسبعون كلُّهم يُحكَمون بقوالبِهم. وحكمُ المالكِ نصًّا: «الجدولُ
     لا يسقط، **يبقى مقروءًا أثرًا لا حكمًا**».
   ⛔ **وبقاءُ التحريرِ هنا كذبٌ على من يستعمله**: يفتح ويعدّل ويحفظ فيرى
     «تمَّ الحفظ» — **ولا يتغيّر شيءٌ لأيِّ مستخدم**. وهي لا-عمليّةٌ صامتةٌ
     أسوأُ من منعٍ صريح، لأنَّ صاحبَها يظنُّ أنّه منح أو سحب.
   ◆ **والإقفالُ إنفاذُ حكمٍ قائمٍ لا حكمٌ جديد** — وهو ما فُعل بالمصفوفةِ قبلُ.
   ◆ **والمنعُ يسبق كلَّ معالجٍ** فلا يبقى بابٌ يكتب: معالجاتُ الحفظِ والحذفِ
     والمنحِ الشاملِ والسحبِ الشاملِ كلُّها بعدَ هذا السطر. */
$RP_READONLY = true;
if ($RP_READONLY && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ems_gov_flash_redirect(
        'role_permissions.php',
        'هذه الشاشة للقراءة التاريخية. الجدول لم يعد يحكم احدا بعد التحول، '
        . 'والتحرير يقع في قالب الدور والاسناد من سجل المنح.',
        'GOV-INFO-200', ''
    );
    exit();
}
