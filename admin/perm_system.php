...
$phases = array(
    array('①', 'القديم وحدَه يقرر',        true,                          'العَلَم legacy — وهو حالُنا'),
    array('②', 'ملءُ القوالب من الواقع',   $tplItems > 0,                 $tplItems . ' بندًا في ' . $tplPub . ' نسخةً منشورة'),
    array('③', 'تشغيلُ الظل والمقارنة',    $diffs > 0 || $usersPos > 0,   'يحتاج جسرَ وصولٍ للمستخدمين'),
    array('④', 'صفرُ فرقٍ أربعةَ عشرَ يومًا', ($diffs === 0 && $usersPos > 0), 'لم يبدأ العدّاد بعد'),
    array('⑤', 'قلبُ العَلَم إلى derived',  $ps_derived,                   'بيد المالك في .env'),
    array('⑥', 'القديمُ قراءةً ثم تقاعد',   false,                         'بعد القلب واستقراره'),
);
...
<div class="card">
    <div class="card-h">
        <i class="fas fa-shield-halved"></i> المصدرُ الحاكم الآن:
    ...
    <div class="card-b" style="font-size:.88rem;line-height:1.9">
        <p>ما دام العَلَم <code>EMS_PERM_SOURCE=legacy</code> فالنظامُ الجديدُ <strong>مرآةٌ لا تقرر شيئًا</strong> —
           والذي يمنع ويسمح هو <code>role_permissions</code> وحدَه. القلبُ قرارُ مالكٍ في <code>.env</code>،
           ولا يُتخذ قبل أن يثبت الظلُّ <strong>صفرَ فرقٍ أربعةَ عشرَ يومًا</strong>.</p>
    </div>
</div>

<div class="stat-grid" style="margin-top:16px">
    <div class="stat-card hex-stat-card hex-stat-blue"><div class="stat-row">
        <div><div class="stat-val"><?php echo number_format($liveRows); ?></div><div class="stat-lbl">القديم: منحُ الأدوار (<?php echo $liveRoles; ?> دورًا)</div></div>
    ...
    <div class="stat-card hex-stat-card <?php echo $tplItems > 0 ? 'hex-stat-green' : 'hex-stat-red'; ?>"><div class="stat-row">
        <div><div class="stat-val"><?php echo number_format($tplItems); ?></div><div class="stat-lbl">الجديد: بنودُ القوالب</div></div>
        <div class="stat-ico"><i class="fas fa-layer-group"></i></div>
    </div></div>
    <div class="stat-card hex-stat-card hex-stat-blue"><div class="stat-row">
        <div><div class="stat-val"><?php echo number_format($tplPub); ?></div><div class="stat-lbl">نسخٌ منشورة (من <?php echo $tpl; ?> قالبًا)</div></div>
    ...
    <div class="stat-card hex-stat-card <?php echo $diffs === 0 ? 'hex-stat-green' : 'hex-stat-orange'; ?>"><div class="stat-row">
        <div><div class="stat-val"><?php echo number_format($diffs); ?></div><div class="stat-lbl">فروقُ الظل المسجَّلة</div></div>
        <div class="stat-ico"><i class="fas fa-code-compare"></i></div>
    </div></div>
    <div class="stat-card hex-stat-card hex-stat-blue"><div class="stat-row">
        <div><div class="stat-val"><?php echo number_format($effective); ?></div><div class="stat-lbl">صلاحياتٌ فعّالةٌ محسوبة</div></div>
        <div class="stat-ico"><i class="fas fa-calculator"></i></div>
    </div></div>
</div>

<div class="card" style="margin-top:16px">
    <div class="card-h"><i class="fas fa-road"></i> المراحلُ الست — أين نقف</div>
...
<div class="card" style="margin-top:16px">
    <div class="card-h" style="color:<?php echo $usersPos > 0 ? 'var(--green,#27ae60)' : 'var(--red,#c0392b)'; ?>">
        <i class="fas fa-bridge"></i> جسرُ الوصول — العائقُ الحاكم
    </div>
    <div class="card-b" style="font-size:.88rem;line-height:1.9">
        <p>النظامُ الجديد يصل إلى القالب هكذا:
           <strong>مستخدم ← شخص ← موقعٌ وظيفي ← مسمًّى ← قالب</strong>. وهذه حلقاتُه اليوم:</p>
        <table style="width:100%;border-collapse:collapse;margin-top:8px">
            <tbody>
            <?php
            $bridge = array(
                array('مستخدمون نشطون لهم موقعٌ وظيفي', $usersPos, $usersActive),
                array('أشخاصٌ مسجَّلون', $persons, $usersActive),
                array('مواقعُ وظيفيةٌ مسجَّلة', $positions, $usersActive),
                array('مسمَّياتٌ لها قالب', $titlesTpl, $titlesAll),
            );
            foreach ($bridge as $b):
                $ok = ($b[2] > 0 && $b[1] >= $b[2]);
            ?>
        ...
        <?php if ($usersPos === 0): ?>
        <p style="margin-top:10px;color:#c0392b"><strong>الجسرُ مقطوع:</strong>
           لا مستخدمَ واحدٌ يصله النظامُ الجديد — فالقوالبُ الممتلئةُ لا يقرؤها أحد،
           ولا يمكن تشغيلُ الظلِّ ولا بدءُ عدّاد الأربعة عشر يومًا حتى يُبنى.</p>
        <?php endif; ?>
    </div>
</div>
...
<div class="card" style="margin-top:16px">
    ...
    <div class="card-b" style="overflow-x:auto">
        <table style="width:100%;border-collapse:collapse;font-size:.88rem">
            <thead><tr style="text-align:right;border-bottom:2px solid #ddd">
                <th style="padding:8px">الطبقة</th><th style="padding:8px">قوالب</th><th style="padding:8px">بنودٌ منشورة</th>
            </tr></thead>
            <tbody>
            <?php
            $kindLabel = array('relation' => 'نوعُ العلاقة', 'family' => 'العائلةُ الوظيفية',
                'level' => 'المستوى التنظيمي', 'title' => 'المسمّى', 'assignment' => 'التكليف');
        ...
        <?php if ($topTitles): ?>
        <div style="margin-top:12px;font-weight:700">أثقلُ قوالب المسمّى</div>
...
<div class="card" style="margin-top:16px">
    <div class="card-h"><i class="fas fa-code-compare"></i> ميزانُ الظل</div>
    <div class="card-b" style="font-size:.88rem;line-height:1.9">
        <?php if ($diffs === 0): ?>
            <p><strong>لا فرقَ مسجَّلًا</strong> — والعدّادُ لم يبدأ بعدُ لأن الظلَّ لا يعمل
               (يحتاج جسرَ الوصول أعلاه). أولُ تشغيلٍ للظل يبدأ القياس.</p>
        <?php else: ?>
            <p>آخرُ فرقٍ: <strong><?php echo e($lastDiffAt); ?></strong> ·
               سلسلةُ الصفر حتى اليوم: <strong><?php echo (int) $streak; ?></strong> يومًا (المطلوب 14).</p>
...
<div class="card" style="margin-top:16px">
    ...
    <div class="card-b" style="font-size:.86rem;line-height:2">
        <p><strong>المفتاحُ الحاكم:</strong> <code>EMS_PERM_SOURCE</code> في <code>.env</code> —
           <code>legacy</code> (القديم يقرر · حالُنا) أو <code>derived</code> (الجديد يقرر).
           <strong>لا تقلبه قبل صفرِ فرقٍ أربعةَ عشرَ يومًا.</strong></p>
        <p><strong>إعادةُ بناء القوالب من الواقع:</strong>
           <code>php tools/perm_templates_build.php --diff</code> للمعاينة ثم <code>--apply</code>.
           آمنُ الإعادة، ويرفض التطبيق إن اختلف اتحادُ الطبقات عن صلاحيات الدور بندًا واحدًا.</p>
        <p><strong>الصلاحياتُ الفعلية اليوم:</strong> شاشةُ «صلاحيات الأدوار» في لوحة الشركة —
           هي الحاكمةُ ما دام العَلَم <code>legacy</code>.</p>
        <p><strong>تفسيرُ أي منع:</strong> شاشةُ «تفسير مصدر الصلاحية» (الحوكمة) تعرض السلسلة
           خطوةً خطوة: أالشاشةُ مسجَّلة؟ أللدور صلاحية؟ أالرابطُ مربوط؟</p>
        <p><strong>هذه الشاشةُ قرائيةٌ بحتة</strong> — لا تكتب صفًّا ولا تقلب علمًا.</p>
    </div>
</div>
