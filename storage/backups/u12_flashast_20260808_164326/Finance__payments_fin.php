...
if (isset($_GET['execute_id'])) {
    ...
    if (!fin_verify_action_token()) { ems_gov_flash_redirect('payments_fin.php', 'رمز الحماية غير صالح ❌', 'GOV-INFO-200', ''); exit(); } // إصلاح #2
    if (!fin_can_perform($conn, $ctx['role'], 'treasurer')) { ems_gov_flash_redirect('payments_fin.php', 'الصرف/التحصيل يخصّ أمين الخزينة فقط ❌', 'GOV-INFO-200', ''); exit(); } // فصل الواجبات
    $pid = intval($_GET['execute_id']);
    $gate = fin_gate($is_super_admin);
    $pay = $gate->selectOne('fin_payments', array('where' => array('id' => $pid)));
    if (!$pay || $pay['state'] !== 'draft') { ems_gov_flash_redirect('payments_fin.php', 'لا يمكن تنفيذ هذا الدفع ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if ($pay['direction'] === 'disbursement' && $pay['party_type'] === 'supplier') {
        ...
        if ($n > 0) { ems_gov_flash_redirect('payments_fin.php', 'لا صرف: للمورد مستحقات غير مُسوّاة ($n) ❌', 'GOV-INFO-200', ''); exit(); }
    }
