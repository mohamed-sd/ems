...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['holder_name'])) {
    ...
    if ($company_id <= 0)         { ems_gov_flash_redirect('issue_proc.php', 'لا يمكن الحفظ بلا شركة صالحة ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if ($holder_name === '') { ems_gov_flash_redirect('issue_proc.php', 'المستلِم إلزامي ❌', 'GOV-INFO-200', ''); exit(); }
    if ($equipment_id === null && $project_id === null && $maintenance_order_id === null) {
        ems_gov_flash_redirect('issue_proc.php', 'لا صرف بلا بُعد تحميل (معدة/مشروع/أمر) ❌', 'GOV-INFO-200', ''); exit();
    }
