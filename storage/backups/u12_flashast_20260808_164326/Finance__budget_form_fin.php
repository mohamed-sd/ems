...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['dept_module'])) {
    ...
    if ($company_id <= 0) { ems_gov_flash_redirect('budget_form_fin.php', 'لا يمكن الحفظ بلا شركة صالحة ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if (!in_array($dept, $allowed_depts, true)) {
        ems_gov_flash_redirect('budget_form_fin.php', 'لا تُنشئ موازنةً لقسمٍ لا تديره ❌', 'GOV-INFO-200', ''); exit();
    }
    ...
    if (count($lines) < 1) { ems_gov_flash_redirect('budget_form_fin.php', 'الميزانية تحتاج بنداً واحداً فأكثر ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    try {
        ...
        if (strpos($e->getMessage(), 'Duplicate entry') !== false) { ems_gov_flash_redirect('budget_form_fin.php', 'ميزانية هذه الفترة موجودة مسبقاً ❌', 'GOV-INFO-200', ''); exit(); }
        error_log('budget save refused: ' . $e->getMessage());
        ems_gov_flash_redirect('budget_form_fin.php', 'حدث خطأ أثناء الحفظ ❌', 'GOV-INFO-200', ''); exit();
    }
