...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['site_name'])) {
    ...
    if ($project_id <= 0 || $name === '') {
        ems_gov_flash_redirect('sites.php', 'المشروع والاسم إلزاميان ❌', 'GOV-INFO-200', ''); exit();
    }
    ...
    if (!$proj) { ems_gov_flash_redirect('sites.php', 'المشروع غير موجود في نطاقك ❌', 'GOV-REF-404', ''); exit(); }
    ...
    if ($resp > 0) {
        ...
        if (!$emp) { ems_gov_flash_redirect('sites.php', 'المسؤول غير موجود في نطاقك ❌', 'GOV-REF-404', ''); exit(); }
    }
    ...
    try {
        if ($sid > 0) {
            ...
            if (intval($before['is_default']) === 1 && $status === 0) {
                ...
                ems_gov_flash_redirect('sites.php', 'الموقع الافتراضي لا يُعطَّل — هو مرجع الترحيل ❌', 'GOV-INFO-200', ''); exit();
            }
