...
if (!$can_view) {
    ems_gov_flash_redirect('../login.php', 'لا توجد صلاحية عرض المشغلين ❌', 'GOV-PERM-403', '');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['name'])) {
    ...
    if (!empty($employee_code)) {
        ...
        if ($dup) {
            ems_gov_flash_redirect('employees.php', 'كود المشغل موجود مسبقاً ❌', 'GOV-INFO-200', '');
            exit;
        }
    }
...
if (isset($_GET['delete_id'])) {
    ...
    if ($active_contracts > 0 || $active_equipment_assignments > 0) {
        ems_gov_flash_redirect('employees.php', 'لا يمكن حذف المشغل لارتباطه بعقود أو تشغيل نشط ❌', 'GOV-INFO-200', '');
        exit();
    }
