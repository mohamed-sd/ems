...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['name'])) {
    ...
    if ($company_id <= 0)         { ems_gov_flash_redirect('transfer_types_config.php', 'لا يمكن الحفظ بلا شركة صالحة ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if ($name === '' || !array_key_exists($operational_category, $categories) || !array_key_exists($default_bearer, $bearers)) {
        ems_gov_flash_redirect('transfer_types_config.php', 'بيانات غير مكتملة ❌', 'GOV-INFO-200', ''); exit();
    }
    ...
    try {
        ...
        ems_gov_flash_redirect('transfer_types_config.php', 'حدث خطأ أثناء الحفظ ❌', 'GOV-INFO-200', ''); exit();
    }
}
