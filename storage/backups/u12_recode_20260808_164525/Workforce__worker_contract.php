...
if (!$is_super_admin && $company_id <= 0) { ems_gov_flash_redirect('../login.php', 'بيئة شركة غير صالحة ❌', 'GOV-INFO-200', ''); exit(); }
...
if (!$can_view) { ems_gov_flash_redirect('../login.php', 'لا توجد صلاحية عرض العقود ❌', 'GOV-PERM-403', ''); exit(); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_contract') {
    ...
    if (!$is_editing) {
        if ($worker_id <= 0) { ems_gov_flash_redirect('worker_contract.php', 'يجب اختيار عامل ❌', 'GOV-INFO-200', ''); exit(); }
        $ok = false;
        try {
