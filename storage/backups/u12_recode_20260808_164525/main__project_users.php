...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    if (!empty($verifyResult)) {
        ...
        if ($pu_del_ok) {
            ...
            ems_gov_flash_redirect('project_users.php', 'حدث خطأ أثناء الحذف ❌', 'GOV-INFO-200', '');
            exit;
        }
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'edit') {
    ...
    if ($users_has_employee_id && $employee_link_id <= 0) {
        ems_gov_flash_redirect('project_users.php', 'يجب إسناد موظف لهذا الحساب ❌', 'GOV-INFO-200', '');
        exit;
    }
    if ($employee_link_id > 0) {
        ...
        if ($pu_emp_ok === null || $pu_linked !== null) {
            ems_gov_flash_redirect('project_users.php', 'الموظف غير صالح أو مرتبط بحساب آخر ❌', 'GOV-INFO-200', '');
            exit;
        }
    }
    ...
    if ($pu_dup !== null) {
        ems_gov_flash_redirect('project_users.php', 'اسم المستخدم موجود مسبقاً ❌', 'GOV-INFO-200', '');
        exit;
    }
    ...
    if ($pu_upd_ok) {
        ...
        ems_gov_flash_redirect('project_users.php', 'حدث خطأ أثناء التعديل ❌', 'GOV-INFO-200', '');
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['name']) && (!isset($_POST['action']) || $_POST['action'] === 'add')) {
    ...
    if ($pu_ins_ok) {
        ...
        ems_gov_flash_redirect('project_users.php', 'حدث خطأ أثناء الإضافة ❌', 'GOV-INFO-200', '');
        exit;
    }
}
