...
// يعدّله يُعيد توجيهَ الحارس المركزي أو يشطب تسجيلَ شاشةٍ فتصير مفتوحة.
// الحارسُ هنا fail-closed **بالكود** لا بالبيانات — لأن هذه الشاشة قادرةٌ على
// حذف صفِّ الموديول الخاصِّ بها، فحارسٌ يستند إلى صفٍّ يمكن نزعُه من داخلها.
// ويسبق معالجَ POST أدناه لأن insidebar (الحارس المركزي) يأتي بعده.
// ════════════════════════════════════════════════════════════════════════════
require_once __DIR__ . '/../includes/governance_guard.php';
$mod_perms = ems_require_governance_screen($conn, '../main/dashboard.php');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // [ح-01] فعلُ الكتابة يُفحص صراحةً — البوابةُ أعلاه تفحص الدخول لا الفعل.
    ems_require_governance_write($mod_perms,
        !empty($_POST['edit_id']) ? 'edit' : 'add', 'modules.php');
    ...
    if (empty($name) || empty($code)) {
        ...
        // [مُستثنى موثَّق — كتابة RBAC] modules مرجعٌ عام جمَّد عقدُ البوابة كتابته على
        // المدير الأعلى حصرًا. ثغرةُ UAT §15 (وصولُ أيِّ مستخدمٍ مسجَّل) أُغلقت في
        // 2026-08-06 ببوابة ems_require_governance_screen وفحصِ الفعل أعلاه؛ فالكتابةُ
        // الخام هنا صارت خلف أرضيةِ حوكمةٍ صلبة، وتبقى كما هي لأن البوابة لا تكتب
        // في المراجع العامة تعاقديًّا.
...
if (isset($_GET['delete_id'])) {
    // [ح-01] الحذفُ يسبق insidebar فيُفحص هنا صراحةً. ملاحظةٌ بنيوية: حذفُ صفِّ
    // موديولٍ يجعل شاشتَه غيرَ مسجَّلة — وهذا يُغلقها على من سوى أرضية الحوكمة
    // (fail-closed) ولا يفتحها، فالحذفُ يضيّق ولا يوسّع.
    ems_require_governance_write($mod_perms, 'delete', 'modules.php');
    ...
    if ($stmt->execute()) {
        ...
        ems_gov_flash_redirect('modules.php', 'حدث خطأ في الحذف ❌', 'GOV-INFO-200', '');
    }
    exit;
}
