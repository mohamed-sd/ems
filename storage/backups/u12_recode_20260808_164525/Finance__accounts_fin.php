...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['code'])) {
    ...
    if ($company_id <= 0)         { ems_gov_flash_redirect('accounts_fin.php', 'لا يمكن الحفظ بلا شركة صالحة ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if ($code === '' || $name === '' || !isset($account_types[$account_type])) {
        ems_gov_flash_redirect('accounts_fin.php', 'بيانات غير مكتملة (كود/اسم/نوع) ❌', 'GOV-INFO-200', ''); exit();
    }
    ...
    if ($is_editing) {
        ...
        try {
            ...
            if (strpos($e->getMessage(), 'Duplicate entry') !== false) {
                ems_gov_flash_redirect('accounts_fin.php', 'رقم الحساب مكرر في الشركة ❌', 'GOV-INFO-200', ''); exit();
            }
