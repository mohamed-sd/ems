...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'new_plan') {
    ...
    if ($company_id <= 0) { ems_gov_flash_redirect('preventive_plans.php', 'لا يمكن الإنشاء بلا شركة ❌', 'GOV-INFO-200', ''); exit(); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'postpone_plan') {
    ...
    if ($reason === '') { ems_gov_flash_redirect('preventive_plans.php', 'التأجيلُ بسببٍ مكتوبٍ — لا تأجيلَ صامتًا ❌', 'GOV-INFO-200', ''); exit(); }
    $plan = mnt_fetch_plan($conn, $pid, $company_id, $is_super_admin);
    if (!$plan || $plan['next_due_date'] === null) { ems_gov_flash_redirect('preventive_plans.php', 'خطةٌ غير صالحة للتأجيل ❌', 'GOV-INFO-200', ''); exit(); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'generate_order') {
    ...
    if (!$new_id) { // الفريدُ حكمٌ عند التزاحم — mysqli لا يرمي (گوتشا config)
        ems_gov_flash_redirect('preventive_plans.php', 'رفض الفريدُ التوليدَ — الدورةُ مولَّدةٌ سلفًا (M-36) ❌', 'GOV-INFO-200', '');
        exit();
    }
