...
if ($company_id <= 0 && !$is_super_admin) {
    ems_gov_flash_redirect('../main/dashboard.php', 'لا توجد بيئة شركة صالحة ❌', 'GOV-INFO-200', '');
    exit();
}
...
if ($ticket_id > 0) {
    ...
    if (!$ticket || !tkt_can_view_ticket($ticket, $ctx)) {
        ems_gov_flash_redirect('tickets_list.php', 'التذكرة غير موجودة أو خارج نطاقك ❌', 'GOV-REF-404', '');
        exit();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $ticket_id === 0 && isset($_POST['complaint'])) {
    if ($company_id <= 0) { ems_gov_flash_redirect('ticket_form.php', 'لا يمكن الإبلاغ بلا شركة صالحة ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if ($type_id <= 0 || $complaint === '' || $reporting_person === '') {
        ems_gov_flash_redirect('ticket_form.php', 'النوع والوصف واسم المُبلِّغ إلزامية ❌', 'GOV-INFO-200', ''); exit();
    }
    ...
    if (!$type) { ems_gov_flash_redirect('ticket_form.php', 'نوع البلاغ غير صالح ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    try {
        ...
        ems_gov_flash_redirect('ticket_form.php', 'حدث خطأ أثناء تسجيل البلاغ ❌', 'GOV-INFO-200', ''); exit();
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $ticket && ($_POST['action'] ?? '') === 'transition') {
    ...
    if (!tkt_transition_allows($tr, $ticket['stage'])) { header("Location: $self_url&msg=المرحلة+الحالية+لا+تسمح+بهذا+الانتقال+❌"); exit(); }
    ...
    if ($do === 'close') {
        ...
        if ($open_ws > 0) {
            header("Location: $self_url&msg=" . urlencode('لا يمكن الإغلاق: ' . $open_ws . ' مسارًا إلزاميًّا ما زال مفتوحًا ❌')); exit();
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $ticket && ($_POST['action'] ?? '') === 'branch') {
    ...
    try {
        ...
        header("Location: $self_url&msg=تعذر+إصدار+رقم+الفرع+❌"); exit();
    }
    try {
        tkt_gate(false)->runInTransaction(function ($g) use (
    ...
    <div class="card"><div class="card-body">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:10px;align-items:flex-start;">
            <?php foreach ($avail as $key => $tr): ?>
                <form method="post" action="<?php echo htmlspecialchars($self_url); ?>" style="display:inline-block;">
        ...
        <?php else: ?>
            <form method="post" action="<?php echo htmlspecialchars($self_url); ?>" style="display:flex;gap:8px;align-items:center;"
