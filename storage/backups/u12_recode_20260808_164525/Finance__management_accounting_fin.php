...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['center_code'])) {
    ...
    if ($code === '' || $name === '') { ems_gov_flash_redirect('management_accounting_fin.php', 'بيانات المركز غير مكتملة ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    try {
        ...
        if (strpos($e->getMessage(), 'Duplicate entry') !== false) { ems_gov_flash_redirect('management_accounting_fin.php', 'كود المركز مكرر ❌', 'GOV-INFO-200', ''); exit(); }
        error_log('fin_cost_centers insert refused: ' . $e->getMessage());
        ems_gov_flash_redirect('management_accounting_fin.php', 'حدث خطأ أثناء الحفظ ❌', 'GOV-INFO-200', ''); exit();
    }
