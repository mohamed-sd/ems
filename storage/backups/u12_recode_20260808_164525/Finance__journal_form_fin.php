...
if (isset($_GET['post_id'])) {
    ...
    if (!fin_verify_action_token()) { ems_gov_flash_redirect('journal_form_fin.php', 'رمز الحماية غير صالح ❌', 'GOV-INFO-200', ''); exit(); } // إصلاح #2
    if (!fin_can_perform($conn, $ctx['role'], 'finance_manager')) { ems_gov_flash_redirect('journal_form_fin.php', 'الترحيل يخصّ المدير المالي فقط ❌', 'GOV-INFO-200', ''); exit(); } // فصل الواجبات
    ...
    if ($n < 2) { ems_gov_flash_redirect('journal_form_fin.php', 'لا ترحيل: القيد يحتاج سطرين فأكثر ❌', 'GOV-INFO-200', ''); exit(); }
    if (round($d, 2) !== round($c, 2)) { ems_gov_flash_redirect('journal_form_fin.php', 'لا ترحيل: القيد غير متوازن (مدين≠دائن) ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if (!fin_period_posting_open($conn, $company_id, $pdate)) {
        ems_gov_flash_redirect('journal_form_fin.php', 'لا ترحيل: الفترة المالية مقفلة لهذا التاريخ ❌', 'GOV-INFO-200', ''); exit();
    }
    ...
    try {
        ...
        ems_gov_flash_redirect('journal_form_fin.php', 'لا يجوز ترحيل قيدٍ مرتبطٍ بحدثٍ منشورٍ على الناقل ❌', 'GOV-INFO-200', ''); exit();
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['posting_date'])) {
    ...
    if ($company_id <= 0) { ems_gov_flash_redirect('journal_form_fin.php', 'لا يمكن الحفظ بلا شركة صالحة ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if ($posting_date === '') { ems_gov_flash_redirect('journal_form_fin.php', 'تاريخ الترحيل مطلوب ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if ($memo === '') { ems_gov_flash_redirect('journal_form_fin.php', 'بيان القيد (السبب) إلزامي للقيد اليدوي ❌', 'GOV-INFO-200', ''); exit(); }
    // M-38: العملة من الدليل المسجَّل حصرًا
    require_once __DIR__ . '/../includes/fx.php';
    $jr_code = ems_fx_code($jr_currency !== '' ? $jr_currency : 'SDG');
    if ($jr_code === null) { ems_gov_flash_redirect('journal_form_fin.php', 'عملة غير مسجَّلة ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if (count($lines) < 2) { ems_gov_flash_redirect('journal_form_fin.php', 'القيد يحتاج سطرين صالحين فأكثر ❌', 'GOV-INFO-200', ''); exit(); }
