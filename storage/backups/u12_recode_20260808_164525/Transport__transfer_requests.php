...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        if ($transfer_type_id <= 0 || !array_key_exists($source_module, $srcs) || $reason === '') {
            ems_gov_flash_redirect('transfer_requests.php', 'بيانات الطلب غير مكتملة (النوع/المصدر/المبرّر) ❌', 'GOV-INFO-200', ''); exit();
        }
        ...
        try {
            ...
            ems_gov_flash_redirect('transfer_requests.php', 'حدث خطأ أثناء الحفظ ❌', 'GOV-INFO-200', ''); exit();
        }
    ...
    if ($action === 'convert') {
        ...
        if (!array_key_exists($direction, $dirs)) { ems_gov_flash_redirect('transfer_requests.php', 'الاتجاه إلزامي لتحويل الطلب أمراً ❌', 'GOV-INFO-200', ''); exit(); }
        ...
        if ($from_loc <= 0 || $to_loc <= 0) { ems_gov_flash_redirect('transfer_requests.php', 'الطلب بلا موقعَي انطلاق/وصول — أكملهما أولاً ❌', 'GOV-INFO-200', ''); exit(); }
