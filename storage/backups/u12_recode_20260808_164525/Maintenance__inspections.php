...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'convert_note_ticket') {
    ...
    if (!$type) { ems_gov_flash_redirect('inspections.php', 'لا نوعَ بلاغٍ فعّالًا — أنشئه أولًا ❌', 'GOV-INFO-200', ''); exit(); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'new_inspection') {
    ...
    if ($company_id <= 0) { ems_gov_flash_redirect('inspections.php', 'لا يمكن الإنشاء بلا شركة ❌', 'GOV-INFO-200', ''); exit(); }
