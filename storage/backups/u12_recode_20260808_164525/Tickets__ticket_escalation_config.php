...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['name'])) {
    ...
    if ($company_id <= 0) { ems_gov_flash_redirect('ticket_escalation_config.php', 'لا يمكن الحفظ بلا شركة ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if ($name === '' || $to_role === '' || $after <= 0 || $level_no < 1 || $level_no > 5) {
        ems_gov_flash_redirect('ticket_escalation_config.php', 'بيانات غير مكتملة (المستوى 1..5) ❌', 'GOV-INFO-200', ''); exit();
    }
    ...
    try {
        ...
        ems_gov_flash_redirect('ticket_escalation_config.php', 'حدث خطأ أثناء الحفظ ❌', 'GOV-INFO-200', ''); exit();
    }
}
