...
if (!$can_view) {
    ems_gov_flash_redirect('../login.php', 'لا توجد صلاحية عرض سجل العامل ❌', 'GOV-PERM-403', '');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_worker') {
        ...
        if (!$is_editing) {
            ...
            if ($employee_id <= 0) { ems_gov_flash_redirect('worker_register.php', 'يجب اختيار موظف ❌', 'GOV-INFO-200', ''); exit(); }
            $reg_dup = null;
            ...
            if ($reg_dup !== null) { ems_gov_flash_redirect('worker_register.php', 'الموظف مصنّفٌ عاملاً مسبقاً ❌', 'GOV-INFO-200', ''); exit(); }
