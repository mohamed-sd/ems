...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['expected_destination'])) {
    ...
    if ($company_id <= 0)         { ems_gov_flash_redirect('receipt_custody_proc.php', 'لا يمكن الحفظ بلا شركة صالحة ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if ($holder_name === '') { ems_gov_flash_redirect('receipt_custody_proc.php', 'اسم المستلِم إلزامي ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if ($expected_destination === 'مخزن' && !$warehouse_id) {
        ems_gov_flash_redirect('receipt_custody_proc.php', 'حدد مخزن الإدخال — الوجهة مخزن ❌', 'GOV-INFO-200', ''); exit();
    }
