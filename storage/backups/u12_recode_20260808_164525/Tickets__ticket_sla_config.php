...
if (!$is_super_admin && $company_id <= 0) {
    ems_gov_flash_redirect('../login.php', 'لا توجد بيئة شركة صالحة للمستخدم ❌', 'GOV-INFO-200', ''); exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['name'])) {
    ...
    if ($company_id <= 0) { ems_gov_flash_redirect('ticket_sla_config.php', 'لا يمكن الحفظ بلا شركة صالحة ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if ($name === '' || $resp <= 0 || $reso <= 0) {
        ems_gov_flash_redirect('ticket_sla_config.php', 'الاسم وساعات الاستجابة والإنجاز إلزامية ❌', 'GOV-INFO-200', ''); exit();
    }
    $data = array(
    ...
    try {
        ...
        ems_gov_flash_redirect('ticket_sla_config.php', 'حدث خطأ أثناء الحفظ ❌', 'GOV-INFO-200', ''); exit();
    }
}
