...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'convert_units') {
    ...
    if (!fin_verify_action_token()) { ems_gov_flash_redirect('unit_records_fin.php', 'رمز الحماية غير صالح ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if (!$ids) { ems_gov_flash_redirect('unit_records_fin.php', 'لم تحدّد أي يوم للاعتماد ❌', 'GOV-INFO-200', ''); exit(); }
