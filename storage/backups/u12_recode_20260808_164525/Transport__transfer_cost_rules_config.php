...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['movement_type'])) {
    ...
    if ($company_id <= 0)         { ems_gov_flash_redirect('transfer_cost_rules_config.php', 'لا يمكن الحفظ بلا شركة صالحة ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if (!array_key_exists($movement_type, $movement_types) || !array_key_exists($duration_operator, $operators) || !array_key_exists($default_bearer, $bearers)) {
        ems_gov_flash_redirect('transfer_cost_rules_config.php', 'بيانات غير مكتملة ❌', 'GOV-INFO-200', ''); exit();
    }
    ...
    try {
        ...
        ems_gov_flash_redirect('transfer_cost_rules_config.php', 'حدث خطأ أثناء الحفظ ❌', 'GOV-INFO-200', ''); exit();
    }
}
