...
if (!$can_view) {
    ems_gov_flash_redirect('../login.php', 'لا توجد صلاحية عرض الموردين ❌', 'GOV-PERM-403', '');
    exit();
}
...
if (isset($_GET['delete_id'])) {
    ...
    if (!$scope_check) {
        ems_gov_flash_redirect('suppliers.php', 'لا يمكن حذف مورد لا يتبع لشركتك ❌', 'GOV-INFO-200', '');
        exit();
    }
    ...
    if ($equip_count > 0 || $contracts_count > 0) {
        ems_gov_flash_redirect('suppliers.php', 'لا يمكن حذف المورد لأنه مرتبط بمعدات أو عقود موجودة ❌', 'GOV-INFO-200', '');
        exit();
    }
    ...
    try {
        ...
        ems_gov_flash_redirect('suppliers.php', 'حدث خطأ أثناء الحذف ❌', 'GOV-INFO-200', '');
        exit();
    }
}
