...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['unit_code'])) {
    ...
    if ($code === '' || $name === '') { ems_gov_flash_redirect('accountants_fin.php', 'بيانات الوحدة غير مكتملة ❌', 'GOV-INFO-200', ''); exit(); }
    try {
        ...
        if (strpos($e->getMessage(), 'Duplicate entry') !== false) { ems_gov_flash_redirect('accountants_fin.php', 'كود الوحدة مكرر ❌', 'GOV-INFO-200', ''); exit(); }
        error_log('fin_units insert refused: ' . $e->getMessage());
        ems_gov_flash_redirect('accountants_fin.php', 'حدث خطأ أثناء الحفظ ❌', 'GOV-INFO-200', ''); exit();
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['admin_module'])) {
    ...
    if ($emp <= 0 || $mod === '' || $unit <= 0) { ems_gov_flash_redirect('accountants_fin.php', 'بيانات المحاسب غير مكتملة ❌', 'GOV-INFO-200', ''); exit(); }
    try {
        ...
        if (strpos($e->getMessage(), 'Duplicate entry') !== false) { ems_gov_flash_redirect('accountants_fin.php', 'المحاسب مُسنَد لهذه الإدارة مسبقاً ❌', 'GOV-INFO-200', ''); exit(); }
