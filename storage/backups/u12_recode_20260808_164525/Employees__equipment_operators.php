...
if (!$can_view) { ems_gov_flash_redirect('../login.php', 'لا توجد صلاحية عرض المشغلين ❌', 'GOV-PERM-403', ''); exit(); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$is_editing) {
        if ($employee_id <= 0) { ems_gov_flash_redirect('equipment_operators.php', 'يجب اختيار موظف ❌', 'GOV-INFO-200', ''); exit(); }
        ...
        if ($dup) { ems_gov_flash_redirect('equipment_operators.php', 'هذا الموظف مسجّلٌ مشغّلاً مسبقاً ❌', 'GOV-INFO-200', ''); exit(); }
        $ok = false;
        try {
