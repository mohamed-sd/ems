...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['entity'])) {
    ...
    if ($company_id <= 0)          { ems_gov_flash_redirect('master_data_proc.php', 'لا يمكن الحفظ بلا شركة صالحة ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if ($entity === 'lookup') {
        ...
        if (!in_array($type, $lookup_types, true) || $name === '') {
            ems_gov_flash_redirect('master_data_proc.php', 'بيانات غير مكتملة ❌', 'GOV-INFO-200', ''); exit();
        }
        ...
        try {
            ...
            ems_gov_flash_redirect('master_data_proc.php', 'حدث خطأ أثناء الحفظ ❌', 'GOV-INFO-200', ''); exit();
        }
    }
    ...
    ems_gov_flash_redirect('master_data_proc.php', 'كيان غير معروف ❌', 'GOV-INFO-200', ''); exit();
}
