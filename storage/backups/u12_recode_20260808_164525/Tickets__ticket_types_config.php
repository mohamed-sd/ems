...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['name'])) {
    ...
    if ($company_id <= 0)          { ems_gov_flash_redirect('ticket_types_config.php', 'لا يمكن الحفظ بلا شركة صالحة ❌', 'GOV-INFO-200', ''); exit(); }
    ...
    if ($name === '' || !in_array($owner_role_id, $owner_ids, true) || !array_key_exists($default_nature, $natures)
        ...
        ems_gov_flash_redirect('ticket_types_config.php', 'بيانات غير مكتملة ❌', 'GOV-INFO-200', ''); exit();
    }
    ...
    try {
        if ($is_editing) {
            ...
            if (!$row || $row['company_id'] === null) {
                ems_gov_flash_redirect('ticket_types_config.php', 'الأنواع العامة للقراءة فقط ❌', 'GOV-INFO-200', ''); exit();
            }
        ...
        ems_gov_flash_redirect('ticket_types_config.php', 'حدث خطأ أثناء الحفظ ❌', 'GOV-INFO-200', ''); exit();
    }
}
