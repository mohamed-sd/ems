...
if (isset($_GET['run_dep'])) {
    ...
    if (!fin_verify_action_token()) { ems_gov_flash_redirect('assets_fin.php', 'رمز الحماية غير صالح ❌', 'GOV-INFO-200', ''); exit(); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['code'])) {
    ...
    if ($code === '' || $name === '' || $cost <= 0) { ems_gov_flash_redirect('assets_fin.php', 'بيانات الأصل غير مكتملة ❌', 'GOV-INFO-200', ''); exit(); }
    try {
        ...
        if (strpos($e->getMessage(), 'Duplicate entry') !== false) { ems_gov_flash_redirect('assets_fin.php', 'كود الأصل مكرر ❌', 'GOV-INFO-200', ''); exit(); }
        error_log('fin_assets insert refused: ' . $e->getMessage());
        ems_gov_flash_redirect('assets_fin.php', 'حدث خطأ أثناء الحفظ ❌', 'GOV-INFO-200', ''); exit();
    }
