...
if (!function_exists('cdnote_approve')) {
    ...
    function cdnote_approve($conn, $gate, $note_id, $uid = 0)
    {
        ...
        /* ══ INJ-0027 — **يدٌ معلومةٌ قبلَ يدٍ ثانية**.
             كشفه جسُّ `tools/fix_inj0027_tests.php`: إشعارٌ بـ`created_by = 0`
             يجيزه `uid = 0` فتتحرك الذمّةُ ويُخزَّن `approved_by = NULL` —
             لأن حارسَ الذاتِ يصمت أمام مُنشئٍ مجهول، وفحصَ اليدين يشترط
             `$preparer > 0`. فالصفرُ عبَر البابين معًا.
             والحكمُ: «مستخدمٌ آخرُ **بصلاحية المالية** يجيزه» — والمجهولُ ليس
             مستخدمًا آخر، بل لا مستخدمَ أصلًا. ومنادي الإنتاجِ الوحيدُ
             (`Contracts/claims.php`) يمرّر مستخدمَ الجلسة، فلا مسارَ مشروعًا
             يُكسر. */
        if (intval($uid) <= 0) {
            $out['code'] = 403;
