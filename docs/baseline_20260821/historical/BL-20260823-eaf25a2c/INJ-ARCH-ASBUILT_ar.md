...
> **اللقطة:** `BL-20260823-eaf25a2c` · القياس: 2026-08-23 · 06:34→07:10 · التفصيل في [SNAPSHOT.md](SNAPSHOT.md)
> **اللقطات الثلاث السابقة** في [`historical/`](historical/) — أرقامها تاريخ.
...
> **المعرّفات المشتركة:** `SCR-xxxx` (INJ-SCREENS-MASTER.xlsx) · `CAP-xx` · `GAP-xx` · **و`FR-xxx-nnn` (سجل المتطلبات الرسمي — 61 مطلبًا، مصدره `tools/injfrd01_status.php`)**.

---

## §0 ما تغيّر معماريًّا منذ اللقطة السابقة (90 التزامًا · جولة INJ-FRD-REM-01)

**أكبر جولة في السلسلة.** وطبيعتها مختلفة عمّا سبقها: الجولات الثلاث الأولى قاست النظام ثم أصلحت ما وجدته. **هذه الجولة قاست النظام على وثيقةٍ خارجية** — سجل متطلبات رسمي من 61 مطلبًا مقسّم إلى ثماني حزم تغيير، وكل مطلب له حالة إغلاق منفصلة وشاهد يسمّيه.

| # | التغيير | الشاهد |
|---|---|---|
| ١ | **سجل متطلبات حاكم بحالات منفصلة**: 61 مطلبًا · **26 مُغلَق بالدليل · 21 منفَّذ لا مُغلَق · 13 محجوز بقرار مالك · 1 محجوز بغياب مصدر · صفر مفتوح**. و«لا نسبة إجمالية واحدة» بنصّ الأمر | `tools/injfrd01_status.php` · `docs/INJ-FRD-REM-01_CLOSURE_REPORT_ar.md` (مشتقّ حيًّا لا مكتوب بيد) |
| ٢ | **`GAP-14` أُغلق فعليًّا** بعد ادّعاءَي إغلاق كاذبَين: `$verb` صار يُعرَّف في `:218` **قبل** استعماله في `:240`، وأُضيف عمودا **`verb` و`verb_state`** إلى `guard_denials` بقيد `chk_denial_verb_present` — والقيد يميّز `PRE_VERB` (رفض يسبق حلّ الفعل مشروعًا) من `REQUIRED` (5,354 مقابل 351) | `action_guard.php:218,240` · `SHOW COLUMNS` · `CHECK_CONSTRAINTS` |
| ٣ | **`GAP-22` أُغلق**: `fin_party_scope` صار يقرأ **سجلًّا** (`fin_party_scope_registry`) ويعيد `PARTY_SCOPE_NONE` للدور ≤ 0 — **fail-closed بدل `return null`** الذي كان يعني «كل الأطراف» | `Finance/fin_helpers.php` · `FR-SEC-001 EVIDENCE_CLOSED` |
| ٤ | **مانع إنفاذ السلّم أُزيل — والإعلان بقي محجوزًا بقياس**: قياس الأثر قبل التفعيل كشف أن `enforce` كان سيمنع **31 من 76 مستخدمًا** بعطب قراءة (`role_id` فارغ)، فأُصلح بدالّة `ems_resolve_actor_role` تقرأ بالعمودَين في ثلاثة مواضع. والإعلان محجوز لأن **`gov_ladder_shadow` صفر صف** فلا نافذة أثر مقيسة | `96e36541` · `.env` |
| ٥ | **حزام أدلّة يقيس الدفتر على نفسه** — كل دليل على التزامه، ولا دليل يسبق نفسه، والسجل يُقارَن بمصدره فلا يتعفّن | `c508cb3d` · `eaf25a2c` |
| ٦ | **ثلاث حزم أُغلقت بصفر مفتوح**: الأحداث · الصلاحيات · سلامة المالية — و`CHG-*` الثماني كلها **صفر مفتوح** | جدول الحزم في تقرير الإغلاق |

**والنمط الحاكم لهذه الجولة — «الحالة تُفصَل ولا تُجمَع»:** بدل «مغلق/مفتوح» صار للمطلب أربع حالات، أهمها التمييز بين **`EVIDENCE_CLOSED`** (26) و**`IMPLEMENTED_NOT_CLOSED`** (21): الكود منفَّذ والشاهد قائم، لكن شرط الإغلاق (نافذة ظلّ · ممارسة بشرية · مصدر حاكم) لم يُستوفَ. **وهذا بالضبط ما طلبته قاعدة «القرار + الكود + بلا اختبار ≠ Closed».**
...
```
┌─ المتصفح ──────────────────────────────────────────────────────────────────┐
│ شاشة PHP (458 مصيَّرة) │ topbar (10 روابط · مُعفاة بسجل) │ AJAX │ /api (Bearer)│
└──────┬──────────────────────┬───────────────────┬────────────┬─────────────┘
       ▼                      ▼                   ▼            ▼
...
│  ب) role_permissions(3130)         │  │ (dynamic_nav القديم: للسوبر فقط)     │
...
│ 10 مواضع —     │ │ events (21,538) │ │ → PostingService → journal_entries │
...
│ → ems_job_queue (9,570 منجزة) → 8 جدولات · الإنذار يعمل بحق              │
└────────────────────────────────────────────────────────────────────────────┘
┌─ القاعدة ─ 655 جدول أساس · 497 بعمود company_id · FK 296 · CHECK 355 ─────┐
```
...
- **الثاني الحي:** `check_permission()` (`:191`، الاستعلام الخام `:207-209`) و`unified_nav.php:88-94` يقرآن `role_permissions` (3,130) متجاهلَين القالب ⇒ القائمة والأزرار من مصدر والدخول من آخر.
...
**الأرقام:** 655 جدول أساس + 28 منظرًا · **497** بعمود `company_id` · FK **296** · CHECK **355** · UNIQUE **422** · Triggers **صفر مرئية للمستخدم `ems_app`** وهي موجودة فعلًا (امتياز يحجبها — تُقاس وظيفيًّا).
...
| أحداث | `ems_business_events`(21,538) + `ems_event_deliveries`(21,295) + `event_consumers`(91) + منظران | EventPublisher/Worker حصرًا | governance_watch + شاشات المراقبة | company_id | `chk_consumers>0` · `json_valid` · **`fk_evdeliv_event` CASCADE (جديد)** |
