...
> **ستُّ لقطاتٍ سابقة** في [`historical/`](historical/) — أرقامها تاريخ.
...
> ⛔ **تحذيرُ إعادةِ الإنتاج:** الشجرةُ العاملةُ فيها **٥٦ بندًا غيرَ ملتزم** — فما تصفه هذه الوثيقةُ **شجرةٌ عاملةٌ لا التزام**.

---

## §0 ما تغيّر معماريًّا منذ اللقطة السابقة (صفرُ التزامٍ جديد · وجولةٌ كاملةٌ خارج الالتزام)

**المعماريةُ نفسُها لم تتغيّر — وهذا مقيسٌ لا مُفترَض:** `git status` يُظهر أنّ التعديلَ محصورٌ في `tools/` و`docs/`، و**صفرُ ملفٍّ** في `app/` أو `includes/` أو `database/` أو `tests/`. ⇒ **الأحكامُ البنيويةُ في §١…§٩ قائمةٌ بحرفها**، والذي تغيّر: **البياناتُ والسجلّاتُ وحالةُ الشجرة**.

| # | التغيير | الشاهد |
|---|---|---|
| ١ | ⛔ **الشجرةُ اتّسخت ثانيةً**: صفر ⇒ **٥٦ بندًا** (**٣٩ من جولةِ `RPR-02-A`** 18:20→19:53 **خارجَ أيِّ التزام** · و١٧ من مخرجاتي) ⇒ `GAP-70` **أُعيد فتحُه** | `git status` |
| ٢ | ⛔ **البصمةُ سقطت تبعًا** — `✘ البصمةُ المحسوبةُ تطابق المسجَّلة` ⇒ **فاحصٌ واحدٌ راسبٌ أسقط ثلاثَ فجواتٍ** (`GAP-14` · `GAP-26` · `GAP-32`) | `injfix01_release_and_action_finality_proof.php` |
| ٣ | ⛔ **أربعُ شاشاتٍ ارتدَّت ملكيتُها إلى `EX-CEO`** رغم أنّ **قاعدةَ حكمِها المكتوبةَ** تنصُّ على إخراجِها منه (`RPR-W15 §٤·٢`) · و`SCR-0532` دُهس حكمُ `W01_SURFACE_REGISTER` بـ`W10_SPLIT:W10_PARENT_DEFAULT` | مقارنةُ السجلِّ بلقطتِه السابقة |
| ٤ | ✔ **`PLATFORM` صار إدارةً مسجَّلةً** — `repair01_departments` **21 ⇒ 22** ⇒ `GAP-68` **مُغلق**: الاثنتا عشرةَ شاشةً صار لمالكها اسمٌ لا رمزُ قرار | قياس حي |
| ٥ | ⛔ **`GAP-18` لم يكن مُغلقًا** — البوابةُ 4/4 خضراء (شقُّ الدفتر) **وشقُّ البصمةِ راسب**: 8 جداولَ حيّةٍ غائبةٌ عن الاستنساخ و**فرقُ أعمدةٍ 3,434** | `injfix01_clean_clone_fingerprint_proof.php` |
| ٦ | ⛔ **مقياسُ الإغلاق تراجع 27/33 ⇒ 22/33** — بسببِ **أربعِ شواهدَ حمراءَ من 34** لا بسببِ خمسِ انتكاساتٍ مستقلّة | `tools/injfix01_gap_coverage.php` |
| ٧ | ◐ **سجلُّ المتطلباتِ اتّسع 61 ⇒ 66** بخمسةِ مطالبَ جديدةٍ (`FR-GOV-007..011`، منها **P0 واحد**) **كلُّها `OPEN`** ⇒ البسطُ ثابتٌ عند 26 والمقامُ نما، فالنسبةُ **42.6٪ ⇒ 39.4٪** بلا تراجعِ عمل | `tools/injfrd01_status.php` |
| ٨ | ◐ **سجلّاتُ REPAIR01 نمت**: قرارات 108 ⇒ **125** · حقول 7,491 ⇒ **7,591** · متطلبات 429 ⇒ **433** · فجواتُ هدفٍ 334 ⇒ **335** | `baseline_db_dump5` |
| ٩ | **الفروق الطفيفة**: الطابور منجز 15,848 ⇒ **16,208**. وما عداه **ثابتٌ تمامًا**: الجداول 953 · CHECK 831 · التنقّل 2096/1768/637 · الصلاحيات 3757 · الأحداث 21,546 بـ58 مفتاحًا · القيود 6,713/13,426 بفرق **0.00** · الظهورات 1200 · التسويات 469 بصفرٍ مدفوع | قياس حي |

**النمطُ الحاكمُ في هذه اللقطة — «المقامُ الذي لا يشمل المتغيِّر يُعمي التقرير»:**
- تقريرُ إعادةِ الاستيعاب (`RPR02A_REINGEST.md`) يعلن **تسعةَ عشرَ مقياسًا بـΔ=0**، وهو صادقٌ فيما عدَّ — **ولا يعُدُّ `repair01_screen_registry`**، وهو الجدولُ الذي تغيّر فيه **52 صفًّا**. ⇒ **«صفرُ فرقٍ» يصدُق على المقامِ ويكذب على السؤال.**
- **حكمٌ يحمل قاعدتَه ويناقضها**: القاعدةُ «إعادةُ ملكيةٍ من مكتبِ الرئيس» مكتوبةٌ في **11** صفًّا — **7 تُطيعها و4 تناقضها**. ⇒ **نصُّ القاعدةِ لا يحرس القيمة؛ الحارسُ وحدَه يحرسها.**
- **علاجٌ بلا حاجبٍ يرتدُّ**: نظافةُ الشجرةِ عولجت في اللقطةِ السابقةِ **بفعلٍ لا بحاجب**، فارتدَّت في أوّلِ جولةٍ تالية — بخلافِ دفترِ الهجراتِ الذي عولج **بحاجبٍ** فصمَد.
...
│ → ems_job_queue (16,208 منجزة) → 8 جدولات · الإنذار يعمل بحق        │
└────────────────────────────────────────────────────────────────────────────┘
...
**المقام: 66 مطلبًا** (كان 61) موزّعةً على **تسعِ** حزمِ تغيير — و**خمسةٌ مفتوحةٌ الآن** (`FR-GOV-007..011` في `CHG-RPR-0203`، منها **P0 واحد**) بعد أن كان **صفرٌ مفتوح**.
| الحالة | العدد | من **66** | المعنى |
|---|---:|---:|---|
| `EVIDENCE_CLOSED` | **26** | **39.4٪** | شاهد مُشغَّل يثبت الإغلاق |
| `IMPLEMENTED_NOT_CLOSED` | **21** | **31.8٪** | الكود منفَّذ والشاهد قائم — وشرط الإغلاق لم يُستوفَ |
| `BLOCKED_OWNER_DECISION` | **13** | **19.7٪** | يلزمه فعل مالك: نافذة ظلّ · ممارسة تفويض · قرار مقام |
| `BLOCKED_GOVERNING_SOURCE` | **1** | **1.5٪** | مصدره الحاكم (`workbook.xlsx`) غير متاح |
| `OPEN` | **5** | **7.6٪** | `FR-GOV-007..011` — مُعلَنةٌ في `CHG-RPR-0203` ولم يبدأ عملُها؛ **منها P0 واحد** |
...
| **`CHG-RPR-0203`** | 0 | 0 | 0 | **5** | **5** — **الحزمةُ التاسعةُ الجديدة**: `FR-GOV-007..011` كلُّها مُعلَنةٌ ولم يبدأ عملُها |
...
**المصدر:** `repair01_screen_registry` (783) · `repair01_departments` (**22** — بعد تسجيلِ `PLATFORM`) · وحولهما **141 جدول `repair01_*`**.
...
| `owner_code` + `owner_rule` | `DEP-nn` + قاعدة الإسناد (`NAV_CANONICAL_OWNER` · `W01_SURFACE_REGISTER` …) | **صفر شاشة بلا مالك** (783/783) — والأساسُ مكتوبٌ لا مستنتَج · ⛔ **لكنّ النصَّ لا يحرس القيمةَ**: أربعةُ صفوفٍ قاعدتُها تنقض مالكَها (GAP-73) |
...
- **المستهلكون — البسطُ ما زال صفرًا، والطرفان يكبران ولا يلتقيان:** **0 / 58** مفتاحًا منتَجًا له مستهلكُ أعمالٍ نشط — **الثمانيةُ والخمسون كلُّها بحارسِ الحوكمةِ وحدَه** (`handler_class` = `GovernanceWatchConsumer` في **58/58** من `gov_event_rulings`). وفي المقابلِ **الاشتراكاتُ الفعّالةُ غيرُ الحارسِ نمت 4 ⇒ 32** موزَّعةً على خمسِ عائلاتٍ خدميّة (`FinancingCycleService` 10 · `AccountingCycleService` 9 · `SplitProjectionConsumer` 5 · `TreasuryCycleService` 4 · وأربعٌ أخرى) — **وكلُّ الاثنتين والثلاثين على مفاتيحَ لا تُنتَج ولا مرّةً واحدة**. ⇒ **ثمانيةُ أضعافِ التوصيلِ وصفرُ تماسٍّ بالواقع**: 58 مفتاحًا يُنتَج · 102 مفتاحًا مشترَكٌ عليه · **التقاطعُ بمستهلكِ أعمالٍ = صفر**.
...
| **GAP-18** | دفترُ الهجراتِ **وبصمةُ المخطَّط** | — | ◐ **شقٌّ مُغلقٌ وشقٌّ راسب**: الدفترُ يطابق القرصَ (`repair01_migration_gate` **4/4** بمقاماتٍ مطبوعة · 58 ⇒ صفر) — **وبصمةُ المخطَّطِ لا تُعاد إنتاجُها من استنساخٍ نظيف**: **8 جداولَ حيّةٍ غائبةٌ** عن الاستنساخ و**فرقُ أعمدةٍ 3,434**. ⇒ **إغلاقي السابقَ كان على شقٍّ واحدٍ من حكمٍ ذي شقَّين**  | **نعم — يمنع إعادةَ الإنتاج · P1** |
...
| GAP-68 | 12 شاشة مالكها `PLATFORM` بلا اسمِ إدارة | — | **مُغلق** ✔ — `PLATFORM` صار صفًّا في `repair01_departments` (21 ⇒ **22**) فصار للمالكِ اسمٌ لا رمزُ قرار | — |
...
| **GAP-70** | **الشجرةُ العاملةُ غيرُ نظيفةٍ أثناء القياس** | — | ⛔ **أُعيد فتحُه** — صفر ⇒ **٥٦ بندًا** (جولةُ `RPR-02-A` خارجَ الالتزام). **والعلاجُ السابقُ كان فعلًا لا حاجبًا فارتدّ في أوّلِ جولةٍ تالية** · وأثرُه مقيس: أسقط بصمةَ الإصدارِ ومعها ثلاثُ فجوات  | **نعم — يمنع إعادةَ الإنتاج · P1** |
...
| **GAP-73** | **السجلُّ الرسميُّ يناقض قاعدتَه المكتوبة**: أربعةُ صفوفٍ تحمل `verdict_rule` = «RPR-W15 §٤·٢ · إعادةُ ملكيةٍ من مكتبِ الرئيسِ إلى إدارتِها» **ومالكُها `EX-CEO`** — أي عادت إلى ما نصَّت القاعدةُ على إخراجِها منه (`SCR-0505` · `SCR-0508` · `SCR-0515` · `SCR-0532`). **والمقياسُ بسؤالٍ واحد: القاعدةُ في 11 صفًّا — 7 تُطيعها و4 تناقضها.** وفي `SCR-0532` دُهس `owner_rule` من `W01_SURFACE_REGISTER` إلى `W10_SPLIT:W10_PARENT_DEFAULT` | **نعم** — يُفسد عمودَ الملكيةِ في المعرّفِ المشترك | **جديد** | **P1** |
...
---
*مولَّدة قياسًا حيًّا على `BL-20260828c-96a640e1+WT` — **على شجرةٍ عاملةٍ فيها ٥٦ بندًا غيرَ ملتزم**. أي قراءة بعد هجرة أو التزام جديد تستلزم إعادة قياس أو وسم `STALE`.*
