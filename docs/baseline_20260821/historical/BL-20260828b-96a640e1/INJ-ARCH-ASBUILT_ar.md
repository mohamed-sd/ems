...
> **اللقطة:** `BL-20260828b-96a640e1` · القياس: 2026-08-28 · 16:35→17:15 · التفصيل في [SNAPSHOT.md](SNAPSHOT.md)
> **خمس لقطات سابقة** في [`historical/`](historical/) — أرقامها تاريخ.
...
## §0 ما تغيّر معماريًّا منذ اللقطة السابقة (4 التزامات · الطور صفر من REPAIR01)

**دلتا صغيرة وأثر حاكم.** أربعة التزامات أغلقت الفجوة الوحيدة التي أعدتُ فتحها في اللقطة السابقة، **بالعلاج الذي يمنع تكرارها لا بالإدخال اليدوي**.

| # | التغيير | الشاهد |
|---|---|---|
| ١ | **بوابة الهجرات بُنيت** — `tools/repair01_migration_gate.php` بأربعة حواجب، **كلٌّ يطبع مقامه**: `G-MIG-01` (0 غير مقيَّدة من 379) · `G-MIG-02` (462 مُدارة · صفر خارج الدفتر) · `G-MIG-03` (703 صفًّا · 241 بلا ملف · **صفر بلا حكم**) · `G-MIG-04` (1 خارج العُرف · صفر بلا حكم). **4/4 أخضر — شغّلتُها بنفسي** | تشغيل حي |
| ٢ | **الهجرات خارج الدفتر 58 → صفر** · والدفتر 645 → **703** (577 applied + 126 baseline) | مقارنة glob×ledger |
| ٣ | **الشجرة نُظِّفت** — 13 بندًا غير ملتزم → **صفر** ⇒ كل السطح المقيس داخل التزام (GAP-70 مُغلق) | `git status` |
| ٤ | **التقارير الثلاثة أُعيدت على شجرة نظيفة** — «فبصمةٌ تصف حالةً منقضيةً لا تصف شيئًا» | `6221090b` |
| ٥ | **مراجعة عكسية للبرومت نفسه**: 65 مطلبًا **كلٌّ بمرساةٍ نصّيّة** · الجولة الأولى 46/62 ثم **65/65** · و**ستٌّ من الغائبات كانت عطبَ الفاحصِ لا المحتوى** (ترميز ماركداون يقطع العبارة) فأُصلحت الأداةُ لا مخرجُها · و**المقام نفسه كان ناقصًا صفَّين** فدُقِّق قبل الحكم | `96a640e1` |
| ٦ | **الفروق الطفيفة**: جداول 952 → **953** · CHECK 829 → **831** · ظهورات المساحات 1199 → **1200** · الطابور منجز 15,534 → **15,848**. وما عداه **ثابت** (التنقّل · الصلاحيات · الأحداث · القيود) | قياس حي |

**النمط الحاكم — «العلاج بقدر الارتداد، والحاجب يُثبَت بالكسر»:**
- الحاجب **لا يُقبل أخضره وحده**: كان راسبًا سلفًا بـ57، فـ«رسَب بعد الكسر» يصدُق ولو لم تُحدث الهجرة الصورية أثرًا. **فصار الدليل حركةَ عدّاد بواحدٍ بالضبط ثم عودة**: `[0,379,0,0]` ⇒ `[1,380,1,1]` ⇒ `[0,379,0,0]`.
- **القيدُ فعلٌ لا نيّة**: هجرة تستدعي الدفتر ولم تُشغَّل ⇒ `G-MIG-02` يرسُب.
- **وحاجبٌ يوجب كسر حاجبٍ آخر ليس حاجبًا**: الصياغة الأولى طالبت 326 ملفًّا، منها 269 مقيَّدة سلفًا وتحريرها يُشعل كاشف التحرير في `migrate.php` — **فحُصر المقام في غير المقيَّد وحده**.

⇒ **ويطابق هذا تشخيصي في اللقطة السابقة حرفًا**: القاعدة كانت سليمة وتعمل حين تُستدعى (قياسهم: 52 تستدعي وكلها مقيَّدة · 57 لا تستدعي وكلها خارج الدفتر) — **والعطب أن الاستدعاء عُرفٌ، فصار شرطًا**.
...
│ → ems_job_queue (15,848 منجزة) → 8 جدولات · الإنذار يعمل بحق        │
└────────────────────────────────────────────────────────────────────────────┘
...
| GAP-18 | هجرات خارج الدفتر | — | **مُغلق بحاجزٍ مُثبَتٍ بالكسر** ✔ — 58 → **صفر** · `repair01_migration_gate.php` 4/4 بمقامات مطبوعة · والدليل حركةُ عدّادٍ بواحدٍ ثم عودة | — |
...
| GAP-70 | الشجرة العاملة غير نظيفة أثناء القياس | — | **مُغلق** ✔ — 13 بندًا → **صفر**؛ والتقارير أُعيدت على شجرة نظيفة | — |
...
---
*مولَّدة قياسًا حيًّا على `BL-20260828b-96a640e1`. أي قراءة بعد هجرة أو التزام جديد تستلزم إعادة قياس أو وسم `STALE`.*
