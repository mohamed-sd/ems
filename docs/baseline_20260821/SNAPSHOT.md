...
**معرّف اللقطة:** `BL-20260901-13010626` — كل رقم في مخرجات هذه الحزمة مقيسٌ على هذه اللقطة وحدها.
...
> **تشغيلٌ حاكمٌ واحدٌ** على `HEAD=13010626` (شجرةٌ نظيفةٌ قبل القياس) أعاد توليدَ:
...
>
> **وأربعُ نتائجَ تُقرأ قبل أيِّ رقم** (دلتا حملةِ GOV_EXEC aba54573 ⇒ 13010626 · **7 التزامات**):
...
| البند | القيمة | الدليل |
|---|---|---|
| تاريخ/وقت القياس | **2026-09-01 · 02:35 → 02:45** (UTC+03) | طوابع الاستخراج |
...
| commit_hash | **`13010626`** — «GOV_EXEC §11 و§23» | `git rev-parse HEAD` |
| التزامات غير مدفوعة | **326 تجاه `origin/main`** · **7 التزامات منذ لقطة aba54573** | `git rev-list --count` |
| الوسوم | **20** | `git tag` |
| **الشجرة العاملة** | ✔ **نظيفة عند بدء القياس** — والمتغيّر بعده مخرجات التشغيلة الحاكمة نفسها | `git status --porcelain` |
| إصدار الـSchema | **998 جدولًا أساسيًّا** (+3: مصالحةُ المعرِّفات · جدولُ الإسناد · سجلُّ نفاذِ القرارات) + 28 منظرًا | information_schema |
| **دفتر الهجرات** | `schema_migrations` **822 صفًّا** (+6: ثلاثُ هجراتٍ وعكوسُها baseline) · خارج الدفتر **صفر** · البوابة **4/4 خضراء** | `tools/repair01_migration_gate.php` |
...
| تمرين التثبيت | **`DR-2026-0010` pass** على المخطَّط الحالي (1026 كائنًا) | `dr_drills` |
| ثبات اللقطة | `HEAD` لم يتغيّر أثناء التشغيلة الحاكمة | `git rev-parse HEAD` قبل/بعد |

## حزمةُ الأدلّةِ الموحَّدة (GATE-00)

الوثائقُ الثماني — كلُّها بمعرِّف `BL-20260901-13010626`:
