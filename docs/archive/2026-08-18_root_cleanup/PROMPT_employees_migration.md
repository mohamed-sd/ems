...
> **هذا تغيير جوهري واسع المدى يمسّ مكاناً مركزياً. نفّذه على نسخة قاعدة بيانات تجريبية (staging) ونسخة من الملفات، مع اختبار انحدار شامل قبل أي تطبيق على الإنتاج.**
> **مهم: لا تنفّذ أي أوامر git إطلاقاً (لا فرع، لا commit، لا أي شيء). الاعتماد فقط على نسخة احتياطية يدوية للملفات وقاعدة البيانات.**
...
- **أسماء أعمدة المفاتيح في الجداول الأخرى تبقى كما هي** (`driver_id`, `timesheet.driver`...) وتشير إلى `employees.id` — **لا تُعاد تسميتها إطلاقاً في هذه المرحلة** (هذه «الموجة 1»؛ إعادة تسمية الأعمدة `driver_id → employee_id` مؤجَّلة لمرحلة لاحقة منفصلة «الموجة 2»). **لا تنفّذ أي إعادة تسمية للأعمدة الآن.**
...
1. **لا تستخدم git إطلاقاً.** خذ **نسخة احتياطية يدوية كاملة لقاعدة البيانات** (mysqldump) و**نسخة من ملفات المشروع** قبل أي تعديل، ونفّذ كل شيء على **نسخة staging من قاعدة البيانات** أولاً.
...
6. **خطة تراجع (Rollback):** موثّقة — **استعادة النسخة الاحتياطية اليدوية للملفات وقاعدة البيانات** عند أي فشل في الانحدار (بدون git).
