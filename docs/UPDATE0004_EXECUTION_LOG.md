...
## الموجة ⑨ · SEC القوالب والتغيير — 4/4

| المهمة | ما فُعل | الملفات | الاختبار | اجتهاد |
|---|---|---|---|---|
| SEC-23 | `PermissionTemplateService`: مسودة → **لا وسم «مختبَر» ووضع اختبار الصلاحيات مطفأ** → **لا نشر بلا أثر محسوب** (كم مستخدمًا · المضاف · المسحوب مقابل المنشور) → نشر يجعل السابق superseded · وتعديل النافذ **423** | `PermissionTemplateService.php` | S24 (عشرة أوجه) ✅ | — |
| SEC-24 | `PermissionChangeWorkflow`: فتح بrisk من النوع وخطوات §8 · قرارات متسلسلة بحارس الذات (منشئ الطلب والمستفيد لا يعتمدان) · **الرباعية لا تُختصر: بثلاث يبقى معلقًا ولا يُطبَّق** · apply بعد الاكتمال يعيد بناء المشتق | `PermissionChangeWorkflow.php` | S19 (ستة أوجه) ✅ | — |
| SEC-25 | `PermissionReviewService`: فتح دورة ببنود (موظف×صلاحية) من المشتق · قرار البند مرة واحدة (Insert-only) · لا توقيع وفيها بند بلا قرار · التوقيع للمدير وحده · والمتأخرة **escalated** بتنبيه الإدارة العامة | `PermissionReviewService.php` | S25 (سبعة أوجه) ✅ | — |
| SEC-26 | شاشتا الحوكمة: **مركز الحوكمة بمجموعاته الثماني** (§10.1: لوحة · اعتماد الطلبات · متابعة الخامل/المنتهي/بلا مصدر · الإغلاق بالمراجعة والتأسيس · تقرير «لماذا يملك؟» حيًّا · إعدادات القوالب والحراس) + **معالج الإحدى عشرة خطوة** بمعاينة ⑨ وإرسال ⑩ — للدور 15 في باب GOV | `admin/sec_governance.php` · `admin/sec_employee_wizard.php` · هجرة التسجيل | HTTP 200 بالعلامات للدور 15 ✅ | J-24 |

**الحزام:** `tests/sec_templates_workflow_test.php` = **24/24** ✅

## الموجة ⑩ · SEC التأسيس والقلب — 4/4 (البوابة ② مغلقة 30/30)

| المهمة | ما فُعل | الملفات | الاختبار | اجتهاد |
|---|---|---|---|---|
| SEC-27 | `FoundingModeService`: الوضعان معًا · تفعيل بلا نهاية → 422 (S13) · الشريط الظاهر بنص ونهاية · التوسيع في discovery وحده · والحراس لا يُعطَّلون (اختُبروا مفعَّلين) | `FoundingModeService.php` | S13·S14 ✅ | — |
| SEC-28 | البروتوكول السداسي: جرد → قوالب → إخراج المتدربين (أرشفة لا حذف) → إعادة التصنيف → الخفض الجماعي بقرار واحد → **شهادة ببنودها الأربعة ولا إغلاق مدَّعى** — ولا إغلاق قبل اجتياز وضع اختبار الصلاحيات | نفسه (closeProtocol) | S15 ✅ | — |
| SEC-29 | `PermSourceService`: العلم الواحد `EMS_PERM_SOURCE=legacy` (لا يُقلب ذاتيًّا §1.3) · قرار القديم بقاعدة تحويل D2 · الظل يقارن ويسجّل كل فرق في `perm_shadow_diffs` · ميزان «صفر فرق 14 يومًا» · تقرير المراحل الست | `PermSourceService.php` · هجرة الظل · `.env` | الظل يسجّل الفرق ✅ | J-25 |
| SEC-30 | الحزام الجامع S1→S27: الأحزمة الأربعة فرعيًّا + S7·S10·S13·S14·S15·S20·S21 هنا + تقرير القلب | `tests/sec_full_belt.php` | **29/29** ✅ | — |

**+ `PositionService::securitySuspend` (S21 §8-⑨): تعليق فوري بلا انتظار وإعادة بناء لحظية.**

## البوابة ③ · TKT-01 — الموجات ⑪→⑭ (18/18)

**القياس قبل التنفيذ:** وحدة بلاغات حية (186/186 · 19 بلاغًا · 10 جداول ticket_*) — `ticket_types` أغنى من المتوقع (code·owner_role_id·default_nature ثلاثي) فالتوسيع لا الاستبدال.

### الموجة ⑪ · البنية — 6/6
| المهمة | ما فُعل | اجتهاد |
|---|---|---|
| TKT-01 | توسيع `ticket_types`: nature الثمانية (خرط default_nature: recurring→problem) · closure_policy الخمس (حادثة/طارئ لا آلي · سلامة committee) · السرية والمجهول · + 6 أنواع للطبائع الناقصة | J-26 |
| TKT-02 | `ticket_type_workstreams` بالتفعيل الشرطي — بذر مسارات عطل المعدة الخمسة (المشتريات شرطي بStockUnavailable) + مسار افتراضي لكل نوع (لا بلاغ بلا مالك) = 27 تعريفًا | — |
| TKT-03 | توسيع `tickets`: head_state مشتق (قيده في الخدمة) · حقلا السرية · السياق المحمول · site/contract/shift/period · روابط التكرار الثلاثة — وخرط المقفل قديمًا | — |
| TKT-04 | `ticket_workstreams` بUQ(البلاغ×النوع×التسلسل) — فللإدارة مساران (فحص ثم إصلاح) | — |
| TKT-05 | holds (سبب محكوم + expected_until إلزامي) · escalations (Insert-only) · participants (duplicate_reporter) · responses (ws_id فارغ للرد المركزي) | — |
| TKT-06 | effects بis_provisional · communications — والاطلاع السري في `sensitive_read_log` القائم (LEG-01) بنطاق ticket.secret | — |

**الحزام:** `tests/tkt_structure_test.php` = **35/35** ✅

### الموجة ⑫ · التوجيه — 4/4
TicketRouter (السياق 422 · **المكلف من تكليفات ORG-01 النافذة بالموقع والتاريخ والنائب** · الأولوية ترتفع لمقعد تعاقدي وقد يرفعها المبلغ ولا يخفضها · لا مكلف → مدير الحركة وتنبيه) · WorkstreamActivator (الحدث يفتح الشرطي · skipped عند الإغلاق) · DuplicateDetector (العرض قبل الحفظ · متابع لا بلاغ ثانٍ · 3 بشهر → مشكلة) · SlaMonitor في `Tickets/cron_tickets.php` (تصعيد بعطالة · التعليق المتجاوز يُصعَّد نفسه · الرفع يزيح الساعة). **الحزام:** `tests/tkt_routing_test.php` = **21/21** ✅

### الموجة ⑬ · الحالات والأثر — 4/4
TicketStateService (كل أحكام T4·T6·T7·T13·T17·T10·T8 — وhead_state لا يكتبه إلا معيد الحساب) · TicketEffectService (**الخطوات الأربع بترتيب محروس 409** — النهائية بعد الاعتمادين فقط) · ConfidentialityGuard (الفصل في الاستعلام · المجهول للحوكمة · الاطلاع مسجل) · خرط الموروث (`tools/tkt_legacy_map.php`: 19 بلاغًا بمسار legacy — صفر موروث بلا مسار). **الحزام:** `tests/tkt_state_effect_test.php` = **25/25** ✅

### الموجة ⑭ · السياق والقياس — 4/4
| المهمة | ما فُعل | اجتهاد |
|---|---|---|
| TKT-15 | مكوّن `includes/report_button.php` + نقطة الفتح `Tickets/ticket_contextual_open.php` (السياق للقراءة · كشف التكرار قبل الحفظ) — **والمواضع الثمانية**: 7 أزرار غنية (الحركة بموقعها · الصيانة بأمرها · المخزون · النقل · المشغلون · الاعتمادات · المستخلص) + **الزر العام في الرأس الموحد لكل شاشة** (⑧) | J-27 |
| TKT-16 | WatchTowerService: المؤشرات الثمانية بمستهدفاتها + تقرير «من يتأخر ومن لا يستجيب» بالاسم والإدارة ومتوسط التأخير + إصداره الدوري | — |
| TKT-17 | شاشتا `ticket_workstreams_board.php` (الرأس المشتق ومساراته بأفعالها) و`watchtower.php` — تسجيل الدور 24 والفتح السياقي لكل الأدوار | — |
| TKT-18 | الحزام الجامع T1→T18 (الثلاثة فرعيًّا + T9 السلامة + T11 الجرد + HTTP) | — |

**الحزام:** `tests/tkt_full_belt.php` = **21/21** ✅ (T9: تصعيد exec مباشر وأثر إيقاف بنيوي وسياسة committee)

## البوابة ④ · NAV-01 — الموجات ⑮→⑱ (12/12)

**القياس قبل التنفيذ:** 623 رابط تنقل · 72 مجمَّعًا فقط (المالية بمجموعات مخصصة 7 حية) · `chk_nav_door` الحي يقبل الثمانية أصلًا (الانحراف كان في الهجرات المؤرشفة وانغلق بمسار التثبيت الجديد) · `nav_redirects` بعدّاد hits قائم (5 صفوف حية) · الدور 5: 17 رابطًا والدور 6: 20 · 6 مستخدمين على الدور 5.

### الموجة ⑮ · الجرد — 3/3
NAV-01 الجرد الواحد (623 صفًّا · `docs/nav01/NAV-D1`) يغذي SEC-02/05 بمرور واحد · NAV-02 مصفوفة الأعمدة السبعة لكل صف بقرار ★ (`NAV-D2`) — صفر صف بلا مجموعة وظهور وإجراء · NAV-03 حسم DEC-NAV-E صفًّا صفًّا (`NAV-D3`): **★الباقي الدور 6** (ORG-01 + تكليف site_movement_mgr) و5 روابط تُنقل (J-28).

### الموجة ⑯ · الأبواب والمجموعات — 3/3
NAV-04 هجرة تصحيحية تضمن القيد بالثمانية (وجدته مقفلًا فأثبتته بنيويًّا في المسار) · NAV-05 g1..g8 قياسية لكل دور (200 مجموعة) وإسناد 551 رابطًا بالكلمات ثم الباب (DEC-NAV-J) · NAV-06 حارس السبعة `tools/nav_seven_guard.php`.

### الموجة ⑰ · مساحة عملي — 2/2
NAV-07 `main/my_workspace.php`: العناصر الثمانية بعداداتها الحية — HOME/g1 لكل الأدوار (HTTP 200 ✅) · NAV-08 `includes/route_redirect.php` يوحّد رصاصات `nav_redirects` القائمة بعدّادها.

### الموجة ⑱ · التجميع — 4/4
NAV-09 المالية: 79 رابطًا من المخصصة إلى القياسية والقديمة أُطفئت (رجوعها قلب صف) · NAV-10 الدمج: 5 روابط نُقلت للدور 6 وقائمة 5 توحدت عليه (25 مرآة والقديمة مطفأة — المستخدمون الستة بلا مساس) · NAV-11 قلب قائمة التشغيل: أُخرج 9 روابط لا يملكها (كيانات · تفويض · تراخيص · تمويل · صلاحيات) active=0 · NAV-12 إعادة التوزيع بالكلمات + تمييز 3 أسماء مكررة — **صفر اسم مكرر وصفر يتيم**.

**القبول المتبقي معلَنًا (صدق التقرير):** «صفر مجموعة فوق سبعة» غير متحقق في **18 (دور×مجموعة)** — كلها من صنف «السجلات g3» للأدوار الغنية، وعلاجها المنصوص تحويل الفائض تبويبات في شاشات أم (NAV-01 §10-②) — **وهو بناء شاشات أم لكل كيان يقترحه NAV-D2 صفًّا صفًّا وينتظر قرار المالك** (تقرير الحارس: `docs/nav01/NAV-D6_seven_guard_report.txt`) — J-29.

## البوابة ⑤ · الجاهزية غير الوظيفية — الموجات ⑲⑳㉑ (15/15)

### الموجة ⑲ · N-25 التوقيت — 4/4
NFR-01 الفحص الحاكم: التخزين محلي (SYSTEM=Egypt DST) وPHP كان UTC — **فارق 3 ساعات موسمي** (لا «ساعة»). NFR-02 التوحيد على ساعة القاعدة (`date_default_timezone_set` خلف `EMS_APP_TIMEZONE`) — فارق 0 ثانية. NFR-03 خطة ترحيل UTC واعية بالDST جاهزة ولا تُشغَّل (الإزاحة المسطحة تفسد الشتاء). NFR-04 موثَّق «إعداد لا ترحيل». **الحزام:** `tests/nfr_timezone_test.php` = **6/6**.

### الموجة ⑳ · N-24 الطابور — 6/6
`ems_job_queue` بمحاولات تصاعدية (1د·5د·25د) وسجل ظاهر · `runBatched` 20×100 بمعاملة لكل دفعة (فشل دفعة لا يسقط الباقي) · معالجات الدوريات/المطابقة/الاستدراك · enqueue فوري + إشعار الاكتمال · **حارس الخمس ثوان** (راصد في config + `mustQueue`). N-06 غير معرَّف (PLAN-02 غائبة — سابقة موثقة). **الحزام:** `tests/nfr_queue_test.php` = **17/17**.

### الموجة ㉑ · N-26 البنية — 5/5
NFR-11 الفحص الحاكم يتصدر (آلة واحدة WAMP — فصلها يسبق كل بند) · NFR-12 OPcache + توجيهات 256M جاهزة · NFR-13 الجلسات إلى القاعدة (`ems_sessions` + معالج + `auto_prepend`) خلف `EMS_SESSION_STORE` · NFR-14 القياس الحي (اتصالات 151 · `SYSTEM_VARIABLES_ADMIN` محجوب فالتوجيهات تُسلَّم جاهزة) · NFR-15 جدولة ليلية. **الحزام:** `tests/nfr_infra_test.php` = **17/17**.

## البوابة ⑥ · UAT — الموجات ㉒㉓㉔㉕ (19/19)

### الموجة ㉒ · التحصين والتحضير — 6/6
بند التحصين الستة (فصل الحسابات محجوب `mysql.*` · setup معطَّل · العدة معزولة · CSRF · **الاسترجاع مختبَر فعلًا** · المصدر الثاني للأسرار) — **مكتمل** · `uat_runs`/`uat_evidence` · الوسم `UAT-2026` · خطة الحجم بتوزيع غير منتظم · **من ينفّذ: مستخدمو الإدارات والفريق يوثّق**. **الحزام:** `tests/uat_hardening_test.php` = **18/18**. الأداة: `tools/uat_hardening_verify.php`.

### الموجة ㉓ · التجربة الوظيفية — 5/5
الأشهر الثلاثة · المحطات (بلاغ سياقي → توجيه من ORG-01 → مسارات متوازية) · حافة منتصف الليل (عمر متسق بعد N-25). **الحزام:** `tests/uat_functional_test.php` = **8/8**.

### الموجة ㉔ · الكسر والإقفال — 4/4
**18+ سيناريو كسر** (الحرّاس السبعة عشر فرادى: 8 never → 403 · 7 break_glass بكسر زجاج · 2 رقابة تعويضية + اعتماد الذات · بلا مدة · بلا نطاق · **⑱ استثناء مفتوح المدة**) · الشواهد الأربعة عشر · حلقات §12.1 الأربع — كلها تُرفض برسائل مفهومة. **الحزام:** `tests/uat_break_test.php` = **23/23**.

### الموجة ㉕ · الحمل والتزامن والقرار — 4/4
تضارب التزامن الستة (مدير حركة واحد بالقاعدة · إذن يُستهلك مرة · الباقي بقيود القاعدة) · المسيّر 20×100 تحت الحمل عبر الطابور · العزل صفر تسرب · التكرار الثلاثي متطابق · **بوابات منع الإطلاق الأربع مجتازة** + تقرير القرار `docs/uat/UAT_decision_report_ar.md`. **الحزام:** `tests/uat_load_decision_test.php` = **11/11**.
