...
$rows00 = [
    ...
    ['بصمة المخطط', substr($base['schema_sha1'], 0, 16), 'لم تتغير بعد مسح الشواهد ✔ (نفس البصمة)', ''],
    ['— الحكم النهائي —', '', '', 'h'],
    ['حالة بوابة الإصدار', 'FAILED', 'release_gate.php رمز خروجه 1 · حواجب P0 مفتوحة', 'bad'],
    ['الجاهزية', 'غيرُ جاهزٍ للإطلاق ولا للعرضِ الخارجي — داخليٌّ بشروط', 'الأسبابُ الحاجبةُ أدناه', 'bad'],
    ...
    ['ترحيلُ الوقائعِ إلى الدفتر', $ebX['fin_events_posted'] . ' / ' . $ebX['financial_events'] . ' = ' . round($ebX['fin_events_posted'] / max(1, $ebX['financial_events']) * 100, 2) . '٪',
        'المروحةُ تنشر ولا تُرحِّل — 5,244 واقعةً في draft وposted_at فارغٌ كلُّه', 'bad'],
...
$r++;
$sh->setCellValue("A$r", 'الأسبابُ الحاجبةُ — كلُّها مقيسةٌ حيًّا في هذه الجولةِ لا منقولةٌ عن تقرير:');
$sh->getStyle("A$r")->getFont()->setBold(true); $r++;
foreach ([
  '① أمنٌ: 7 شاشاتٍ مسجَّلةٍ تُصيَّر لقارئٍ بلا منحة — عقود موردين وموظفين وتقارير (ra05c) — لا تنادي الحارس',
  '② أمنٌ: 293 نموذج POST عاديٍّ بلا رمز CSRF تحت مسارات الإنفاذ — الحاقن المركزي يغطي fetch/XHR لا النماذج',
  '③ ماليٌّ: 5,244 واقعةً ماليةً من 5,255 لم تُرحَّل إلى الدفتر (state=draft · posted_at فارغ) — الدفترُ يُملأ من مسارِ أجورٍ موازٍ لا من المروحة',
  '④ ماليٌّ: 298 مستخلصًا بصفرِ بندٍ في claim_lines، و285 فاتورةً صدرت فوقها — مستنَدٌ ماليٌّ بلا تفصيلٍ يسنده',
  '⑤ تجاريٌّ: 120 عقدًا بـquotation_id فارغٍ كلِّه — سلسلةُ «عرضٌ ← عقد» مقطوعةٌ حيًّا رغم وجودِ العمود',
  '⑥ تكاملٌ: 20 تسليمَ حدثٍ من 21,197 واقعة (0.09٪) وأربعةُ مستهلكين — الناقلُ سجلٌّ لا ناقل',
  '⑦ تشغيليٌّ: 17 مهمةً ميتةً من 20 في الطابور — job_type يحمل نصَّ ملاحظةِ UAT لا رمزَ نوع',
  '⑧ استمراريةٌ: أحدثُ نسخةِ بياناتٍ عمرُها 10 أيامٍ وlog_bin مغلق ⇒ RPO الفعليُّ 10 أيامٍ ولا محضرَ استعادةٍ قطُّ',
  '⑨ حوكمةٌ: ' . $p0open . ' من 13 حاجبَ P0 غيرُ مُغلقٍ بالطبقاتِ الثلاث · وINJ-0219 انحدارٌ حيٌّ · release_gate=FAILED',
] as $line) { $sh->setCellValue("A$r", $line); $sh->mergeCells("A$r:C$r"); $sh->getStyle("A$r")->getAlignment()->setWrapText(true); $sh->getStyle("A$r")->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB($BAD); $r++; }
...
$blockers = [
    ...
    ['B2', '7 شاشات تُصيَّر بلا منحة', 'Security', 'ra05c: قارئ يرى عقود مورد/موظف/تقارير', 'مفتوح — لم يُكتشف سابقًا', 'إضافة enforce_current_page_view_permission لكل منها'],
    ['B3', '293 نموذج POST بلا CSRF', 'Security', 'ra03c تحت 27 مسار إنفاذ', 'مفتوح — الحاقن يغطي fetch/XHR لا النماذج', 'csrf_field() في جسم كل نموذج'],
    ...
    ['B8', 'الوقائعُ الماليةُ لا تُرحَّل إلى الدفتر', 'Financial',
        'ra08: ' . $ebX['fin_events_posted'] . ' من ' . $ebX['financial_events'] . ' لها journal_entry_id · 5,244 في state=draft · posted_at فارغٌ في كلِّ الصفوف',
        'مفتوح — لم يُكتشف سابقًا',
        'تشغيلُ مُرحِّلِ الوقائعِ أو إعلانُ الدفترِ مصدرًا موازيًا صراحةً · اختبارُ قبول: واقعةٌ معتمدةٌ ⇐ قيدٌ متوازنٌ خلال دورةٍ واحدة'],
    ...
    ['B12', 'طابورُ المهامِ ميتٌ بنسبةِ 85٪', 'Operational',
        'ra08: ' . $nfrX['job_queue']['dead'] . ' من ' . $nfrX['job_queue']['rows'] . ' في state=dead · job_type يحمل نصَّ ملاحظةِ UAT لا رمزَ نوع',
        'مفتوح — بقايا جولةِ UAT ماتت',
        'كنسُ بقايا UAT من الجداولِ التشغيلية · وحصرُ job_type بقائمةِ أنواعٍ مسموحةٍ (ENUM أو CHECK)'],
    ['B13', 'لا استعادةَ لنقطةِ زمنٍ ولا نسخةَ بياناتٍ حديثة', 'Continuity',
        'ra08: log_bin=' . ($nfrX['db_settings']['log_bin'] ?? '—') . ' · أحدثُ نسخةِ بياناتٍ عمرُها 10 أيام · 442 لقطةَ بنيةٍ بلا بيانات · لا محضرَ استعادة',
        'مفتوح — لم يُكتشف سابقًا',
        'تفعيلُ log_bin ونسخٌ يوميٌّ آليٌّ ومحضرُ تجربةِ استعادةٍ موقَّع · إعلانُ RPO/RTO'],
];
