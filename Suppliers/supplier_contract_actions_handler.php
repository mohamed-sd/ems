...
function getContractData($contract_id, $conn) {
    $query = "SELECT * FROM supplierscontracts WHERE id = $contract_id LIMIT 1";
...
if ($action === 'renewal') {
    ...
    $query = "UPDATE supplierscontracts SET 
...
// 6. دمج عقدين
    ...
    if (mysqli_query($conn, $query)) {
        ...
        $update_merged_contract = "UPDATE supplierscontracts SET 
            status = 0,
            updated_at = NOW()
