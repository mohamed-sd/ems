 ...
 *
 * INJFIX-SENSITIVE-EXEMPT: هذا السطحُ يمسُّ `users.password` **تحقُّقًا لا بثًّا**.
 *   العمودُ يُقرأ في الاستعلامِ سطرَ 82 ليُمرَّر إلى `password_verify()` سطرَ 97،
 *   ولا يدخل حمولةَ الاستجابةِ في أيِّ مسار (`auth_build_context_payload` لا
 *   يذكره). و`phone` في الحمولةِ من جدولِ `users` — لا `employees.phone`
 *   المسجَّلِ حساسًا — وهو **هاتفُ صاحبِ الرمزِ نفسِه** يُرجَع له هو.
 *   ⇐ فالإعفاءُ مُعلَنٌ بسببِه هنا، لا مسكوتٌ عنه في فاحصٍ يمرُّ صامتًا.
 *   ◆ ولا يُقرأ هذا إذنًا عامًّا للملف: أيُّ بثٍّ جديدٍ لحقلٍ حساسٍ منه يمرُّ
 *     بـ`api_sensitive_value()` كغيرِه.
 *
 * @package EMS\Api
 */
