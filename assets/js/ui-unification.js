(function () {
    ...
    function bootFieldLabels() {
        ...
        for (var i = 0; i < nodes.length; i++) {
            ...
            /* ⑤ قاموسُ الأسماءِ الدلالية — **مُؤلَّفٌ بيدٍ لا مولَّد**
               ◆ الفرقُ جوهريّ: التوليدُ الآليُّ من اسمِ الحقلِ يُنتج «qty_billable»
                 عنوانًا للمستخدم — عيبٌ أسوأُ من الغياب. أما قاموسٌ يُكتب فيه
                 لكلِّ اسمٍ معروفٍ عنوانُه العربيُّ فهو تأليفٌ مرةً واحدةً في
                 موضعٍ واحد.
               ◆ و**المجهولُ يبقى بلا عنوان** عمدًا: لا مدخلةَ ⇒ لا وسمَ وصفيّ،
                 فلا يتسرّب اسمٌ تقنيٌّ إلى قارئِ الشاشة. والدَّينُ يبقى مرئيًّا
                 في البوابةِ بدل أن يُدفن تحت عنوانٍ رديء. */
            if (txt === '') {
                var nm = (el.getAttribute('name') || '').replace(/\[\]$/, '').replace(/\[.*$/, '');
                txt = EMS_FIELD_LABELS[nm] || '';
            }
    ...
    /* ══ قاموسُ عناوينِ الحقولِ الدلالية (AC-U7 · SH-08) ═══════════════════
       كلُّ مدخلةٍ هنا **مكتوبةٌ بيدٍ** بعد النظرِ في سياقِ الحقلِ في شاشاتِه.
       ولا يُضاف اسمٌ لم يُفهم معناه — الغيابُ أصدقُ من عنوانٍ مضلِّل. */
    var EMS_FIELD_LABELS = {
        // الأطرافُ والكيانات
        supplier_id: 'المورد',        employee_id: 'الموظف',      driver_id: 'السائق',
        client_id: 'العميل',          entity_id: 'الكيان',        project_id: 'المشروع',
        contract_id: 'العقد',         site_id: 'الموقع',          equipment: 'المعدة',
        equipment_id: 'المعدة',       capacity_id: 'القدرة',      unit_id: 'الوحدة',
        drivers: 'السائقون',          add_driver_ids: 'سائقون يُضافون',
        // التواريخُ والمُدد
        valid_from: 'ساري من',        valid_to: 'ساري حتى',       end_date: 'تاريخ الانتهاء',
        start_date: 'تاريخ البدء',    expiry_date: 'تاريخ الانتهاء',
        effective_date: 'تاريخ التطبيق', signed_date: 'تاريخ التوقيع',
        received_date: 'تاريخ الاستلام', period: 'الفترة',        q_period: 'الفترة',
        // الحالاتُ والقرارات
        state: 'الحالة',              status: 'الحالة',           new_state: 'الحالة الجديدة',
        to_state: 'الحالة المقصودة',  state_at: 'الحالة عند التاريخ',
        decision: 'القرار',           resolution: 'المآل',        reason: 'السبب',
        rejection_class: 'تصنيف الرفض', hold_kind: 'نوع التعليق',
        // التصنيفاتُ والأنواع
        type: 'النوع',                ttype: 'النوع',             type_id: 'النوع',
        alloc_type: 'نوع التخصيص',    bearer_type: 'الجهة المتحمِّلة',
        scope_type: 'نطاق التطبيق',   protection_type: 'صنف الحماية',
        element_code: 'رمز العنصر',   tax_code: 'الرمز الضريبيّ',
        // الكمياتُ والمستندات
        qty_billable: 'الكمية القابلة للفوترة',
        attachment: 'مرفق',           proof: 'مستند الإثبات',     spec_photo: 'صورة المواصفة',
        // الترشيح
        q_project: 'ترشيح بالمشروع',  rfq: 'طلب عرض السعر',

        // ── الدفعةُ الثانيةُ: ما ظهر بعد تطبيقِ الأولى ────────────────────
        // الأطرافُ والحسابات
        person_id: 'الشخص',           counterparty_id: 'الطرف المقابل',
        financier_entity_id: 'جهة التمويل', operator_employee_id: 'المشغّل',
        account_id: 'الحساب',         acct: 'الحساب',            sup: 'المورد',
        entity_role: 'صفة الكيان',    project: 'المشروع',
        // التواريخ
        effective_from: 'ساري من',    new_expiry: 'تاريخ الانتهاء الجديد',
        cycle_start: 'بداية الدورة',  due_date: 'تاريخ الاستحقاق',
        date: 'التاريخ',
        // التصنيفات
        event_type: 'نوع الواقعة',    meter_type: 'نوع العدّاد',
        category_id: 'الفئة',         child_type_id: 'النوع الفرعيّ',
        auth_type: 'نوع التفويض',     guard_class: 'صنف الحماية',
        fault_department: 'الإدارة المسؤولة عن العطل',
        direction: 'الاتجاه',         currency: 'العملة',
        permission_code: 'رمز الصلاحية', new_oblig: 'الالتزام الجديد',
        enable: 'تفعيل'
    };
