      ...
      if ($stmt) {
        ...
        if ($res&&mysqli_num_rows($res)===1) {
          ...
          if (isset($user['password']) && password_verify($p, $user['password'])) {
              $is_super_admin = isset($user['role']) && strval($user['role']) === '-1';
              $company_id_val = (isset($user['company_id']) && intval($user['company_id']) > 0) ? intval($user['company_id']) : 0;
              ...
              if (!$is_super_admin && $company_id_val > 0 && db_table_has_column($conn, 'admin_companies', 'status')) {
                $companyStatus = 'active';
                $statusStmt = @mysqli_prepare($conn, 'SELECT status FROM admin_companies WHERE id = ? LIMIT 1');
                if ($statusStmt) {
                  mysqli_stmt_bind_param($statusStmt, 'i', $company_id_val);
                  mysqli_stmt_execute($statusStmt);
                  $statusRes = mysqli_stmt_get_result($statusStmt);
                  $statusRow = $statusRes ? mysqli_fetch_assoc($statusRes) : null;
                  mysqli_stmt_close($statusStmt);

                  if ($statusRow && isset($statusRow['status']) && trim($statusRow['status']) !== '') {
                    $companyStatus = strtolower(trim($statusRow['status']));
                  }
                }
            ...
            $_SESSION['user']=['id'=>$user['id'],'name'=>$user['name'],'username'=>$user['username'],
              ...
              'mine_id'=>isset($user['mine_id']) ? $user['mine_id'] : null,'contract_id'=>isset($user['contract_id']) ? $user['contract_id'] : null,
                'company_id'=>$company_id_val > 0 ? $company_id_val : null,
...
.portal-links{margin-top:14px;display:flex;gap:10px;flex-wrap:wrap}
.portal-link{
  flex:1 1 160px;display:flex;align-items:center;justify-content:center;gap:7px;
  border:1px solid var(--bdr);border-radius:10px;padding:10px 12px;
  font-size:.8rem;font-weight:700;color:var(--navy);background:#fff;
  transition:border-color var(--ease),transform var(--ease),box-shadow var(--ease),background var(--ease)
}
.portal-link:hover{
  border-color:rgba(232,184,0,.45);background:rgba(232,184,0,.08);
