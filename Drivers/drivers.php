...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['name'])) {
    ...
    if ($id > 0) {
        ...
        if (mysqli_query($conn, $insert_query)) {
            header("Location: drivers_comprehensive.php?msg=تم+إضافة+المشغل+بنجاح+✅");
            exit;
        } else {
            header("Location: drivers_comprehensive.php?msg=حدث+خطأ+أثناء+الإضافة+❌: " . mysqli_error($conn));
            exit;
        }
    }
}
            ...
            <table id="driversTable" class="display nowrap">
                ...
                <tbody>
                    ...
                    $query = "SELECT d.*, s.name as supplier_name,
                             (SELECT COUNT(*) FROM drivercontracts WHERE driver_id = d.id) as numcontracts
                             FROM drivers d
