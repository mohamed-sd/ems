...
require_once __DIR__ . '/../includes/entity_tabs.php'; echo ems_entity_tabs('contract', 'المخاطر');
...
if (!function_exists('risk_redirect_with_msg')) {
    ...
    {
        ems_gov_flash_redirect('commercial_risks.php', $msg, 'GOV-INFO-200', '');
        exit();
    }
}
...
// رمز CSRF — [ع-0أ] اعتماد الرمز المركزي بدل رمزٍ محلّيٍّ منفصل: الحاقنُ المركزي
// يضع حقل csrf_token برمز الجلسة، وهذا كان يطبع حقلًا ثانيًا بالاسم نفسه وقيمةٍ
// أخرى — وPHP يأخذ الأخير فيفشل الحارس المركزي (403). توحيدُ القيمة يُبقي الفحص
// المحلّي أدناه فعّالًا ويُمرّر الحارس المركزي أيًّا كان الفائز.
$risk_csrf_token = generate_csrf_token();
...
if (!$can_view) {
    ems_gov_flash_redirect('../main/dashboard.php', 'لا توجد صلاحية عرض المخاطر التجارية ❌', 'GOV-PERM-403', '');
    exit();
}
            ...
            <div class="card-body">
                <div class="form-grid">
                    <div id="generated_code_wrapper" class="auto">
                        ...
                        <input type="text" id="generated_risk_code" class="generated-code-field" value="<?php echo risk_e($next_risk_code); ?>" readonly tabindex="-1" title="هذا الكود للعرض فقط، يمكنك نسخه واستخدامه في حقل كود الخطر" />
                    ...
                    <div>
                        ...
                        <input type="text" name="risk_code" id="risk_code" placeholder="مثال: RSK-001" required
                            value="<?php echo risk_e($next_risk_code); ?>"
                            pattern="[A-Za-z0-9_\-]+" />
                    </div>
                    <div>
                ...
                <div class="pu-form-actions">
                    <button type="submit" class="btn-primary"><i class="fas fa-save"></i> <span id="submitBtnText">حفظ الخطر</span></button>
                    <button type="button" id="riskFormCancelBtn" class="btn-secondary"><i class="fas fa-times"></i> إلغاء</button>
                </div>
            </div>
        </div>
    </form>
                ...
                <table id="riskTable" class="display risk-table-nowrap no-datatable" data-state-save="false">
                    <thead>
                        <tr>
                            ...
                            <th class="ems-fn-th" data-fn="1">رقم الخطر</th>
...
<script>
    ...
    function setEditMode() { formTitle.text('تعديل الخطر'); submitBtnText.text('تحديث الخطر'); setGeneratedCodeShown(false); }
