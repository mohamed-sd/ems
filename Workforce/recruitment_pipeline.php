...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    if (isset($_POST['new_vacancy'])) {
        ...
        if ($tt === '') { $msg = 'مسمى الشاغر إلزامي (422)'; }
        else {
        ...
        if ($r && ($a = mysqli_fetch_assoc($r))) {
            ...
            if ($idx === false || $idx >= count($ORDER) - 1) { $msg = 'لا خطوة تالية — مثبت أو خارج (409)'; }
            else {
                $next = $ORDER[$idx + 1];
                ...
                if ($next === 'onboarded' && intval($_POST['employee_id'] ?? 0) <= 0) {
                    $msg = 'المباشرة تستلزم ربط الموظف المعين (employee_id) — 422';
                } else {
                    $extra = '';
        ...
        if ($why === '') { $msg = 'سبب الرفض إلزامي (422)'; }
        else {
            $r = mysqli_query($conn, "SELECT stage FROM rec_applications WHERE app_id = $aid AND company_id = $company_id");
...
?>
  ...
  <div class="rec-forms-row">
    <form method="post" class="ems-form rec-inline-form">
      ...
      <div><label for="emsf_1785_5c43b">① طلب شاغر جديد</label><input type="text" name="title_text" class="form-control" placeholder="المسمى" required id="emsf_1785_5c43b"></div>
    ...
    <form method="post" class="ems-form rec-inline-form">
      ...
      <div><label for="emsf_1787_75453">③ سيرة لمتقدم</label>
      ...
      <div><label for="emsf_1790_cebd8">مرجع السيرة</label><input type="text" name="cv_ref" class="form-control rec-w-130" id="emsf_1790_cebd8"></div>
      <button class="btn btn-primary">سجل</button>
    </form>
  </div>
