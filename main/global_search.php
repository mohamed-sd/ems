 ...
 *
 * ◆ **INJ-FIX-01 · GAP-30 — المرآةُ كانت تكذب**: كان السطرُ الأخيرُ
 *   `return ($p['id'] === null) || !empty($p['can_view']);` — أي أن **الشاشةَ
 *   غيرَ المسجَّلةِ في `modules` تُفتح للجميع**، وتعليقُه يبرّر ذلك بأنها «تبقى
 *   مفتوحةً كما تبقى هناك».
 * ◆ **ولم يعد ذلك صحيحًا**: حارسُ التنفيذِ شُدِّد (FIX-01 · RF-01) فصار الحلُّ
 *   الفاشلُ إلى موديولٍ **منعًا مطلقًا** بـ`GOV-PERM-404-MODULE`. فبقيت المرآةُ
 *   على حالِها **تصف حارسًا تغيَّر**. والنتيجةُ عيبان معًا:
 *     ① **تسريبُ وجود**: البحثُ يُظهر صفوفَ شاشةٍ غيرِ مسجَّلةٍ لمن لا يملكها.
 *     ② **ورابطٌ يقود إلى منع**: الوجهةُ تردُّه الحارسُ بعدَ النقر.
 * ◆ **والقلبُ إلى فشلٍ مغلقٍ بلا فقد**: أوجهةُ البحثِ الأربعَ عشرةَ كلُّها
 *   مسجَّلةٌ في `modules` (مقيسٌ) — فالتشديدُ لا يُخفي نتيجةً واحدةً قائمة،
 *   ويمنع ما سيُضاف غدًا بلا تسجيل.
 * ◆ **والإعفاءُ يبقى من `can_view` لا من التسجيل**: البلاغاتُ كالمراسلات
 *   («أيُّ مستخدمٍ مسجَّلٍ يصلها») — لكنَّ شاشةً بلا صفٍّ في السجلِّ لا تُعفى،
 *   وإلا صار الإعفاءُ بابًا خلفيًّا للقاعدةِ نفسِها.
 */
function gs_can_open($conn, $screen)
{
    ...
    if ($p['id'] === null) { return false; }          // غيرُ مسجَّلةٍ ⇒ لا تُفتح
    if (in_array($screen, $exempt, true)) { return true; }
    return !empty($p['can_view']);
}
