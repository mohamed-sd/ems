...
/* ── منطقةُ الوصولِ السريعِ من سجلِّ الدورِ (شاشة «تحكم الروابط») ─────────────
   ◆ **العطبُ الذي يرفعه**: المصدرُ الإرثيُّ أعلاه رايتُه على **صفِّ الوحدةِ**
     (`modules.is_quick`)، والوحدةُ يقرؤها الدورُ المالكُ **وأبناؤه** — فرفعُ
     رايةٍ لدورٍ يرفعها لغيرِه، ولا سبيلَ لدورٍ أن يضبط بلاطاتِه وحدَه. وحبّةُ
     الملاحةِ في هذه المعماريّةِ صارت `nav_items` (صفٌّ لكلِّ دورٍ ومسار).
   ◆ **والانتقالُ لا يغيّر ما يُرى**: `nav_items.is_quick` بُذرت من هذه الكتلةِ
     نفسِها (الترحيل 2028_05_28) — فاللقطةُ قبلَ التحويلِ وبعدَه واحدة.
   ◆ **وللسوبرِ ومن خارجِ الملاحةِ الموحّدةِ يبقى الإرثُ حرفًا**: لا صفوفَ
     `nav_items` لدورِه (-1)، فالردُّ إلى القديمِ صيانةٌ لا تراجع. */
/* ◆ **والمصدرُ الموحَّدُ يُحمَّل هنا صراحةً**: `insidebar.php` يُضمَّن بعدَ هذه
     الكتلةِ بأربعِ مئةِ سطرٍ، فسؤالُ `function_exists` قبلَه يرجع «لا» دائمًا —
     ويسلك النداءُ الإرثَ صامتًا فيصير المربَّعُ يُوضَع ولا يُقرأ. (مقيسٌ: بلاطةٌ
     مؤشَّرةٌ لم تظهر حتى حُمِّل الملفُّ هنا.) */
require_once dirname(__FILE__) . '/../includes/unified_nav.php';
$dashQuickRows = null;
if (function_exists('unifiedNavEnabled') && unifiedNavEnabled($role) && function_exists('getUnifiedQuickItems')) {
  $dashQuickRows = [];
  foreach (getUnifiedQuickItems($conn, $role) as $q) {
    /* المسارُ يُخزَّن بصيغتين («x.php» و«../x.php») — تُسوّى قبلَ البناء. */
    $qr = preg_replace('~^(\.\./)+~', '', ltrim((string) $q['route'], '/'));
    if ($qr === '') { continue; }
    $dashQuickRows[] = [
      '../' . $qr,
      (string) $q['label_ar'],
      !empty($q['icon']) ? (string) $q['icon'] : 'fa fa-link'
    ];
  }
  $links = $dashQuickRows;
}
...
<!-- ══ DASH-V2 · END ══════════════════════════════════════════════════════ -->
  ...
  <div class="shot-body">
    ...
    /* ◆ **وصفُّ الدورِ يغلب تجميعَ المجموعات**: متى كان للدورِ سجلُّ ملاحةٍ
         موحَّدٌ صار ما يضعه المالكُ في «تحكم الروابط» هو الحاكمَ — وإلّا لظلَّ
         مربَّعُ الوصولِ السريعِ يُوضَع ولا يُقرأ. وتجميعُ المجموعاتِ يبقى لمن
         لا سجلَّ موحَّدَ له. */
    if ($dashQuickRows === null && !empty($dashGroupTiles)) {
    ...
    <?php if ($dashQuickRows === null && !empty($dashGroupTiles)): ?>
    <script>
