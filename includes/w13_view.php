<?php
/**
 * includes/w13_view.php — أدواتُ عرضٍ لأسطحِ المرحلةِ الثالثةَ عشرة (RPR-W13)
 * ═══════════════════════════════════════════════════════════════════════════
...
if (!function_exists('ems_w13_ar')) {
    ...
    function w13_perms($conn, $code, $is_super)
    {
    ...
    function w13_gate($is_super = false)
    {
        $gate = ems_tenant_db();
        return $is_super ? $gate->forAllTenants('w13 people super view') : $gate;
    }

    /** قراءةٌ آمنةٌ لصفوفِ جدولٍ عبرَ البوّابة — والخطأُ يُسجَّل ولا يُبتلَع صامتًا */
    function w13_rows($is_super, $table, array $opt = array())
    {
        try { return w13_gate($is_super)->select($table, $opt); }
        catch (\Throwable $t) { error_log('w13_rows ' . $table . ': ' . $t->getMessage()); return array(); }
    }

    /** عدُّ الصفوفِ التي يساوي عمودُها قيمةً — بالمقارنةِ النصّيّةِ لا بالنوع */
    function ems_w13_count(array $rows, $col, $val)
    {
        $n = 0;
        foreach ($rows as $r) {
    ...
    function ems_w13_distinct(array $rows, $col)
    {
        $seen = array();
        foreach ($rows as $r) {
    ...
    /** صفوفٌ عمودُها خالٍ */
    function ems_w13_empty(array $rows, $col)
    {
        $n = 0;
        foreach ($rows as $r) {
    ...
    /** صفوفٌ عمودُها مملوء */
    function ems_w13_filled(array $rows, $col) { return count($rows) - ems_w13_empty($rows, $col); }

    /** مجموعُ عمودٍ عشريّ */
    function ems_w13_sumf(array $rows, $col)
    {
        $s = 0.0;
    ...
    function ems_w13_num($v) { return number_format((float) $v, 2); }

    /** نصٌّ للعرضِ — والعدمُ شرطةٌ لا فراغٌ يلتبس بعمودٍ مفقود */
    function ems_w13_txt($v)
    {
        $v = (string) $v;
