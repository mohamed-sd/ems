...
if (!function_exists('navarch_authorized_routes')) {
    ...
    function navarch_authorized_routes($conn, $roleId)
    {
        ...
        /* ⛔ **والمقاطعةُ تسري على غيرِ المغطَّى أيضًا** — وهذا تمامُ ما نُصَّ عليه
             أعلاه: كان `if (!empty($st['covered']))` يترك `$tpl = null` لمن لا
             قالبَ له، **فلا يُقاطَع تصريحُه أصلًا** ويبقى على `role_permissions`
             وحدَه — بينما يردُّه `get_module_permissions` بـ
             `no_profile_no_fallback`. فالسجلّانِ اللذانِ حُذِّر من افتراقِهما
             **مفترقانِ فعلًا عند غيرِ المغطَّى**: قِيس **454 مسارًا مصرَّحًا يردُّه
             الحارسُ عند النقر** في خمسِ جلساتٍ حيّة (136 · 136 · 108 · 44 · 30).
           ◆ **والصفرُ حكمٌ لا غياب**: `allowed` الفارغةُ تعني «لا شاشةَ» — وهي
             عينُ حكمِ الحارسِ لغيرِ المغطَّى، لا تعذُّرَ قراءة.
           ⚠ **والسوبر `-1` مستثنًى**: `ems_template_nav_state` تردُّ له
             `covered=false` بشرطِها الأوّل، فمقاطعتُه تُفرِغ سايدبارَه.
           ◆ **وشرطُ تطابقِ الدورِ باقٍ كما هو**: حبّةُ القالبِ مستخدمٌ لا دور،
             فلا يُقاطَع تصييرٌ يُطلَب بدورٍ لا يخصُّ صاحبَ الجلسة. */
        $tpl = null;
        if (function_exists('ems_template_nav_state')
            && isset($_SESSION['user']['role'])
            && (int) $_SESSION['user']['role'] === $roleId
            && intval($_SESSION['user']['id'] ?? 0) > 0
            && strval($_SESSION['user']['role']) !== '-1') {
        ...
        /* ══ ⛔ **و`nav_items` ليست مصدرَ تصريحٍ للمغطَّى — بل خريطةَ مسار** ═════
           **العطبُ المقيس**: الجملةُ أعلاه تشترط **صفًّا في `nav_items` لهذا
             الدور** قبلَ أن يمرَّ المسار. فشاشةٌ **موضوعةٌ PRIMARY في مساحةِ
             المستخدمِ الحاكمة** ويسمح بها **قالبُه** ويفتحها **الحارسُ** — لا
             تُصيَّر له إن لم يكن لها صفٌّ في السجلِّ القديم. وهذا **مصدرُ تصريحٍ
             ثالثٌ** يضيّق ما أذِن به الحارس.
           ◆ **مقيسٌ حيًّا**: 119 حالةً في 11 دورًا — `Operations/ops_room` و
             `Suppliers/supplier_offers` و`Finance/depr_run` وغيرُها: كلُّها
             `PRIMARY` في مساحةِ صاحبِها، مسموحةٌ بالقالب، **بلا صفِّ `nav_items`**.
           ◆ **ولا تحتاج بيانات**: المُصيِّرُ يأخذ **التسميةَ والمجموعةَ والترتيبَ
             من الموضعِ الحاكمِ نفسِه** (372 صفَّ PRIMARY نشطًا · صفرُ نقصٍ في
             الحقولِ الثلاثة) — فلا شيءَ من `nav_items` يلزمه سوى قرارِ التصريح.
           ⇒ **فالمغطَّى تصريحُه قالبُه حرفًا**: `Rendered = Placements ∩ Authorized`
             و«المُصرَّحُ به» هو ما يفتحه الحارسُ لا ما سجّله جدولٌ موازٍ.
           ⚠ **وغيرُ المغطَّى وسطرُ الأوامرِ والسوبر على المسارِ القائمِ حرفًا**
             (`$tpl === null`) — فلا يتغيّر لهم سطر.
           ◆ **وبندٌ بلا رمزِ فحصٍ يبقى مارًّا**: حارسُه في وجهتِه لا في القائمة،
             فيُضمُّ من `nav_items` كما كان. */
        if ($tpl !== null) {
            foreach ($tpl as $ref => $_ok) {
                $norm = navarch_norm_route((string) $ref);
                if ($norm === '') { continue; }
                if ($scoped) {
                    $wsq = isset($wsMap[strtolower($norm)]) ? $wsMap[strtolower($norm)] : null;
                    if ($wsq !== null && !ems_workspace_allowed($conn, $wsq)) { continue; }
                }
                $out[$norm] = true;
            }
        }
