...
<?php
/* ── شريطُ تبويباتِ الملفِّ — تحتَ الشريطِ الأصفرِ مباشرةً (قرارُ المالك) ──
   ◆ كان يُطبع **قبل** `<div class="main">` في 24 شاشة، فيخرج من غلافِ الشاشةِ
     ويظهر بجانبِها لا داخلَها. وموضعُه هنا: بعد `.main_head` (الشريطِ الأصفرِ
     الذي يحمل زرَّ «عن الشاشة») وقبلَ شريطِ رحلةِ الكِيان.
   ◆ والصرفُ من طابورٍ لا من طباعةٍ مباشرة، لأنّ الشاشةَ تُضمِّن الشريطَ قبل
     أن تفتح `.main` — التفصيلُ في `includes/file_tabs_kit.php`. */
if (function_exists('ems_file_tabs_flush')) { echo ems_file_tabs_flush(); }

/* ── ثم شريطُ رحلةِ الكِيان — تحتَ تبويباتِ الملفِّ لا فوقَها ────────────────
   الترتيبُ من العامِّ إلى الخاص: رأسُ الشاشةِ ← أيُّ ملفٍّ نحن فيه ← أينَ نحن
   من رحلتِه ← المحتوى. و`ems_entity_tabs` لا تصفُّ إلا إن نُودِيت قبلَ هذا
   السطر؛ وما نُودِي بعدَه تطبعه شاشتُه بنفسِها في موضعِها الصحيحِ أصلًا. */
if (function_exists('ems_entity_tabs_flush')) { echo ems_entity_tabs_flush(); }

/* ويُرفع العلَمُ **بعد** الصرفِ لا قبلَه: فنداءٌ يقع في هذا السطرِ نفسِه من
   شاشةٍ تُضمِّن الرأسَ مرتين لا يُحسب «بعد الرأس» خطأً. */
$GLOBALS['__ems_head_printed'] = true;
