...
if (!function_exists('ems_w14_grid')) {
    ...
    /**
     * صفوفُ جدولِ الدليلِ عبرَ بوّابةِ العزل — للأسطحِ التي تعرض جدولَ ورقتِها.
     * ◆ **ولا استعلامَ في السطح**: البوّابةُ تحقن الكيانَ، وسقّاطةُ `GAP-29`
     *   ترصد نصَّ الاستعلامِ على جدولِ مستأجِرٍ في شاشةٍ حيّة.
     * ◆ **والعطبُ يُسجَّل ولا يُبتلَع**: جدولٌ غيرُ مسجَّلٍ في سجلِّ البوّابةِ
     *   يرمي، فيُكتب في سجلِّ الأخطاءِ وتُعاد مصفوفةٌ فارغةٌ يراها المستخدمُ
     *   نصَّ فراغٍ لا صفحةً بيضاء.
     */
    function ems_w14_guide_rows($table, $limit = 500, $orderBy = 'id DESC')
    {
        try {
            $opt = array('limit' => (int) $limit);
            if ($orderBy !== '') { $opt['orderBy'] = $orderBy; }
            return ems_tenant_db()->select($table, $opt);
        } catch (\Throwable $t) {
            error_log('ems_w14_guide_rows ' . $table . ': ' . $t->getMessage());
            return array();
        }
    }
