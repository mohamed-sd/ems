...
function getUnifiedNavItems($conn, $roleId) {
    ...
    /* ── RPR-03 §٦ — طبقةُ قوالبِ GOV-AUTH-01 في قرارِ الظهورِ نفسِه ─────────
       ...
       ⛔ **والترشيحُ يسري على غيرِ المغطَّى أيضًا** — وهذا هو العطبُ المقيس:
         كان الشرطُ `if ($tpl['covered'])` يُمرِّرُ **غيرَ المغطَّى بلا ترشيحٍ
         إلى المنحِ القائم**، بينما `get_module_permissions` تردُّه بـ
         `no_profile_no_fallback` — أي أنَّ القائمةَ تُبنى من `role_permissions`
         والبابَ يُحكَم بـ`gov_profile_items`: **حبّتان مختلفتان لسؤالٍ واحد**
         (الدورُ مقابلَ المستخدم). فقِيس **334 رابطًا يُعرَض ويردُّه الحارسُ عند
         النقر** في خمسِ جلساتٍ حيّة — منها ثلاثُ قوائمَ ميتةٍ بالكامل (90 · 76 ·
         48 بندًا، صفرُ رابطٍ يفتح).
       ◆ **والمنطقُ يصحُّ من تلقاءِ نفسِه بلا شرط**: غيرُ المغطَّى `allowed`
         عندَه فارغةٌ، فيسقط كلُّ ما يحمل رمزَ فحصٍ — وهو **عينُ حكمِ الحارس**.
         فالقائمةُ تصير إسقاطًا لقرارِ الحارسِ لا مصدرًا ثانيًا يوازيه.
       ⚠ **وفخّانِ مقيسان يمنعان جعلَه بلا شرطٍ إطلاقًا**:
         ① **السوبر `-1`**: `ems_template_nav_state` ترجع له `covered=false`
            (شرطُها الأوّل)، فترشيحٌ بلا استثناءٍ **يُفرِغ قائمتَه بالكامل**.
            وصفرُ حسابٍ يحمله اليوم — عيبٌ نائمٌ لا معدوم.
         ② **سطرُ الأوامر**: `$uid <= 0` ⇒ `covered=false` كذلك، و
            `nav_doors_integrity_test` و`nav_landing_anchor_test` تناديان هذه
            الدالّةَ **بلا جلسة** — فكانتا سترجعان فارغًا وتُخرِجان أخضرَ كاذبًا
            على صفرٍ مقيس. ⇐ الترشيحُ لجلسةٍ حيّةٍ وحدَها. */
    $tpl        = unifiedNavTemplateState($conn);
    $__navUid   = intval($_SESSION['user']['id'] ?? 0);
    $__navSuper = strval($_SESSION['user']['role'] ?? '') === '-1';
    if ($__navUid > 0 && !$__navSuper) {
...
function printEmsTenGroupNav($conn, $items, $uxMap, $uxCurMap, $basePrefix, $badges, $afterHome = '') {
    ...
    if (isset($GLOBALS['__uxui_cur_role'])) {
        ...
        /* ⛔ **ويسري على غيرِ المغطَّى أيضًا** — النسخةُ الرابعةُ من العطبِ نفسِه:
             `if ($__tpl2['covered'])` كان يترك بنودَ غيرِ المغطَّى **بلا وسمِ
             منعٍ**، فتعيد حلقةُ الاصطناعِ خلقَها من السجلّ لمن يردُّه الحارسُ
             كليًّا. وهذه الدالّةُ خلفَ فرعِ `navarch` فلا تُبلَغ اليوم — **عيبٌ
             نائمٌ لا معدوم**، ويُسدُّ مع إخوتِه لا بعدَ أن يستيقظ.
           ⚠ والسوبرُ وسطرُ الأوامرِ مستثنيانِ كما في المُصيِّرَين. */
        $__tpl2  = unifiedNavTemplateState($conn);
        $__uid2  = intval($_SESSION['user']['id'] ?? 0);
        $__sup2  = strval($_SESSION['user']['role'] ?? '') === '-1';
        if ($__uid2 > 0 && !$__sup2) {
