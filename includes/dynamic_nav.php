...
function getDynamicNavLinks($conn, $roleId) {
    ...
    /* ══ ⛔ **والملكيّةُ ليست صلاحيّة** ═══════════════════════════════════════
       العطبُ البنيويُّ المقيس: هذه الدالّةُ تبني السايدبارَ من `owner_role_id`
       **وحدَه** — أي «أيُّ إدارةٍ تملك الشاشة» — **بلا فحصِ صلاحيّةٍ إطلاقًا**:
       لا `role_permissions` ولا طبقةَ القوالب. فكلُّ شاشةٍ تملكها إدارةُ الدورِ
       (أو دورُه الأب) تُعرَض، فتحها الحارسُ أو ردَّها.
       ◆ **وهي أشدُّ من إخوتِها الأربع**: تلك كانت تُرشِّح بالسجلِّ الخطأ، وهذه
         لا تُرشِّح أصلًا — فتصيب **المغطَّى بقالبٍ** أيضًا، لا غيرَ المغطَّى وحدَه.
       ◆ **والمقيسُ حيًّا**: مستخدما الدورَين 34 و35 (المسارُ الإرثيُّ وحدَه يبلغ
         هذه الدالّةَ تصييرًا) — **58 رابطًا لكلٍّ منهما، 39 يردُّها الحارسُ**:
         «أمر الدفع والتنفيذ» · «سجل المستفيدين» · «اللوحة التنفيذية» وغيرُها.
       ◆ **والترشيحُ بالحالةِ المجمَّعةِ لا بنداءٍ لكلِّ رابط**: استعلامٌ واحدٌ
         مخبَّأٌ (`ems_template_nav_state`) — و`m.code` هو عينُ مفتاحِ القالب.
       ⚠ **والسوبرُ وسطرُ الأوامرِ وتصييرُ دورٍ غيرِ دورِ الجلسةِ مستثناةٌ** — كما
         في `navarch_renderer` حرفًا: حبّةُ القالبِ مستخدمٌ لا دور. */
    $tplAllowed = null;
    if (function_exists('ems_template_nav_state')
        ...
        $__st = ems_template_nav_state($conn);
        $tplAllowed = $__st['allowed'];
    }
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            if ($tplAllowed !== null) {
                ...
                /* بلا كودٍ يحلّ: حارسُه في وجهتِه فلا يُقاطَع هنا. */
                if ($code !== '' && !isset($tplAllowed[$code])) { continue; }
            }
            $links[] = $row;
        }
    }
