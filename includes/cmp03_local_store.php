...
if (!function_exists('cmp03_store_raw')) {
    /**
     * صفوفُ الشاشةِ **بأعمدتِها الأصليّة** لا بحمولةٍ مُعاد تسميتُها.
     * ═══════════════════════════════════════════════════════════════════════
     * ◆ **الحاجزُ الذي ترفعه** — `GOV_UI_EXEC` §11 يوجب أن تحمل الشاشةُ أسماءَ
     *   حقولِ ورقةِ الدليل، و`cmp03_store_rows` تعيد الحمولةَ **مفاتيحُها
     *   التسمياتُ القديمةُ** المولَّدةُ من الشاشة. فلو غُيِّرت التسمياتُ في
     *   الشاشةِ وحدَها لَانقطع مفتاحُ الخليّةِ عن قيمتِه ⇒ «—» في كلِّ صفّ،
     *   وهو بعينِه عطبُ [[declared-column-not-built]] بيدٍ جديدة.
     * ◆ ⇒ فالعرضُ الجديدُ يقرأ **العمودَ** لا التسمية، والخريطةُ في الشاشةِ
     *   تربط اسمَ الورقةِ بالعمودِ مباشرةً. **والكتابةُ لم تُمَسّ**: نموذجُ
     *   الإضافةِ ما زال يمرُّ بـ`cmp03_store_insert` بخريطةِ السجلِّ نفسِها.
     * ⛔ ولا استعلامَ في الشاشة: العزلُ بالكيانِ هنا كما في `cmp03_store_rows`.
     */
    function cmp03_store_raw(mysqli $conn, $canonical, $companyId, $limit = 500)
    {
        $reg = cmp03_registry();
        if (!isset($reg[$canonical])) { return array(); }
        $table = $reg[$canonical]['table'];
