...
if (!function_exists('ems_entity_tabs_for')) {
    /**
     * يُصيِّر الشريطَ **بالمسارِ لا بالاسم**.
     * ═══════════════════════════════════════════════════════════════════════
     * ⚠ **ولمَ وُلدت هذه**: `ems_entity_tabs($ent, $active)` تقارن `$active`
     *   **باسمِ التبويبِ المشكول** — فتمريرُه من الشاشةِ يضع نصًّا مشكولًا في
     *   ملفِّ شاشةٍ حيّة، **وهو ما نزعَته الموجةُ السادسةُ من أكثرَ من ثمانمئةِ
     *   صفحة**. وقد أعدتُه أنا في ثلاثةِ أسطرٍ فردَّني `W6-19` بالاسم.
     *
     * ◆ **والأعمقُ من التشكيل**: تمريرُ الاسمِ يجعله **يسكن موضعَين** —
     *   السجلَّ والشاشة — **فتغييرُ أحدِهما يُطفئ التبويبَ صامتًا**. والمسارُ
     *   مفتاحٌ واحدٌ لا يتفرَّق، **والاسمُ يبقى في السجلِّ وحدَه**.
     */
    function ems_entity_tabs_for(string $route): string
    {
        $rt = strtolower(ltrim($route, '/'));
        foreach (ems_entity_tabs_registry() as $ent => $d) {
            foreach ((isset($d['tabs']) ? $d['tabs'] : array()) as $name => $r) {
                if ($r !== '' && strtolower($r) === $rt) { return ems_entity_tabs($ent, $name); }
            }
        }
        return '';   /* مسارٌ غيرُ مسجَّلٍ في §8-2 — لا شريطَ يُخترَع له */
    }
}
