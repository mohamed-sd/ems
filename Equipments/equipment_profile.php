...
/* ⛔ **حارسُ العرضِ — والتسجيلُ لا يحرس** (M-14 · BR-GOV-01): كانت الشاشةُ
     مسجَّلةً في `modules` وممنوحةً لستَّ عشرةَ دورًا، **ولا تنادي حارسًا**:
     جلسةٌ ونطاقُ شركةٍ فقط — فيفتحها كلُّ مستخدمٍ مسجَّلِ الدخولِ في شركتِه.
     والحارسُ نداءٌ لا صفة.
   ◆ **وهويّتُه هويّةُ شاشتِه**: `equipments_fleet` أدناه ليست هويّةَ البابِ بل
     صلاحيةُ **فعلِ** اعتمادِ الكرتِ («صلاحية اعتماد الكرت = صلاحية تعديل
     المعدات») — فتبقى للكتابةِ وحدَها.
   ◆ **ولا يُسقِط الحارسُ من كان يفتحه**: هجرةُ 2028_05_10 بذرت الكرتَ لكلِّ
     قالبٍ يملك القائمةَ التي تقود إليه بالنقر — فالإغلاقُ **حكمٌ على مباحٍ**
     لا سحبُ منحة. ولا استثناءَ للسوبر: `check_page_permissions` تحكمه. */
$__view = check_page_permissions($conn, 'Equipments/equipment_profile.php');
if (empty($__view['can_view'])) {
    ems_gov_flash_redirect('equipments.php', 'لا توجد صلاحية عرض كرت المعدة', 'GOV-PERM-403', '');
    exit();
}
