...
<div class="main ems-unified-page-shell">
    ...
    ems_screen_about('شبكةُ (معدة × وردية × مشغّل) ليومٍ واحد — بديلُ الشاشتين المنفصلتين. '
        . 'التعارض يفحص في الخادم بحالاته الثلاث: مشغل في معدتين · معدة بلا مشغل · '
        . 'رخصة منتهية — ويظهر قبل أي اعتماد. لا أثر ماليا: تخطيط يسبق التنفيذ.',
        array('اختر اليوم', 'اقرأ شارات التعارض الحمراء', 'عالج التوزيع من شاشة الإدخال'));
    // UXW-01 ⑨: حالاتُ الشاشةِ الدنيا (تحميل · فراغ · خطأ) — مخفيةٌ افتراضيًا
    echo ems_states_bundle('لا توزيع معدات على ورديات هذا اليوم', 'اختر يوما آخر أو افتح إدخال الوحدات لتكليف المشغلين');
    ?>
    ...
    <div class="card dsp-conflict-card"><div class="card-header"><h5>
        ...
        تعارضات اليوم (<?php echo count($conflicts); ?>) — **فحص خادمي شامل**</h5></div>
    <div class="card-body">
    ...
    <?php else: ?>
    <div class="alert alert-success">صفر تعارض في توزيع هذا اليوم ✅</div>
    <?php endif; ?>

    <div class="card"><div class="card-header"><h5><i class="fa fa-table-cells"></i>
        الشبكة — <?php echo count($grid); ?> معدة عاملة في <?php echo htmlspecialchars($date); ?></h5></div>
    <div class="card-body">
        <?php if (!$grid): ems_state_empty('لا توزيع لهذا اليوم', 'افتح إدخال الوحدات', 'units.php'); else: ?>
        <div class="table-container"><table class="alltables display nowrap dsp-table-full" data-no-dt="1">
            <thead><tr><th>الوردية</th><th>☀ نهارية</th><th>🌙 ليلية</th>
              ...
              <th class="ems-fn-th" data-fn="1">كلفه</th>
                ...
                <tr>
                    ...
                    <td><?php if ($cell === null): ?>
                        ...
                        <?php elseif ($cell['operator_id'] === null): ?>
                            <span class="badge badge-danger">⚠ بلا مشغل</span>
                        <?php else: ?>
