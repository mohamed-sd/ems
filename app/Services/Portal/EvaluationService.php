...
class EvaluationService
{
    ...
    public static function selfSave($conn, $gate, $companyId, $capacityId, $from, $to, array $scores, $actor)
    {
        ...
        if ($ev && !in_array((string) $ev['state'], array('SelfDraft'), true)) {
            $out['code'] = 423;
            $out['reason'] = 'الذاتيُّ أُقفل (' . $ev['state'] . ') — لا تعديلَ بعد الإقفال';
            return $out;
        }
        try {
            if ($ev) {
                                     ...
                                     array $scores, $comment, $mgrAccount)
    {
        ...
        if (!$ev) { $out['code'] = 404; $out['reason'] = 'التقييمُ غيرُ موجود'; return $out; }

        // U6: المديرُ يفتح قبل إقفال الموظف ⇒ 423
        if (!in_array((string) $ev['state'], array('SelfClosed', 'MgrDraft'), true)) {
            $out['code'] = 423;
            $out['reason'] = 'تقييمُ المدير **لا يُفتح قبل إقفال الذاتي** (حالُه: '
                           . $ev['state'] . ') — منعًا للتأثير (U6)';
            return $out;
        }
        if ((int) $ev['version'] !== (int) $expectedVersion) {
            $out['code'] = 409; $out['reason'] = 'نسخةٌ متقادمة — أعد القراءة'; return $out;
        }
        ...
        if ($maxGap >= 2 && trim((string) $comment) === '') {
            $out['code'] = 422;
            $out['reason'] = 'فارقٌ ' . $maxGap . ' ≥ درجتين **يوجب تعليقًا** (422 بدونه) — USR-01 §7-②';
            return $out;
        }
    ...
    public static function discuss($conn, $gate, $companyId, $evalId, $expectedVersion, $notes, $actor)
    {
        if (trim((string) $notes) === '') {
            return array('ok' => false, 'code' => 422,
                'reason' => 'المناقشةُ جلسةٌ تُسجَّل — نقاطُ الاتفاق والاختلاف إلزامية');
        }
    ...
    {
        $ev = self::head($gate, $evalId);
        if (!$ev) { return array('ok' => false, 'code' => 404, 'reason' => 'غيرُ موجود'); }
        ...
        if ($score <= 0) {
            return array('ok' => false, 'code' => 422, 'reason' => 'الدرجةُ النهائية بأوزان المحاور — لا صفرًا');
        }
    ...
    public static function issueCertificate($conn, $gate, $companyId, $snapId, $evalId, $actor)
    {
        ...
        if (!$ev || (string) $ev['state'] !== 'Approved') {
            $out['code'] = 422;
            $out['reason'] = '**لا شهادةَ لتقييمٍ لم يُعتمد** (حالُه: ' . ($ev ? $ev['state'] : 'غيرُ موجود') . ') — U7';
            return $out;
        }
        if ((int) $ev['capacity_id'] !== (int) $snap['capacity_id']
            ...
            $out['code'] = 422;
            $out['reason'] = 'اللقطةُ والتقييمُ لفترتين أو صفتين مختلفتين — لا تلفيقَ شهادةٍ من مصدرين';
            return $out;
        }
    ...
    private static function transition($gate, $evalId, $expectedVersion, $fromState, array $set)
    {
        ...
        if ((string) $ev['state'] !== $fromState) {
            return array('ok' => false, 'code' => 423,
                'reason' => 'الانتقالُ من «' . $fromState . '» والحالُ «' . $ev['state'] . '»');
        }
        if ((int) $ev['version'] !== (int) $expectedVersion) {
            return array('ok' => false, 'code' => 409, 'reason' => 'نسخةٌ متقادمة — أعد القراءة ثم أعد الفعل');
        }
