...
class SigmaGuard
{
    ...
    public static function check($gate, $oblId, $targetState = null)
    {
        ...
        $obls = $gate->scopedQuery(array('scope' => array('c' => 'contract_commitments')),
            ...
            array((int) $oblId));
        if (!$obls) { $out['code'] = 404; $out['reason'] = 'الالتزامُ غيرُ موجودٍ في نطاقك'; return $out; }
        $obl = $obls[0];
        if ($obl['primary_units_contracted'] === null) {
            $out['code'] = 422;
            $out['reason'] = 'الالتزامُ بلا عددٍ متعاقدٍ للأساسية — لا Σ بلا مستهدف';
            return $out;
        }
        $target = (int) $obl['primary_units_contracted'];
        $state = $targetState !== null ? (string) $targetState : (string) $obl['plan_state'];
        if (!in_array($state, self::STATES, true)) {
            $out['code'] = 422; $out['reason'] = 'حالةُ خطةٍ خارج الأربع'; return $out;
        }
        ...
        if ($sigma > $target) {
            ...
            $out['reason'] = 'Σ الأساسي (' . $sigma . ') يتجاوز المتعاقدَ (' . $target . ') بفارق '
                           . ($sigma - $target) . ' — لا تجاوزَ إلا بملحقٍ يرفع الالتزامَ أو احتياطيٍّ لا يُحتسب (C1)';
            return $out;
        }
        ...
        if (($state === 'submitted' || $state === 'approved') && $sigma < $target) {
            ...
            if ($exc === '') {
                ...
                $out['reason'] = 'Σ الأساسي (' . $sigma . ') دون المتعاقد (' . $target . ') بفجوة '
                               . ($target - $sigma) . ' — الاعتمادُ يُرفض إلا بقرار استثناءٍ موقَّع (C16)';
                return $out;
            }
            $out['ok'] = true; $out['code'] = 200;
            $out['reason'] = 'اعتمادٌ بفجوةٍ ظاهرة (' . ($target - $sigma) . ') بقرار استثناءٍ: ' . $exc;
            return $out;
        }
        ...
        $out['reason'] = $sigma === $target
            ...
            : 'خطةٌ ناقصةٌ محفوظةٌ — الفجوةُ ' . ($target - $sigma) . ' ظاهرةٌ لا ممنوعة (C15)';
        return $out;
    }
    ...
    public static function transition($gate, $oblId, $newState, $exceptionRef = null)
    {
        ...
        $gate->update('contract_commitments', array('plan_state' => (string) $newState),
            array('id' => (int) $oblId));
        $r['reason'] = 'انتقلت الخطةُ إلى «' . $newState . '» — ' . $r['reason'];
        return $r;
    }
}
