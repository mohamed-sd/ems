    ...
    public static function resolveTimesheet(\mysqli $conn, $tsId)
    {
        ...
        if ($fanoutSource === 'legal') {
            ...
            if ($le) {
                ...
                // ── حارسُ اكتمال المرآة (E-02 ردمُ UAT-B07) ─────────────────────
                // مرآةٌ نصفيةٌ (رأسٌ صفريٌّ بلا أسطر زمن — كردم UAT القديم) لا
                // تدّعي سلطةَ المصدر القانوني: الارتدادُ لأعمدة الدوام **معلَنٌ**
                // لا صامتًا، وإلا فرضت المرآةُ الفارغةُ أصفارَها على المروحة
                // فتعذّر كلُّ ما جُسّر من الموروث. الصفوف الحية (الخدمة تملأ
                // الرأسَ والأسطرَ معًا) لا تمرّ من هنا أبدًا.
                if (count($llines) === 0 && (float) $le['qty'] <= 0) {
                    $ctx['legal_partial_fallback'] = true;
                    $ctx['legal_entry_id'] = intval($le['id']);
