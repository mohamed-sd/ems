...
if ($vSpoken === 0) {
    ...
    /* تفكيكُ الميتةِ المفردةِ من الشيفرةِ (أداةُ rpr03_consumer_emitters) —
       جنسان مختلفان علاجًا: ناطقٌ موجودٌ لم يجرِ (فجوةُ استعمالٍ) وبلا
       ناطقٍ أصلًا (سلكُ نشرٍ لم يُؤلَّف — فعلُ أعمالٍ لا يُرتجل) */
    $emWith = null; $emNone = null;
    $oe = array(); $ce = 0;
    @exec('"' . PHP_BINARY . '" ' . escapeshellarg($ROOT . '/tools/rpr03_consumer_emitters.php') . ' 2>&1', $oe, $ce);
    foreach ($oe as $l0) {
        if (preg_match('~EMITTER_EXISTS = (\d+)~', $l0, $m0)) { $emWith = (int) $m0[1]; }
        if (preg_match('~NO_EMITTER = (\d+)~', $l0, $m0)) { $emNone = (int) $m0[1]; }
    }
    $emNote = ($emWith !== null && $emNone !== null)
        ? " مفكَّكةً من الشيفرة: **{$emWith} ناطقُها موجودٌ لم يجرِ** (فجوةُ استعمالٍ كالمحطّاتِ الصامتة) و**{$emNone} بلا ناطقٍ أصلًا** (سلكُ نشرٍ لم يُؤلَّف — فعلُ أعمال)"
        : '';
    $vNote = " · ⛔ **والعطبُ ليس غيابَ مستهلك**: اشتراكاتُ الأثرِ مبنيّةٌ و**$vDead منها معلَّقٌ على مفردةٍ لم تُنطق قطّ**" . $emNote
...
/* ⑤ و⑥ الاعتماد */
/* ⑤ نافذةُ الظلِّ تُقرأ من مخزنِها الحيِّ `gov_ladder_shadow` — كلُّ تقييمٍ
   يُرصد سماحًا ومنعًا (FR-APP-001) فالمقامُ معلومٌ ولو صفرًا؛ والقيمُ
   الحدّيّةُ قرارُ مالكٍ معلَّقٌ (OA-CAPS-56) والنافذةُ تقيس بلا حدودٍ فلا
   تحجب — بنصِّ بندِها */
$shTot = null; $shDiv = null; $shSpan = '';
$q = @$conn->query("SELECT COUNT(*) t, SUM(COALESCE(diverged,0)) d,
                           MIN(observed_at) a, MAX(observed_at) b FROM gov_ladder_shadow");
if ($q && ($z = $q->fetch_assoc())) {
    $shTot = (int) $z['t']; $shDiv = (int) $z['d'];
    $shSpan = $z['a'] !== null ? ($z['a'] . ' الى ' . $z['b']) : 'لم تُسجَّل تقييماتٌ بعد';
}
$add('نافذةُ ظلٍّ للاعتمادِ مقيسةٌ بمقاييسِها الأربعة', 'منفَّذة',
     $shTot === null ? 'صفر' : ('تقييماتٌ ' . $shTot . ' وتباينٌ ' . $shDiv),
     $shTot === null ? null : $shDiv,
     $shTot === null
       ? '⛔ **غيرُ مقيس** — مخزنُ الظلِّ غيرُ مقروء'
       : ('**النافذةُ منفَّذةٌ بمخزنِها**: كلُّ تقييمِ سلّمٍ يُرصد سماحًا ومنعًا — المقامُ **' . $shTot
        . '** والتباينُ **' . $shDiv . '** (' . $shSpan . ') · والحدودُ العدديّةُ قرارُ مالكٍ معلَّقٌ '
        . '(`OA-CAPS-56`) ⛔ ولا تحجب النافذةُ بلا حدودٍ — بنصِّ بندِها'));
/* ⑥ الرحلاتُ الموصولةُ بسلاليمِها والوضعُ الحاكمُ من مفتاحِه المعلَن */
$jw = null;
$q = @$conn->query("SELECT COUNT(*) t, SUM(ladder_wired = 1) w FROM gov_journey_ladders");
if ($q && ($z = $q->fetch_assoc())) { $jw = array((int) $z['t'], (int) $z['w']); }
$lgMode = function_exists('ems_ladder_mode') ? ems_ladder_mode()
        : (function_exists('ems_env') ? strtolower((string) ems_env('EMS_LADDER_GATE', 'monitor')) : 'monitor');
$add('رحلاتُ اعتمادٍ تحجب فعلًا', '١٤ من ١٤',
     $jw === null ? 'صفر — وضعُ مراقبة' : ($lgMode === 'enforce' ? ($jw[1] . ' تحجب') : ('صفر — وضعُ ' . $lgMode)),
     $jw === null ? null : ($lgMode === 'enforce' ? max(0, $jw[0] - $jw[1]) : 0),
     $jw === null
       ? '⛔ **غيرُ مقيس** — سجلُّ الرحلاتِ غيرُ مقروء'
       : ('الموصولُ بسلّمِه **' . $jw[1] . ' من ' . $jw[0] . '** والتقييمُ يجري عند كلِّ قرارٍ — والوضعُ الحاكمُ '
        . '`' . $lgMode . '` من مفتاحِه المعلَنِ (`EMS_LADDER_GATE`): **الصفرُ المحجوبُ قرارُ تشغيلٍ لا عجزُ عُدّةٍ** '
        . '⛔ وقلبُه الى الحجبِ تغييرُ وصولٍ حيٍّ بقرارِ مالكٍ (`OA-SCHED`/سياسة) لا بيدِ مقياس'));
...
/* ⑨ الرحلاتُ البشرية */
/* ◆ **والتجهيزُ يُقاس ولو لم يُقَسِ الاجتيازُ بعد** — و`CONTINUE` §٦ يأمر بتجهيزِ
   ما يمكن **الآن** «فيبقى التنفيذُ ساعةً حين يتوفّر الأشخاص». ⇒ **يُعرض المُجهَّزُ
   بعددِه** كي يُعرف أنَّ الحاجزَ أشخاصٌ لا تحليل. ⛔ **ولا يُقرأ التجهيزُ اجتيازًا.** */
$jrN = 0; $jrSt = 0; $jrNeg = 0; $jrP = 0;
if ($tbl('repair01_w16_uat')) {
    $jrN   = (int) $one("SELECT COUNT(DISTINCT journey_key) FROM repair01_w16_uat");
    $jrSt  = (int) $one("SELECT COUNT(*) FROM repair01_w16_uat");
    $jrNeg = (int) $one("SELECT COUNT(*) FROM repair01_w16_uat WHERE is_negative = 1");
    $jrP   = (int) $one("SELECT COUNT(*) FROM repair01_w16_uat WHERE status = 'PASSED'");
}
$add('رحلاتٌ بشريّةٌ كاملةٌ بمسارِها السالب', '٦ من ٦', 'صفر', null,
     '⛔ **غيرُ مقيس** — تحتاج مستخدمين حقيقيّين (§٧). '
   . "◆ **والتجهيزُ تمّ**: **$jrN رحلاتٍ** بـ**$jrSt محطّةً** منها **$jrNeg سالبةً**، "
   . 'بأدوارِها وخاناتِ أشخاصِها المشتقّةِ من **تمايزِ الدور** — منتزَعةً من '
   . '`gov_screen_cycle` (‏المراحلُ المرتَّبة) و`repair01_w*_sod` (‏التركيبةُ الممنوعة) '
   . "**لا مؤلَّفةً**. ومجتازٌ **$jrP** ⇒ **الحاجزُ أشخاصٌ لا تحليل**");
...
$gdReady = $tbl('gov_golden_approvals')
    ? (int) $one("SELECT COUNT(*) FROM gov_golden_approvals
                   WHERE screen_file <> '' AND test_account <> '' AND role_id > 0") : 0;
$add('مراجعةٌ يدويّةٌ عميقةٌ للذهبيّاتِ العشر', '١٠ من ١٠', 'صفر', null,
     '⛔ **غيرُ مقيس** — تحتاج مراجعةً بشريّة. '
   . "◆ **والتجهيزُ تمّ**: **$gdReady** من العشرِ مسجَّلةٌ **بحسابِ اختبارٍ ودورٍ وأساسِ اعتماد** "
   . 'في `gov_golden_approvals` ⇒ **الحاجزُ عينٌ بشريّةٌ لا إعداد**');
