 ...
 *
 * ── توسعةُ REPAIR01 · W06 §٤-٩ — نقاءُ لغةِ الواجهة ───────────────────────
 * ◆ صنفانِ آخرانِ **مقفلا الاتّجاهِ من اليومِ الأوّل**: `UI-01` تشكيلٌ ظاهرٌ في
 *   نصِّ شاشةٍ حيّة · `UI-02` زخرفةٌ أو رمزٌ تقنيٌّ ظاهر. **ستّةَ عشرَ سجلًّا.**
 * ◆ **ومقامُهما رموزُ النصِّ لا الملفُّ الخام** (`repair01_ui_text_chunks`):
 *   تعليقاتُ هذا المستودعِ عربيّةٌ مشكولةٌ بكثافة، وعدُّها يجعل الرقمَ وصفًا
 *   لأسلوبِ التوثيقِ لا لِما يراه المستخدم.
 * ◆ **وهما في `repair01_ui_debt_classes()` لا في `repair01_debt_classes()`**:
 *   حاجبُ `W1-11` المُغلَقُ يشترط ثمانيةً في الثانية نصًّا — ولا يُعدَّل حاجبٌ
 *   مُغلقٌ لتيسيرِ مرحلةٍ تستفيد منه. والسقّاطةُ تضمُّ السجلَّين.
 *
...
/* ◆ **الصنفانِ الجديدانِ سجلٌّ ثانٍ لا توسعةٌ للأوّل** (‏W06 §٤-٩): حاجبُ
     `W1-11` المُغلَقُ يشترط ثمانيةً في `repair01_debt_classes()` نصًّا، وضمُّهما
     إليها يُسقطه. فالسقّاطةُ تضمُّ السجلَّين هنا — **وتوسَّع ولا تُبنى**. */
foreach (array_merge(repair01_debt_classes(), repair01_ui_debt_classes()) as $k => $meta) {
...
echo "سجلاتُ الدَّينِ الستّةَ عشرَ وسقّاطتُها المانعةُ للعودة\n";
echo str_repeat('═', 76), "\n";
...
if ($mdOut !== null) {
    $md = "# سجلاتُ الدَّينِ الستّةَ عشرَ — أرقامُ الجولةِ وسقّاطتُها\n\n";
