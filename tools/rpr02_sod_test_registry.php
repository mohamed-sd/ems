...
/* ═══ ⑥ الربط ════════════════════════════════════════════════════════════ */
$rows = array();
$stat = array('CODE_LIST' => 0, 'PROSE' => 0, 'ABSENT' => 0, 'bound' => 0, 'byKey' => 0, 'byCode' => 0);
foreach ($sod as $s) {
    $kind = sod_kind($s['enf']);
    $stat[$kind]++;
    $file = ''; $claim = ''; $bound = 0; $how = '';
    /* **الرابطُ الأقوى: مفتاحُ العمليّةِ نفسُه** — موحَّدٌ عبرَ الموجاتِ كلِّها */
    ...
    if ($bound) {
        $stat['bound']++;
        $claim = 'الفاحصُ `' . $file . '` يذكر ' . ($how === 'PROCESS_KEY'
               ? 'مفتاحَ العمليّةِ `' . $s['key'] . '`' : 'رمزَ الردِّ `' . substr($how, 13) . '`');
