...
/* ═══ ㊲ — قارئٌ مستقلٌّ يفتح ما يغلقه الحارس ═══════════════════════════
   ...
   ◆ **وقد فُتِّشت هذه الخانةُ فردًا فردًا (2026-09-05) فإذا لا قرارَ حيًّا فيها**:
     ① `admin/permissions/role_permissions.php` كونسولُ سوبرٍ غيرُ مسجَّلٍ ولا
        مبلوغٍ من تطبيقِ المستأجرِ، وصفرُ مستخدمِ سوبرٍ حيّ.
     ② `PermSourceService::legacyDecision` **أداةُ مقارنةِ ظلٍّ** يقرن المسارَين
        ولا يحرس — ينادِيها `cron_permissions` وحدَها (قياسٌ لا بوّابة).
     ③ `includes/permissions_helper.php` طبقةُ الصلاحيّاتِ نفسُها: استعلاماتُها
        مساعدُ الملاحةِ ودوالُّ التقارير، لا مسارُ قرارٍ ثانٍ.
     ④ `Settings/role_permissions.php` محروسةٌ بالمسارِ المعياريِّ
        (`enforce_current_page_view_permission`) وتقرأ الجدولَ **للعرض**،
        وكتابتُها مقفلةٌ (㊱).
     ⇒ فالخانةُ **مفسَّرةٌ لا مجهولة**، ولا قارئَ مستقلًّا يقرّر خارجَ القياس. */
...
$add(37, 'قارئٌ مستقلٌّ يفتح ما يغلقه الحارس (OVER_GRANT)', 'صفر', $indOver,
   ...
   . ' · وقارئٌ بلا رمزٍ مُعلَنٍ خارجَ القياس: ' . $indNoCode
     . ' — **فُتِّشوا فردًا فردًا ولا قرارَ حيًّا فيهم** (كونسولٌ غيرُ مبلوغ · '
     . 'أداةُ مقارنةِ ظلٍّ · طبقةُ الصلاحيّاتِ نفسُها · وشاشةُ عرضٍ محروسةٌ ومقفلة)'
   . ($indEx ? ' · ' . implode(' · ', $indEx) : ''));
...
/* ═══ ⑤ — مصدرُ القرارِ واحدٌ **في الشيفرةِ** لا في الواقعِ فحسب (ق-٥)
   ◆ كان يُقرأ «غيرَ مقيس» لأنَّ الفرعَ القديمَ باقٍ ولو لم يحكم أحدًا. وحُذف
     في م-5، فصار العددُ مقيسًا: يُفتَّش **جسمُ دالّةِ القرارِ** عن أيِّ قراءةٍ
     للجدولِ القديم.
   ⛔ **والشاهدُ يقرأ التعليقَ كما يقرأ الشيفرة** — فذِكرُ اسمِ الجدولِ في شرحٍ
     داخلَ الدالّةِ يُبلِّغ مصدرًا ثانيًا حيث لا مصدر. (وقع مقيسًا.) */
$__ds = (string) @file_get_contents($ROOT . '/includes/permissions_helper.php');
$__d1 = strpos($__ds, 'function get_module_permissions(');
$__dBody = '';
if ($__d1 !== false) {
    $__d2 = strpos($__ds, "
function ", $__d1 + 10);
    $__dBody = substr($__ds, $__d1, ($__d2 === false ? strlen($__ds) : $__d2) - $__d1);
}
$legacyInDecision = ($__dBody !== '' && strpos($__dBody, 'role_' . 'permissions') !== false);
$branches = $legacyInDecision ? 2 : 1;
$add(5, 'مصادرُ قرارِ الصلاحية', 'واحد', $branches,
     ($branches === 1 && $cov === $live && $live > 0) ? 'PASS' : 'FAIL',
     ($branches === 1
        ? 'الفرعُ القديمُ حُذف من دالّةِ القرار — والجدولُ باقٍ مقروءًا أثرًا لا حكمًا'
        : 'ما يزال فرعٌ ثانٍ في دالّةِ القرار')
   . ' · والتغطيةُ ' . $cov . ' من ' . $live
   . ' · مُثبَتٌ بـ tests/perm01_single_decision_source.php (8073 زوجًا مطابقًا · وغيرُ المغطَّى يُمنع)');
...
/* ◆ **والفرقُ يُفسَّر بحكمٍ أو يُرسِّب**: حُسم في ق-٦ أنَّ الطبقةَ **تُوسَم
     تاريخيّةً غيرَ نافذة** ولا تُصالَح ولا تُملأ بأسماءِ شاشات. فصفٌّ لا يُعرَف
     صاحبُه في طبقةٍ **موسومةٍ غيرَ نافذةٍ بحكمٍ مسجَّل** له تفسيرٌ — وهو التفسيرُ
     الذي طلبه البند. أمّا بلا حكمٍ فيبقى «فرقًا بلا تفسير». */
$r21 = $rulingOf('effective_permissions');
$ep21 = ($r21 !== null && $r21['ruling'] === 'not_in_force') ? 0 : ($epNoSubject + $epNoSource);
$add(21, 'فرقُ تفويضٍ بلا تفسير', 'صفر', $ep21, $ep21 === 0 ? 'PASS' : 'FAIL',
     ($r21 ? ('موسومةٌ «' . $r21['ruling'] . '» بمرجع ' . $r21['doc_ref'] . ' — تاريخيّةٌ لا تُصالَح') : 'بلا حكمٍ مسجَّل')
   . ' · ومن ' . $epTotal . ' صفًّا: ' . $epNoSubject . ' لا يُعرَف صاحبُه (معرِّفٌ خارجَ users وpersons)'
   . ' · والمفرداتُ أفعالٌ لا شاشات، ولم يُحذف صفٌّ');
...
/* ═══ ㉞ — DIRECT_URL_AUTH_MISMATCH (PERM-01-DEC §2) ═════════════════════
   ◆ **والمعيارُ صفرٌ لا «لا يضرّ»**: كنتُ عددتُ افتراقَ البلاغاتِ حميدًا لأنَّ
     البابَ أوسعُ من القائمة — ورفضه المالكُ نصًّا: «لا يُقبل أن لا يردَّ أحدٌ
     يرى الرابط». فالوصفُ ليس حكمًا، و**أيُّ** افتراقٍ بين هويّةِ العرضِ وهويّةِ
     الإنفاذِ خرقٌ ولو لم يُسقِط أحدًا اليوم — لأنّه يسقط غدًا بمنحةٍ واحدة. */
$add(34, 'DIRECT_URL_AUTH_MISMATCH — هويّةُ العرضِ غيرُ هويّةِ الإنفاذ', 'صفر',
     $twoIds + $benign, ($twoIds + $benign) === 0 ? 'PASS' : 'FAIL',
     'حرِجٌ (يرى ويُردّ): ' . $twoIds . ' · ومفترقٌ بلا أثرٍ اليوم: ' . $benign
   . ' — وكلاهما يُعَدُّ خرقًا بنصِّ §2');
