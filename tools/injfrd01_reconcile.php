...
/* ══ ④ العددُ المُعلَنُ في الوثيقةِ = عددُ صفوفِ الدفتر (البند ٠-٧) ═══════════
 * ◆ **بوابةٌ تمرُّ ٢٠/٢٠ وهي تحمل تناقضًا لا تذكره.** فصولُ الوثيقةِ الثمانيةُ
 *   تجمع **61** والدفترُ **61 صفًّا**، بينما نصُّ الوثيقةِ §٢-١ وذيلُ ورقةِ
 *   الثوابتِ يقولان **٥٩** — والأداةُ **لم تكن تذكر الرقمَ أصلًا**. ومعيارُ
 *   القبولِ ١٢ يشترط مطابقةَ «العدد» نصًّا، فسكوتُ الأداةِ عنه سكوتٌ عن حكمِها.
 * ◆ والاستخراجُ **بالعربيةِ والهنديةِ معًا**: العددُ يُكتب «تسعةً وخمسين» في
 *   الجملةِ و«٥٩» في الجدول — فقارئٌ يعرف صيغةً واحدةً يمرُّ على الأخرى.
 */
/** الأعدادُ حروفًا — **موضعٌ واحدٌ للمعرفة** يقرأ منه المقياسُ وحزامُه معًا. */
function frd_count_words()
{
    return array(
        'سبعةً وخمسين' => 57, 'ثمانيةً وخمسين' => 58, 'تسعةً وخمسين' => 59,
        'ستّين' => 60, 'واحدًا وستين' => 61, 'اثنين وستين' => 62,
        'ثلاثةً وستين' => 63, 'أربعةً وستين' => 64,
    );
}

function frd_declared_counts($doc)
{
    $out = array();
    /* ① الجملةُ التقريرية: «تحمل هذه الوثيقةُ <عددًا> متطلبَ إصلاحٍ وضابطًا» */
    $WORDS = frd_count_words();
    foreach ($WORDS as $w => $n) {
        if (mb_strpos($doc, $w . ' متطلبَ') !== false) {
            $out[] = array('src' => 'نصُّ §٢-١ «' . $w . '»', 'n' => $n);
        }
    }
    /* ② جدولُ الأرقامِ الحاكمة: سطرُ «متطلباتُ إصلاحٍ وضوابط» يليه العدد */
    /* علاماتُ الاتجاهِ غيرُ مرئيةٍ وتفسد المطابقةَ الحرفية — تُنزع أولًا */
    $strip = function ($x) {
        return trim(preg_replace('~[\x{200E}\x{200F}\x{202A}-\x{202E}\x{2066}-\x{2069}\x{00A0}\x{2060}]~u', '', (string) $x));
    };
    $L = array_map($strip, preg_split('~\R~u', $doc));
    /* ◆ الأرقامُ هنديةٌ في الوثيقةِ وعربيةٌ في الدفتر — والخليةُ قد تحمل
     *   فاصلَ جدولٍ `|` من طيِّ الصفوف. فيُوحَّد الشكلانِ قبلَ المطابقة. */
    $ar2en = function ($s) {
        $v = strtr((string) $s, array('٠'=>'0','١'=>'1','٢'=>'2','٣'=>'3','٤'=>'4',
                                     '٥'=>'5','٦'=>'6','٧'=>'7','٨'=>'8','٩'=>'9'));
        return trim(str_replace('|', '', $v));
    };
    /* ◆ **وشكلانِ للسطرِ لا شكلٌ واحد**: `docx_text` هنا يطوي خلايا الصفِّ
     *   بفواصلِ `|`، فصفُّ الجدولِ يصير سطرًا واحدًا — بينما استخراجًا آخرَ
     *   يضع كلَّ خليةٍ في سطر. فيُقرأ الشكلانِ معًا، وإلا قرأ الفاحصُ شكلًا
     *   ومرَّ على الآخرِ صامتًا — وهو عينُ العطبِ الذي يعالجه هذا البند. */
    $KEYS = array('متطلباتُ إصلاحٍ وضوابط' => 'ذيلُ ورقةِ الثوابت',
                  'اختباراتٌ سالبةٌ إلزامية' => 'ذيلُ الثوابت — الاختباراتُ السالبة');
    foreach ($L as $i => $l) {
        $cells = array_map('trim', explode('|', $l));
        foreach ($KEYS as $key => $lbl) {
            /* ① صفُّ جدولٍ مطويٌّ بفواصل: <المفتاح> | <العدد> | <الدلالة> */
            if (count($cells) > 1 && $cells[0] === $key) {
                for ($c = 1; $c < count($cells); $c++) {
                    $v = $ar2en($cells[$c]);
                    if (preg_match('~^\d{1,3}$~', $v)) {
                        $out[] = array('src' => $lbl, 'n' => (int) $v); break 1;
                    }
                }
                continue;
            }
            /* ② خلايا في أسطرٍ متتابعة */
            if ($l !== $key) { continue; }
            for ($j = $i + 1; $j < min($i + 4, count($L)); $j++) {
                $v = $ar2en($L[$j]);
                if (preg_match('~^\d{1,3}$~', $v)) { $out[] = array('src' => $lbl, 'n' => (int) $v); break; }
            }
        }
    }
    return $out;
...
$declared = frd_declared_counts($doc);
$rowsN = count($reqs);
$mismatch = array();
foreach ($declared as $d) { if ((int) $d['n'] !== $rowsN) { $mismatch[] = $d['src'] . '=' . $d['n']; } }
G(count($declared) > 0, 'العددُ المُعلَنُ يُستخرج من الوثيقةِ فعلًا',
  count($declared) > 0 ? count($declared) . ' موضعًا مُعلَنًا' : '**لا موضعَ يُقرأ — والصمتُ ليس مطابقة**');
G(empty($mismatch), 'والعددُ المُعلَنُ في الوثيقةِ = عددُ صفوفِ الدفتر',
  empty($mismatch) ? "كلُّ المواضعِ تقول {$rowsN} — والدفترُ {$rowsN} صفًّا"
                   : "الدفترُ **{$rowsN}** · والوثيقةُ تقول: " . implode(' · ', $mismatch));

/* ── حزامُ صدقِ بوابةِ العدد — `--negative` (البند ٠-٧) ──────────────────────
 * ◆ **بوابةٌ تُقارن رقمًا يجب أن تُجرَّب برقمَين**: واحدٍ يطابق فتمرّ، وآخرَ
 *   يخالف فترسُب — وإلا كانت «المطابقةُ» ثابتًا مكتوبًا في الفاحصِ لا قراءةً منه.
 * ◆ **والحزامُ لا يعرف ما تقوله الوثيقةُ اليوم**، بل يشتقُّ الصيغةَ المُعلَنةَ منها
 *   ثم يبدّلها. فحزامٌ يُثبَّت على «٥٩» يتعفّن في اللحظةِ التي تُصحَّح فيها
 *   الوثيقةُ إلى ٦١ — وهو العطبُ نفسُه الذي يعالجه هذا البند، فلا يُعاد صنعُه هنا.
 * ◆ والعطبُ في **نسخةٍ بالذاكرة** لا في ملفِّ الوثيقة — فلا تُمَسُّ وثيقةٌ حاكمةٌ لأجلِ فحص.
 */
if (in_array('--negative', $argv, true)) {
    echo "\n══ حزامُ صدقِ بوابةِ العدد — تُجرَّب برقمَين ══\n";
    $nOk = 0; $nBad = 0;
    $judge = function ($txt, $rowsN) {
        $mm = array();
        foreach (frd_declared_counts($txt) as $d) {
            if ((int) $d['n'] !== $rowsN) { $mm[] = $d['src'] . '=' . $d['n']; }
        }
        return $mm;
    };
    $EN2AR = array('0'=>'٠','1'=>'١','2'=>'٢','3'=>'٣','4'=>'٤',
                   '5'=>'٥','6'=>'٦','7'=>'٧','8'=>'٨','9'=>'٩');
    $ar = function ($n) use ($EN2AR) { return strtr((string) $n, $EN2AR); };

    /** يبدّل كلَّ عددٍ مُعلَنٍ في النصِّ إلى `$to` — أيًّا كانت صيغتُه اليوم. */
    $retune = function ($txt, $to) use ($ar) {
        $words = frd_count_words();
        $toWord = array_search($to, $words, true);
        foreach (frd_declared_counts($txt) as $d) {
            $from = (int) $d['n'];
            if ($from === $to) { continue; }
            $fromWord = array_search($from, $words, true);
            if ($fromWord !== false && $toWord !== false) {
                $txt = str_replace($fromWord . ' متطلبَ', $toWord . ' متطلبَ', $txt);
            }
            $txt = str_replace('| ' . $ar($from), '| ' . $ar($to), $txt);
        }
        return $txt;
    };

    /* ① نصٌّ عددُه = عددُ صفوفِ الدفتر ⇒ **يجب أن يمرّ** */
    $good = $retune($doc, $rowsN);
    $g = $judge($good, $rowsN);
    $c1 = empty($g);
    echo '  ' . ($c1 ? '✔' : '✘') . " نصٌّ عددُه {$rowsN} **يمرّ** — "
       . ($c1 ? 'صفرُ مخالفة' : implode(' · ', $g)) . "\n";
    $c1 ? $nOk++ : $nBad++;

    /* ② نصٌّ عددُه ≠ صفوفِ الدفتر ⇒ **يجب أن يرسُبَ ويُسمَّى الرقمُ المخالف** */
    $wrong = $rowsN + 1;
    $badTxt = $retune($good, $wrong);
    $m2 = $judge($badTxt, $rowsN);
    $named = false;
    foreach ($m2 as $x) { if (strpos($x, '=' . $wrong) !== false) { $named = true; } }
    $c2 = !empty($m2) && $named;
    echo '  ' . ($c2 ? '✔' : '✘') . " ونصٌّ عددُه {$wrong} **يرسُب ويُسمَّى** — "
       . ($m2 ? implode(' · ', $m2) : 'مرَّ') . "\n";
    $c2 ? $nOk++ : $nBad++;

    /* ③ والمواضعُ المُعلَنةُ **تُقرأ كلُّها** لا موضعًا واحدًا — فقارئٌ يمسك
     *   الجملةَ ويترك الجدولَ يُخضِّر نصفَ التناقض. */
    $c3 = count(frd_declared_counts($doc)) >= 3;
    echo '  ' . ($c3 ? '✔' : '✘') . ' والمواضعُ المُعلَنةُ تُقرأ كلُّها — '
       . count(frd_declared_counts($doc)) . " موضعًا (الجملةُ · ذيلانِ في الثوابت)\n";
    $c3 ? $nOk++ : $nBad++;

    echo "\n" . str_repeat('─', 78) . "\n";
    printf("◆ **صدقُ بوابةِ العدد: %d/3** — تُجرَّب معطوبةً وسليمةً قبلَ تصديقِها\n", $nOk);
    exit($nBad === 0 ? 0 : 1);
}
echo "\n" . str_repeat('─', 78) . "\n";
