 ...
 *   يخالف فترسُب. وإلا كان «المطابقةُ» ثابتًا مكتوبًا في الفاحصِ لا قراءةً منه.
 * ◆ والعطبُ **في نصٍّ منسوخٍ في الذاكرة** لا في ملفِّ الوثيقة — فلا تُمَسُّ
 *   وثيقةٌ حاكمةٌ لأجلِ فحص.
 */
if (in_array('--negative', $argv, true)) {
    ...
    /* ① النصُّ كما هو — يجب أن يُخالف ما دام الدفترُ 61 والوثيقةُ 59 */
    $asIs = $judge($doc, $rowsN);
    $c1 = !empty($asIs);
    echo '  ' . ($c1 ? '✔' : '✘') . " النصُّ كما هو **يُخالف** الدفترَ — "
       . ($c1 ? count($asIs) . ' موضعًا' : 'لم يُخالف') . "\n";
    $c1 ? $nOk++ : $nBad++;

    /* ② نصٌّ مُصلَحٌ في الذاكرة إلى عددِ صفوفِ الدفتر — يجب أن **يمرّ** */
    $EN2AR = array('0'=>'٠','1'=>'١','2'=>'٢','3'=>'٣','4'=>'٤',
                   '5'=>'٥','6'=>'٦','7'=>'٧','8'=>'٨','9'=>'٩');
    $rowsAr = strtr((string) $rowsN, $EN2AR);
    $fixed = str_replace(array('تسعةً وخمسين متطلبَ', '| ٥٩'),
                         array('واحدًا وستين متطلبَ', '| ' . $rowsAr), $doc);
    $c2 = empty($judge($fixed, $rowsN));
    echo '  ' . ($c2 ? '✔' : '✘') . " ونصٌّ مُصلَحٌ إلى {$rowsN} **يمرّ** — فالبوابةُ تقرأ ولا تحفظ\n";
    $c2 ? $nOk++ : $nBad++;

    /* ③ نصٌّ بستّين — يجب أن يرسُبَ **ويُسمّي الستّين** لا رقمًا آخر */
    $sixty = str_replace(array('تسعةً وخمسين متطلبَ', '| ٥٩'),
                         array('ستّين متطلبَ', '| ٦٠'), $doc);
    $m3 = $judge($sixty, $rowsN);
    $named = false;
    foreach ($m3 as $x) { if (strpos($x, '=60') !== false) { $named = true; } }
    $c3 = !empty($m3) && $named;
    echo '  ' . ($c3 ? '✔' : '✘') . " و«٦٠ في الملخص» **يرسُب ويُسمَّى** — "
       . ($m3 ? implode(' · ', $m3) : 'مرَّ') . "\n";
    $c3 ? $nOk++ : $nBad++;
