...
$prjSrc = (int) $one("SELECT COUNT(*) FROM $LIVE AND surface_kind = 'PROJECTION'
                        AND source_of_truth <> ''");
/* ⛔ **والمقامُ «المنطبقُ» — واسمُ المقياسِ يقوله**: «لكلِّ إسقاطٍ **منطبق**».
   ◆ **والإسقاطُ الذي كيانُه من كِيتٍ مشتركٍ أو بنيةٍ صِرفةٍ لا مصدرَ خاصًّا له
     يُسمّى** — و`rpr02_sot_assign` يرفض كتابتَه بنصِّه: *«مرجعٌ كاذبٌ أسوأُ من
     غيابِه: الغائبُ يُعلن نفسَه والكاذبُ يُقرأ صحيحًا»*. ⇒ **فعدُّه في المقامِ
     يطالب المقياسَ بما تمنعه أداتُه** — وذاك تناقضٌ لا نقص.
   ⛔ **والمستبعَدُ يُعرض بعددِه وسببِه ولا يُطوى** (§٤·٣): فمن يملك تضييقَ
     المقامِ صامتًا يملك رفعَ النسبةِ بلا كتابةِ مرجعٍ واحد. */
$prjX = (int) $one("SELECT COUNT(*) FROM $LIVE AND surface_kind = 'PROJECTION'
                      AND source_of_truth = ''
                      AND COALESCE(grain_fact_scope,'') <> 'OWN_FACT'");
$prjApp = $prj - $prjX;
$prjOpen = $prjApp - $prjSrc;
$add('مرجع مصدر صريح لكل إسقاط منطبق', '100٪', 'MEASURED',
     ($prjApp ? round($prjSrc * 100 / $prjApp, 1) : 0) . '٪',
     "$prjSrc من **$prjApp** سطحَ إسقاطٍ **منطبقٍ** يحمل `source_of_truth` غيرَ فارغ — "
   /* ⚠ **والحرفُ العربيُّ يلتصق بالمتغيِّر**: `$prj،` يُقرأ اسمَ متغيِّرٍ واحدًا
      لأنَّ PHP تقبل البايتات فوقَ ١٢٧ في المعرِّفات — فيُطبَع فراغٌ مكانَ عدد.
      ⇒ **يُقوَّس صراحةً** — وهذا عطبٌ يمرُّ صامتًا في كلِّ نصٍّ عربيٍّ مُقحَم. */
   . 'والمقامُ الخامُّ ' . $prj . '، **ومستبعَدٌ ' . $prjX . '** كيانُه من **كِيتٍ مشتركٍ أو بنيةٍ صِرفة** '
   . '(`SHARED_KIT` · `INFRA_ONLY` · بلا كيانٍ مقيس) ⇒ **لا مصدرَ خاصًّا له يُسمّى**، '
   . 'و`rpr02_sot_assign` يرفض كتابتَه بنصِّه: «مرجعٌ كاذبٌ أسوأُ من غيابِه». '
   . '⛔ **والمستبعَدُ معروضٌ بعددِه وسببِه ولا يُطوى** (§٤·٣) — والمفتوحُ الحقيقيُّ **'
   . $prjOpen . '**');
