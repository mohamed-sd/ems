...
$FAM = array(
    ...
    /* ◆ **وفاحصُ فصلِ الواجباتِ يدخل الانحدارَ** — وإلّا كان اثنتَين وتسعين
       دعوى تغطيةٍ لا يُشغَّل أحدُها في جولةٍ ولا يُرى رسوبُه. */
    'فاحصُ فصلِ الواجبات' => 'tests/rpr02_sod_*.php',
);
...
foreach ($FAM as $fam => $glob) {
    foreach (glob($ROOT . '/' . $glob) as $f) {
        ...
        /* ◆ **رمزُ الخروجِ وحدَه لا يكفي**: أدواتٌ تطبع «ساقطة» وتخرج صفرًا.
           ...
           ⚠ **لكنّ التفتيشَ في كلِّ المخرَجِ يُرسِّب البريء**: `W15` خرج صفرًا
             وطبع «خضراء ✔» **ولا رمزَ رسوبٍ فيه** — ومع ذلك عددتُه راسبًا،
             لأنَّ شروحَ الأدواتِ تذكر ألفاظَ الرسوبِ وهي تشرحها.
             ⇒ **فالحكمُ من سطرِ الحكمِ الأخيرِ وحدَه**، وما عداه شرحٌ لا حكم. */
        $verdictLine = '';
