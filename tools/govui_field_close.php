...
if ($APPLY) {
    ...
    foreach ($plans as $pl) {
        ...
        /* الأداةُ تُعاد تشغيلُها — فالخريطةُ تُصحَّح مرّاتٍ قبل أن تستقرّ.
           ...
           خريطةٌ فوقَ خريطة. */
        /* والوسمُ يحمل معرِّفَ شبكتِه: شاشةٌ تخدم هدفَين لها كتلتان، ووسمٌ عامٌّ
           يجعل الثانيةَ تدهس الأولى — قِيس في proc_offers.php وproc_packages.php:
           هدفان في ملفٍّ واحدٍ وكتلةٌ واحدةٌ نجت. */
        $myMark = GFC_MARK . ':' . $sc['grid_id'];
        $i = strpos($s2, $myMark);
        if ($i === false && strpos($s2, GFC_MARK . ':') === false) { $i = strpos($s2, GFC_MARK); }
        if ($i !== false) {
...
"; continue; }
        ...
        $blk  = $ind . "<?php " . GFC_MARK . ":" . $sc['grid_id'] . "
";
