<?php
/**
 * tools/repair01_w5_classify.php — تصنيفُ أسطحِ W05 في سجلِّ المساحات
 * ═══════════════════════════════════════════════════════════════════════════
 * ◆ **لماذا**: W05 بنت ستَّ شاشاتٍ ولم تُدخلها `gov_space_appearances`، فارتفع
 *   دَينُ `NF-24` من ١٢ إلى ١٨ ومنعت السقّاطةُ الالتزام. والسقّاطةُ محقّة:
 ...
 *
 * التشغيل: php tools/repair01_w5_classify.php [--dry]
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
function e5($conn, $v) { return $conn->real_escape_string((string) $v); }
function one5($conn, $sql) { $r = $conn->query($sql); if (!$r) { return null; } $x = $r->fetch_row(); return $x ? $x[0] : null; }

/* ═══ ① أسطحُ النموِّ المختومةُ W05 ═══ */
$screens = array();
$r = $conn->query("SELECT screen_id, route, owner_code FROM repair01_screen_registry WHERE origin = 'W05'");
...
$in = "'" . implode("','", array_map(function ($k) use ($conn) { return e5($conn, $k); }, array_keys($screens))) . "'";
...
$nextId = (int) one5($conn, "SELECT COALESCE(MAX(id), 0) + 1 FROM gov_space_appearances");
$ins = 0; $upd = 0;
foreach ($out as $o) {
    list($w, $cls, $step, $basis, $ownSp) = $o;
    $spCount = (int) one5($conn, "SELECT COUNT(DISTINCT space_ar) FROM gov_space_appearances
                                  WHERE route = '" . e5($conn, $w['route']) . "'");
    $spCount = $spCount + 1;
    $exists = (int) one5($conn, "SELECT COUNT(*) FROM gov_space_appearances
                                 WHERE space_ar = '" . e5($conn, $w['space']) . "'
                                   AND route = '" . e5($conn, $w['route']) . "'");
    $kind = isset($kindOf[$w['space']]) ? $kindOf[$w['space']] : 'DEPARTMENT';
    $sql = $exists
      ? "UPDATE gov_space_appearances SET cls='" . e5($conn, $cls) . "', ownership='VALID', decision='CONFIRMED',
           basis='" . e5($conn, $basis) . "', rule_step=" . (int) $step . ", updated_at=NOW()
         WHERE space_ar='" . e5($conn, $w['space']) . "' AND route='" . e5($conn, $w['route']) . "'"
      : "INSERT INTO gov_space_appearances
         ...
         VALUES (" . (int) $nextId . ",'" . e5($conn, $w['space']) . "','" . e5($conn, $kind) . "','','" . e5($conn, $w['label']) . "',
                 '" . e5($conn, $w['route']) . "','" . e5($conn, $ownSp) . "','BUSINESS_DEPARTMENT',
                 'RPR-W05','VALID','CONFIRMED','سطحُ نموٍّ مختومٌ W05 — صُنِّف بسلّمِ الحسمِ السداسيّ',
                 " . (int) $spCount . ",'" . e5($conn, $cls) . "','VALID','CONFIRMED',
                 '" . e5($conn, $basis) . "'," . (int) $step . ",'',NOW())";
    if ($DRY) { continue; }
...
if ($noSpace) {
    ...
    foreach ($noSpace as $rid => $rts) {
        $nm = one5($conn, "SELECT name FROM roles WHERE id = " . (int) $rid);
