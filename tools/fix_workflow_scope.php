<?php
/**
 * tools/fix_workflow_scope.php — نطاقُ عائلةِ «دورةِ العملِ الخاطئة» مشتقًّا بالقاعدة
 * ═══════════════════════════════════════════════════════════════════════════
 * القاعدة: كلُّ ملاحظةٍ نوعُها `Wrong Workflow` في السجلِّ الجامعِ وحالتُها في
...
echo "══════════════════════════════════════════════════════════════════\n";
echo " نطاقُ «دورةِ العملِ الخاطئة» — مشتقٌّ بالقاعدةِ لا منقولًا\n";
echo "══════════════════════════════════════════════════════════════════\n\n";
echo '  السجلُّ الجامع        : ' . count($all) . " ملاحظة\n";
echo '  منها العائلةُ كلُّها  : ' . count($fam) . "  (Wrong Workflow)\n";
...
if (in_array('--json', $argv, true)) {
    file_put_contents($ROOT . '/docs/fix_progress/workflow_scope.json',
        json_encode(array_values($open), JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
    echo "\n  · كُتب: docs/fix_progress/workflow_scope.json\n";
}
