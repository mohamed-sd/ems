<?php
/**
 * Maintenance/gov_dept_mnt.php — حوكمة الصيانة
 * ─────────────────────────────────────────────────────────────────────────
 * ظهورٌ نطاقيٌّ لمكوّن «حوكمة الإدارة» الواحد على الصيانة:
 ...
 * يمنح، ونطاقُه `gov_dept_mnt` يُطابَق على سجلِّ الشاشاتِ في المعالج.
 *
 * ◆ عدّادُ الأحداثِ مقصورٌ على `source_module = 'maintenance'` — فما ظهر صفرًا
...
$__pp = check_page_permissions($conn, 'Maintenance/gov_dept_mnt.php');
...
if (function_exists('ems_states_bundle')) {
    echo ems_states_bundle('لا قياساتِ فصلِ واجباتٍ معلَنةً لحوكمةِ الصيانة',
                           'تُقاس القياساتُ على أوامرِ الصيانةِ والفحوصِ الحية — وما لم يُقَس يُعلَن ولا يُلفَّق');
}
