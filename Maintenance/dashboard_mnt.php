...
<div class="main mnt-board-main ems-unified-page-shell">
    ...
    <div style="display:grid;grid-template-columns:2fr 1fr;gap:12px;margin-top:14px">
        <div class="card"><div class="card-body">
            <h5 style="margin:0 0 10px"><i class="fas fa-wave-square"></i> نبض الأداء — أوامرُ أُنشئت مقابل أُغلقت (7 أيام)</h5>
...
new Chart(document.getElementById('rbPulse'), {
    ...
    data: {
        ...
        datasets: [
            { label: 'أُنشئت', data: <?php echo json_encode($rb_pulse['in']); ?>,  backgroundColor: 'rgba(146,64,14,.75)' },
            { label: 'أُغلقت', data: <?php echo json_encode($rb_pulse['out']); ?>, backgroundColor: 'rgba(22,101,52,.75)' }
        ]
    },
    options: { responsive: true, plugins: { legend: { position: 'bottom', rtl: true } }, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 } } } }
});
</script>
</body>
</html>
