 ...
 *   تعديل · حذف) لدورٍ **واحدٍ يُختار**.
 * ⛔ **وصارت قراءةً تاريخيّةً بـPERM-01 §7**: التغطيةُ تامّةٌ فـ`role_permissions`
 *   لا يحكم مستخدمًا حيًّا — وحفظٌ هنا وعدٌ لا يُنفَّذ. فالكتابةُ تُردُّ بسببِها.
...
/* ══ PERM-01 §7 — المصفوفةُ صارت **قراءةً تاريخيّة** ══════════════════════
   ◆ **الأمرُ نصًّا**: «إصلاحٌ فوريّ بلا انتظار الواجهة الجديدة: مصفوفة
     الصلاحيات تصير قراءةً تاريخية · وزرّ الحفظ يوجّه إلى تحرير القالب».
   ⛔ **والسببُ مقيسٌ لا مبدئيّ**: التغطيةُ تامّةٌ (75 من 75)، فكلُّ مستخدمٍ حيٍّ
     يُحكَم بقالبِه حصرًا و`role_permissions` **لا يحكم أحدًا**. فحفظٌ هنا
     **وعدٌ لا يُنفَّذ**: الواجهةُ تؤكّد والنظامُ لا يعمل به — وهو عيبُ ثقةٍ لا
     عيبُ أداء.
   ◆ **ويُردُّ بسببِه لا بصمت**: الكتابةُ تُمنع ويُقال للمشغّلِ أين يُحرِّر فعلًا.
   ◆ والجدولُ يبقى مقروءًا: هو **سجلُّ ما كان**، ومصدرُ بذرِ القوالبِ تاريخيًّا. */
$PERM01_MATRIX_READONLY = true;

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($PERM01_MATRIX_READONLY) {
        $err = 'المصفوفة للقراءة التاريخية، والتحرير يقع في قالب الدور. '
             . 'وسبب المنع: التغطية تامة فهذا الجدول لا يحكم مستخدما حيا (PERM-01 §7).';
    ...
    <form method="post" action="">
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <?php /* ◆ لا زرَّ حفظٍ يَعِدُ بما لا يقع — والإرشادُ يقول أين يُحرَّر فعلًا. */ ?>
                <div class="form-actions">
                    <span class="status-badge status-pending">
                        <i class="fa fa-lock"></i> قراءة تاريخية بلا تحرير
                    </span>
                </div>
                <p>هذا الجدول سجل ما كان: التغطية تامة فكل مستخدم حي يحكمه قالب دوره،
                   وهذه الصفوف لا تسري على احد. والتحرير يقع في قالب الدور.</p>
            </div>
        </div>
    </form>
    <?php endif; ?>
