...
/* ═══════════════════════════════════════════════════════════════════════════
 * رموزُ صلاحياتِ المستخدمِ للأعمدةِ الواعيةِ بالصلاحية (شبكةُ إنجاز)
 * ───────────────────────────────────────────────────────────────────────────
 * ◆ قرارُ المالك (ف١٦): «أعمدةٌ واعيةٌ بالصلاحيةِ وتصديرٌ بالمسموحِ لا بالكل».
 *   و`ui-unification.js` يقرأ هذا المسندَ ليُخفيَ العمودَ الموسومَ `data-perm`
 *   **من الشاشةِ ومن التصديرِ معًا** — فلا يجد في إكسل ما لم يرَه في الشاشة.
 * ◆ **وهذا عرضٌ لا حراسة**: الحارسُ الحقيقيُّ في الاستعلامِ وفي
 *   `ems_require_action()`. فلا يُعوَّل عليه في المجالاتِ الستةِ المحظورة —
 *   والحقلُ السريُّ يُمنع في الاستعلامِ لا في المتصفح.
 * ◆ **ولا يُبثُّ إلا ما هو رموزٌ نظيفة**: تُرشَّح بنمطٍ صارمٍ فلا يتسرّب نصٌّ
 *   حرٌّ ولا معرِّفُ مستخدمٍ ولا اسمُ دور.
 * ═══════════════════════════════════════════════════════════════════════════ */
if (isset($_SESSION['user']) && !empty($GLOBALS['EMS_COL_PERMS']) && is_array($GLOBALS['EMS_COL_PERMS'])) {
    $__cp = array();
    foreach ($GLOBALS['EMS_COL_PERMS'] as $__c) {
        if (is_string($__c) && preg_match('/^[a-zA-Z0-9_.\-]{2,64}$/', $__c)) { $__cp[] = $__c; }
    }
    if ($__cp) {
        echo ' data-ems-perms="' . htmlspecialchars(implode(',', array_unique($__cp)), ENT_QUOTES, 'UTF-8') . '"';
    }
}
?>>
...
<a class="ems-skip-link" href="#ems-main-content">تخطي إلى المحتوى</a>
<?php
...
if (isset($_SESSION['imp_session']['imp_id'])
    ...
    echo '<div class="ems-imp-strip" dir="rtl" role="status">'
       . '<span class="ems-imp-strip-text">تعمل الآن موضع <b>'
       . htmlspecialchars((string) $__imp['target_name'], ENT_QUOTES, 'UTF-8')
       . '</b> — بسبب: ' . htmlspecialchars((string) $__imp['reason'], ENT_QUOTES, 'UTF-8')
       ...
       . '<button type="submit" class="btn btn-sm btn-light">إنهاء الجلسة</button>'
       . '</form>'
       . '</div>';
}
?>
