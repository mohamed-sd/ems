...
<div class="main ems-unified-page-shell">
    ...
    <div class="table-wrap"><table class="data-table">
        <thead><tr><th>مرجع العرض</th><th>طلب العروض</th><th>المورد</th><th>تاريخ التقديم</th><th>سار حتى</th><th>العملة</th><th>الإجمالي</th><th>بالأساس</th><th>البنود</th><th>مدة التوريد</th><th>ورد بعد الموعد</th><th>البنود</th><th>معرف العرض</th><th>رقم طلب العروض</th><th>رقم المورد</th><th>اسم المورد</th><th>تاريخ الاستلام</th><th>قيمة العرض</th><th>شروط الدفع المعروضة</th><th>صلاحية العرض</th><th>التقييم الفني</th><th>ملاحظات الفحص</th><th>الترتيب المالي</th><th>حالة العرض</th><th>المنشئ</th><th>تاريخ الإنشاء</th><th>حالة البيانات</th><th>مرجع المصدر</th><th>معرف السطر</th><th>مرجع بند الطلب</th><th>كود الصنف</th><th>سعر الوحدة المعروض</th><th>الكمية</th><th>مدة التوريد للبند</th><th>بديل مقترح؟</th><th>ملاحظة فنية</th><th>ترتيب البند ماليا</th></tr></thead>
        <tbody>
        <?php if ($rows): foreach ($rows as $r): $q = isset($rfqs[(int) $r['rfq_id']]) ? $rfqs[(int) $r['rfq_id']] : null; ?>
            ...
            <td><?= ems_sf($r, 'offer_uid') ?></td><td><?= ems_sf($r, 'rfq_no') ?></td><td><?= ems_sf($r, 'supplier_no') ?></td><td><?= ems_sf($r, 'supplier_name') ?></td><td><?= ems_sf($r, 'receipt_date') ?></td><td><?= ems_sf($r, 'offer_value') ?></td><td><?= ems_sf($r, 'offered_payment_terms') ?></td><td><?= ems_sf($r, 'offer_validity') ?></td><td><?= ems_sf($r, 'technical_evaluation') ?></td><td><?= ems_sf($r, 'inspection_notes') ?></td><td><?= ems_sf($r, 'financial_arrangement') ?></td><td><?= ems_sf($r, 'offer_state') ?></td><td><?= ems_sf($r, 'creator_name') ?></td><td><?= ems_sf($r, 'created_date') ?></td><td><?= ems_sf($r, 'data_state') ?></td><td><?= ems_sf($r, 'source_ref') ?></td><td><?= ems_sf($r, 'line_uid') ?></td><td><?= ems_sf($r, 'request_item_ref') ?></td><td><?= ems_sf($r, 'item_code') ?></td><td><?= ems_sf($r, 'offered_unit_price') ?></td><td><?= ems_sf($r, 'quantity') ?></td><td><?= ems_sf($r, 'item_lead_time') ?></td><td><?= ems_sf($r, 'alternative_proposed') ?></td><td><?= ems_sf($r, 'technical_note') ?></td><td><?= ems_sf($r, 'item_financial_ranking') ?></td></tr>
